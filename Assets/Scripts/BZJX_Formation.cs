...
public class BZJX_Formation : MonoBehaviour
{
	...
	public static BZJX_Formation G_Formation;

	private BZJX_FISH_TYPE FishType;

	private BZJX_FORMATION FormationType;

	private Vector3 rotVector = default(Vector3);

	public static BZJX_Formation GetSingleton()
	{
		return G_Formation;
	}
	...
	private void CreateAFish(BZJX_FORMATION formationType, float x, float y, float rot, BZJX_FISH_TYPE FishType, int i, float index = 0f)
	{
		Transform transform = BZJX_FishPoolMngr.GetSingleton().CreateFishForBig(FishType, i);
		rotVector = transform.eulerAngles;
		rotVector.x = rot / (float)Math.PI * 180f;
		transform.eulerAngles = rotVector;
		transform.GetComponent<BZJX_NormalFish>().InitNormalFish(FormationType, x, y, rot, index, FishType, i);
		transform.GetComponent<BZJX_NormalFish>().StartMove(move: true);
	}

	public void ShowFormation(BZJX_FORMATION type)
	{
	...
	private void CreateFormationBigFishes()
	{
		...
		for (int i = 0; i < 64; i++)
		{
			...
			if (i < 8)
			{
				FishType = BZJX_FISH_TYPE.Fish_GoldenDragon;
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_Beauty;
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_GoldenSharkB;
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_Butterfly;
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_Arrow;
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_GoldenShark;
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_SilverShark;
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_Bat;
			...
			num4 /= 110f;
			num5 /= 110f;
			FormationType = BZJX_FORMATION.Formation_BigFishes;
	...
	private void CreateFormation_Fluctuate()
	{
		...
		for (int i = 0; i < 250; i++)
		{
			if (i < 70)
			{
				FishType = BZJX_FISH_TYPE.Fish_Shrimp;
				if (i % 2 == 0)
				{
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_Zebra;
				if (i % 2 == 0)
				{
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_YellowSpot;
				if (i % 2 == 0)
				{
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_Lamp;
				if (i % 2 == 0)
				{
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_Trailer;
				if (i % 2 == 0)
				{
			...
			num /= 110f;
			num2 /= 110f;
			FormationType = BZJX_FORMATION.Formation_Fluctuate;
		...
		for (int j = 250; j < 500; j++)
		{
			if (j < 320)
			{
				FishType = BZJX_FISH_TYPE.Fish_Grass;
				if (j % 2 == 0)
				{
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_BigEars;
				if (j % 2 == 0)
				{
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_Ugly;
				if (j % 2 == 0)
				{
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_Hedgehog;
				if (j % 2 == 0)
				{
			...
			{
				FishType = BZJX_FISH_TYPE.Fish_Turtle;
				if (j % 2 == 0)
				{
	...
	private void CreateFormation_ConcentricCircles()
	{
		...
		for (int i = 0; i < 119; i++)
		{
			...
			FormationType = BZJX_FORMATION.Formation_ConcentricCircles;
	...
	private void CreateFormation_NineCircle()
	{
		...
		for (int i = 0; i < 160; i++)
		{
			...
			num7 /= 110f;
			num8 /= 110f;
			FormationType = BZJX_FORMATION.Formation_NineCircle;
			CreateAFish(FormationType, num7, num8, num6, FishType, i, num4);
		}
	}
	...
	private void CreateFormation_Hexagonal()
	{
		...
		for (int i = 0; i < num4; i++)
		{
			...
			num8 /= 110f;
			num9 /= 110f;
			FormationType = BZJX_FORMATION.Formation_Hexagonal;
	...
	private void CreateFormationRedCarpet()
	{
		for (int i = 0; i < 160; i++)
		{
			...
			num2 /= 110f;
			int i2 = i;
			FormationType = BZJX_FORMATION.Formation_RedCarpet;
	...
	private void CreateYaoQianShu(int left)
	{
		...
		for (int i = 0; i < 149; i++)
		{
			...
			num2 /= 100f;
			int i2 = i;
			FormationType = BZJX_FORMATION.Formation_YaoQianShuL;
		...
		for (int l = 0; l < 6; l++)
		{
			if (l < 2)
			{
				num = 240f + num3;
				num2 = 250 - l * 500;
				FishType = BZJX_FISH_TYPE.Fish_Penguin;
			}
			else if (l < 4)
			{
	...
	{
		BZJX_FishPoolMngr.GetSingleton().DestroyFish(fish);
	}
