...
public class TF_Sockets : MonoBehaviour
{
	public bool isReconnect = true;

	private static TF_Sockets _MyCreateSocket;
	...
	private TF_DataEncrypt m_DataEncrypt;
	...
	public static TF_Sockets GetSingleton()
	{
	...
	public void CreateSocketGetPoint(TF_MessageControl MyMessageControl, TF_DataEncrypt MyDataEncrypt)
	{
		CreateReceiveThread();
	...
	public void SocketConnect()
	{
		...
		if (m_nSocketStartFlag)
		{
			if (!sendCheckVersion)
			{
				SendCheckVersion(TF_Constants.VERSION_CODE);
	...
	private bool _socketStart()
	{
		...
		IPEndPoint remoteEP = new IPEndPoint(IPAddress.Parse(TF_GameInfo.getInstance().IP), 19900);
		try
		{
	...
	private void _receiveNetMessage(byte[] msgBytes)
	{
		...
		else
		{
			try
			{
				hashtable = TF_ReadFileTool.JsonToClass<Hashtable>(@string);
			}
			catch (Exception)
			{
				msgBytes = m_DataEncrypt.DecompressBytes(msgBytes);
				Console.WriteLine(Encoding.UTF8.GetString(msgBytes));
				hashtable = TF_ReadFileTool.JsonToClass<Hashtable>(Encoding.UTF8.GetString(msgBytes));
	...
	public bool _sendMsg(string strMethod, object[] args)
	{
		...
		else
		{
			...
			hashtable.Add("version", TF_Constants.Version);
			hashtable.Add("time", m_DataEncrypt.GetUnixTime());
			string s = TF_ReadFileTool.JsonByObject(hashtable);
	...
	public bool SendRequestSeat(int deskid, int seatid)
	{
	...
	public bool SendCoinIn(int coinInNumber)
	{
	...
	public bool SendGunHitfish(int gunid, TF_HitFish[] hitfish1, bool bHaveKnife, TF_HitFish[] hitfish2)
	{
		string empty = string.Empty;
