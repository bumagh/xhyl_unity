...
public class CRL_AlertDialog : CRL_MB_Singleton<CRL_AlertDialog>
{
	...
	private void Awake()
	{
		...
		InitFinGame();
		if (CRL_MB_Singleton<CRL_AlertDialog>._instance == null)
		{
			CRL_MB_Singleton<CRL_AlertDialog>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void ShowDialog(string content, bool showOkCancel = false, Action callback = null)
	{
		...
		CRL_MB_Singleton<CRL_GameRoot>.GetInstance().StartCoroutine(CRL_Utils.DelayCall(delay, delegate
		{
	...
	{
		isTouchForbidden = !CRL_MB_Singleton<CRL_GameManager>.GetInstance().GetTouchEnable();
		if (isTouchForbidden)
		{
			CRL_MB_Singleton<CRL_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "alert", includeEsc: false);
		}
		...
		if (CRL_MB_Singleton<CRL_OptionsController>.GetInstance() != null)
		{
	...
	public void Hide()
	{
		...
		if (isTouchForbidden)
		{
			CRL_MB_Singleton<CRL_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "alert done, reforbidden", includeEsc: false);
		}
	}
	...
	public void OnBtnCancel_Click()
	{
		if (!CRL_MySqlConnection.tryLockOnePoint)
		{
			if (CRL_SoundManager.Instance != null)
			{
