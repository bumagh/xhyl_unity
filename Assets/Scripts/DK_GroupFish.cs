...
public class DK_GroupFish : DK_ISwimObj
{
	...
	protected override void _setFishLayer()
	{
		...
		else if (mFishType >= DK_FISH_TYPE.Fish_BigEars_Group && mFishType <= DK_FISH_TYPE.Fish_Double_Kill)
		{
			num = 540 + (int)(mFishType - 33) * 40;
		}
		int num2 = 840 - num;
		int fishIndexInLayer = DK_FishPoolMngr.GetSingleton().GetFishIndexInLayer(mFishType);
		layer = num2 - fishIndexInLayer;
		if (mFishType >= DK_FISH_TYPE.Fish_BigEars_Group && mFishType <= DK_FISH_TYPE.Fish_Double_Kill)
		{
			for (int i = 0; i < fishChildCount; i++)
			{
