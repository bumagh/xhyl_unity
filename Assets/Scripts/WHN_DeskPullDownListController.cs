...
public class WHN_DeskPullDownListController : MonoBehaviour
{
	public static WHN_DeskPullDownListController Instance;
	...
	public void OnListButtonDown()
	{
		...
		imgBtns[WHN_MB_Singleton<WHN_LobbyViewController>.GetInstance().curDeskIndex].sprite = arrImgButton[1];
	...
	public void ChangeListHeightByDeskNum(int desknum, List<WHN_Desk> desklist)
	{
	...
	public void UpdateDeskInList(List<WHN_Desk> desks)
	{
	...
	public void SelectDeskByButton(int index)
	{
		if (bMoveFinished)
		{
			if (index > WHN_MB_Singleton<WHN_LobbyViewController>.GetInstance().curDeskIndex)
			{
				WHN_MB_Singleton<WHN_LobbyViewController>.GetInstance().ForceMoveDesksToRight(index);
			}
			else if (index < WHN_MB_Singleton<WHN_LobbyViewController>.GetInstance().curDeskIndex)
			{
				WHN_MB_Singleton<WHN_LobbyViewController>.GetInstance().ForceMoveDesksToLeft(index);
			}
