...
public class BCBM_ChatPanel : MonoBehaviour
{
	...
	public int ChatSeatID
	{
		set
		{
			if (value < 1 || value > 8)
			{
				BCBM_ErrorManager.GetSingleton().AddError("私聊玩家座位序号错误：" + value);
				value = 1;
			}
			mChatSeatId = value;
		}
	...
	{
		if (BCBM_GameInfo.getInstance().UserInfo.IsGlobalFibbidChat || BCBM_GameInfo.getInstance().UserInfo.IsSelfFibbidChat)
		{
	...
	{
		float num = BCBM_MyUICommon.CalculateCharCount(textLabel.text);
	...
	{
		if ((!BCBM_GameInfo.getInstance().Setted.bIsForbidPublicChat && iChatType == 1) || (!BCBM_GameInfo.getInstance().Setted.bIsForbidPrivateChat && iChatType == 0))
		{
	...
	public void _onClickPublicSendBtn()
	{
		...
		else if (!BCBM_MyTest.TEST)
		{
			BCBM_NetMngr.GetSingleton().MyCreateSocket.SendSendChat(1, mChatReceiverKeyId, mPublicInput.text);
	...
	public void _onClickPrivateSendBtn()
	{
		...
		else
		{
			if (!BCBM_MyTest.TEST)
			{
				BCBM_NetMngr.GetSingleton().MyCreateSocket.SendSendChat(0, mChatReceiverKeyId, mPrivateInput.text);
