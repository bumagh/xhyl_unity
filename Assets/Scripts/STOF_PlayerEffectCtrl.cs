...
public class STOF_PlayerEffectCtrl : MonoBehaviour
{
	...
	public void ShowCoin(STOF_FISH_TYPE fishType, Vector3 pos)
	{
		if (fishType < STOF_FISH_TYPE.Fish_Turtle || (fishType < STOF_FISH_TYPE.Fish_Same_Turtle && fishType >= STOF_FISH_TYPE.Fish_Same_Shrimp))
		{
			_showSilver(STOF_EffectMngr.GetSingleton().GetFlyCoinNum(fishType), pos);
			STOF_MusicMngr.GetSingleton().PlayGameSound(STOF_MusicMngr.GAME_SOUND.SOUND_SILVER_COIN);
		}
		else
		{
			_showGold(STOF_EffectMngr.GetSingleton().GetFlyCoinNum(fishType), pos);
			STOF_MusicMngr.GetSingleton().PlayGameSound(STOF_MusicMngr.GAME_SOUND.SOUND_GOLD_COIN);
		}
	}

	public void ShowLiziCard(Vector3 pos)
	{
		Transform transform = STOF_EffectMngr.GetSingleton().CreateEffectObj(_Lizi.transform);
		if (pos.x < -6f)
		{
		...
		transform.GetComponent<STOF_FlyObj>().AnimatedFlyTo(_playerPos);
		_fLiziTime += 20f;
		STOF_GameInfo.getInstance().GameScene.PlayEnergyGun(mPlayerIndex, isPlay: true);
	}
	...
	public void ShowFishScore(Vector3 pos, int nScore)
	{
		if (nScore > 0)
		{
			Transform transform = STOF_EffectMngr.GetSingleton().CreateEffectObj(_Score.transform);
			...
			transform.GetComponent<STOF_FlyObj>().AnimatedShow(_playerPos);
		}
	}
	...
	private void _showGold(int nCoinNum, Vector3 pos)
	{
		...
		{
			Transform transform = STOF_EffectMngr.GetSingleton().CreateEffectObj(objCoinGold.transform);
			float d = -1f;
			...
			transform.GetComponent<STOF_FlyObj>().FlyTo(_playerPos);
		}
	}
	...
	private void _showSilver(int nCoinNum, Vector3 pos)
	{
		for (int i = 0; i < nCoinNum; i++)
		{
			Transform transform = STOF_EffectMngr.GetSingleton().CreateEffectObj(objCoinSilver.transform);
	...
	private void _updateBigPrizePlate()
	{
		...
		if (_prizePlateQue.Count >= 1 && _showTimePlate >= 1f)
		{
			...
			{
				_curPrizePlateObj.GetComponent<STOF_PrizePlate>().DestroyObj();
			}
			int nNum = _prizePlateQue.Dequeue();
			_curPrizePlateObj = STOF_EffectMngr.GetSingleton().CreateEffectObj(_prizePlate.transform).gameObject;
			_curPrizePlateObj.GetComponent<STOF_PrizePlate>().ShowScore(nNum, _playerPos, mPlayerIndex);
			_showTimePlate = 0f;
			STOF_MusicMngr.GetSingleton().PlayGameSound(STOF_MusicMngr.GAME_SOUND.SOUND_SCORE_PANEL);
		}
	}
	...
	private void _updateLiziBarBette()
	{
		if (_fLiziTime > 0f)
		{
			...
			if (_fLiziTime <= 0f)
			{
				STOF_GameInfo.getInstance().GameScene.PlayEnergyGun(mPlayerIndex, isPlay: false);
			}
		}
		else
		{
