...
public class WHN_GameRoot : WHN_MB_Singleton<WHN_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(WHN_MB_Singleton<WHN_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            WHN_MB_Singleton<WHN_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : WHN_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : WHN_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
