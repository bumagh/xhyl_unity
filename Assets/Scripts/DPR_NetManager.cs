...
public class DPR_NetManager : DPR_MB_Singleton<DPR_NetManager>
{
	public delegate void NetMessageHandler(object[] args);

	private DPR_ClientSocket _pClient;

	private DPR_DataEncrypt _pCrypto;
	...
	private Queue<DPR_NetMessage> _netMsgQueue = new Queue<DPR_NetMessage>();

	private Dictionary<string, NetMessageHandler> _netMsgHandlerDic = new Dictionary<string, NetMessageHandler>();

	private void Awake()
	{
		if (DPR_MB_Singleton<DPR_NetManager>._instance == null)
		{
			DPR_MB_Singleton<DPR_NetManager>.SetInstance(this);
		}
	...
	private void _initClientSocket()
	{
		...
		_pClient = new DPR_ClientSocket(useTcpClient: true);
		_pCrypto = new DPR_DataEncrypt();
	...
	private void _handleException(Exception ex)
	{
		...
		isReady = false;
		isLogined = false;
		Queue<DPR_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
			_netMsgQueue.Enqueue(new DPR_NetMessage("netDown", new object[1]
			{
				ex
			}));
		}
	...
	private void _handleConnected()
	{
		try
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new DPR_NetMessage("netConnected", new object[0]));
			}
		}
		catch (Exception ex)
		{
	...
	{
		UnityEngine.Debug.Log("ZH2_GVars.versionCode: " + DPR_MySqlConnection.versionCode);
		Send("userService/checkVersion", new object[1]
		{
			DPR_MySqlConnection.versionCode
		});
	}
	...
	protected void _handleData(byte[] msgBuffer, int packetId, int endIndex)
	{
		...
		lock (netMsgQueue)
		{
			DPR_NetMessage dPR_NetMessage = new DPR_NetMessage(text, args);
			dPR_NetMessage.jsonString = @string;
			dPR_NetMessage.packetId = packetId;
			dPR_NetMessage.endIndex = endIndex;
			_netMsgQueue.Enqueue(dPR_NetMessage);
		}
	}

	private IEnumerator _checkTimeout(int id, int timeout, DPR_ClientSocket cs)
	{
		UnityEngine.Debug.Log(DPR_LogHelper.Aqua("_checkTimeout start id: {0}", id));
		...
		if (cs != null && cs.doneList.Count > id && !cs.doneList[id])
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new DPR_NetMessage("timeout", new object[1]
				{
					id
				}));
				isTimeout = true;
		...
		if (isTimeout)
		{
			UnityEngine.Debug.Log(DPR_LogHelper.Aqua("_checkTimeout start id: {0} timeout: {1}", id, isTimeout));
		}
	}
	...
	private IEnumerator _checkHeartTimeout(int timeout)
	{
		...
		if (isReady && !isHeartPong)
		{
			...
			lock (netMsgQueue)
			{
				...
				isReady = false;
				isLogined = false;
				_netMsgQueue.Enqueue(new DPR_NetMessage("heart timeout", new object[0]));
				isTimeout = true;
			}
		}
		if (isTimeout)
		{
			UnityEngine.Debug.Log(DPR_LogHelper.Aqua("_checkHeartTimeout is timeout"));
		}
	}

	public void _checkTimeoutStart(int id, int timeout, DPR_ClientSocket cs)
	{
	...
	public void Send(string method, object[] args)
	{
		...
		{
			UnityEngine.Debug.Log(DPR_LogHelper.Magenta("drop send>>method: {0}", method));
			return;
		}
		Hashtable hashtable = new Hashtable();
		hashtable.Add("method", method);
		hashtable.Add("args", args);
		hashtable.Add("version", DPR_MySqlConnection.Version);
		if (isReady)
		{
	...
	private bool GetNetMessage(out DPR_NetMessage netMsg)
	{
		Queue<DPR_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
	...
	private void DispatchNetMessage(DPR_NetMessage netMsg)
	{
		if (dropDispatch)
		{
			UnityEngine.Debug.Log(DPR_LogHelper.Magenta("drop send>>method: {0}", netMsg.method));
		}
