...
public class ESP_GameRoot : ESP_MB_Singleton<ESP_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(ESP_MB_Singleton<ESP_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            ESP_MB_Singleton<ESP_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : ESP_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : ESP_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
