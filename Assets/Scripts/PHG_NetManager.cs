...
public class PHG_NetManager : PHG_MB_Singleton<PHG_NetManager>
{
	public delegate void NetMessageHandler(object[] args);

	private PHG_ClientSocket _pClient;

	private PHG_DataEncrypt _pCrypto;
	...
	private Queue<PHG_NetMessage> _netMsgQueue = new Queue<PHG_NetMessage>();

	private Dictionary<string, NetMessageHandler> _netMsgHandlerDic = new Dictionary<string, NetMessageHandler>();

	private void Awake()
	{
		if (PHG_MB_Singleton<PHG_NetManager>._instance == null)
		{
			PHG_MB_Singleton<PHG_NetManager>.SetInstance(this);
		}
	...
	private void _initClientSocket()
	{
		...
		_pClient = new PHG_ClientSocket(useTcpClient: true);
		_pCrypto = new PHG_DataEncrypt();
	...
	private void _handleException(Exception ex)
	{
		...
		isReady = false;
		isLogined = false;
		Queue<PHG_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
			_netMsgQueue.Enqueue(new PHG_NetMessage("netDown", new object[1]
			{
				ex
			}));
		}
	...
	private void _handleConnected()
	{
		try
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new PHG_NetMessage("netConnected", new object[0]));
			}
		}
		catch (Exception ex)
		{
	...
	{
		UnityEngine.Debug.Log("ZH2_GVars.versionCode: " + PHG_GVars.versionCode);
		Send("userService/checkVersion", new object[1]
		{
			PHG_GVars.versionCode
		});
	}
	...
	protected void _handleData(byte[] msgBuffer, int packetId, int endIndex)
	{
		...
		lock (netMsgQueue)
		{
			PHG_NetMessage pHG_NetMessage = new PHG_NetMessage(text, args);
			pHG_NetMessage.jsonString = @string;
			pHG_NetMessage.packetId = packetId;
			pHG_NetMessage.endIndex = endIndex;
			_netMsgQueue.Enqueue(pHG_NetMessage);
		}
	}

	private IEnumerator _checkTimeout(int id, int timeout, PHG_ClientSocket cs)
	{
		UnityEngine.Debug.Log(PHG_LogHelper.Aqua("_checkTimeout start id: {0}", id));
		...
		if (cs != null && cs.doneList.Count > id && !cs.doneList[id])
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new PHG_NetMessage("timeout", new object[1]
				{
					id
				}));
				isTimeout = true;
		...
		if (isTimeout)
		{
			UnityEngine.Debug.Log(PHG_LogHelper.Aqua("_checkTimeout start id: {0} timeout: {1}", id, isTimeout));
		}
	}
	...
	private IEnumerator _checkHeartTimeout(int timeout)
	{
		...
		if (isReady && !isHeartPong)
		{
			...
			lock (netMsgQueue)
			{
				...
				isReady = false;
				isLogined = false;
				_netMsgQueue.Enqueue(new PHG_NetMessage("heart timeout", new object[0]));
				isTimeout = true;
			}
		}
		if (isTimeout)
		{
			UnityEngine.Debug.Log(PHG_LogHelper.Aqua("_checkHeartTimeout is timeout"));
		}
	}

	public void _checkTimeoutStart(int id, int timeout, PHG_ClientSocket cs)
	{
	...
	public void Send(string method, object[] args)
	{
		...
		{
			UnityEngine.Debug.Log(PHG_LogHelper.Magenta("drop send>>method: {0}", method));
			return;
		}
		Hashtable hashtable = new Hashtable();
		hashtable.Add("method", method);
		hashtable.Add("args", args);
		hashtable.Add("version", PHG_GVars.Version);
		if (isReady)
		{
	...
	private bool GetNetMessage(out PHG_NetMessage netMsg)
	{
		Queue<PHG_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
	...
	private void DispatchNetMessage(PHG_NetMessage netMsg)
	{
		if (dropDispatch)
		{
			UnityEngine.Debug.Log(PHG_LogHelper.Magenta("drop send>>method: {0}", netMsg.method));
		}
