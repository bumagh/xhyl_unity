...
public class PTM_DeskPullDownListController : MonoBehaviour
{
	public static PTM_DeskPullDownListController Instance;
	...
	public void OnListButtonDown()
	{
		...
		imgBtns[PTM_MB_Singleton<PTM_LobbyViewController>.GetInstance().curDeskIndex].sprite = arrImgButton[1];
	...
	public void ChangeListHeightByDeskNum(int desknum, List<PTM_Desk> desklist)
	{
	...
	public void UpdateDeskInList(List<PTM_Desk> desks)
	{
	...
	public void SelectDeskByButton(int index)
	{
		if (bMoveFinished)
		{
			if (index > PTM_MB_Singleton<PTM_LobbyViewController>.GetInstance().curDeskIndex)
			{
				PTM_MB_Singleton<PTM_LobbyViewController>.GetInstance().ForceMoveDesksToRight(index);
			}
			else if (index < PTM_MB_Singleton<PTM_LobbyViewController>.GetInstance().curDeskIndex)
			{
				PTM_MB_Singleton<PTM_LobbyViewController>.GetInstance().ForceMoveDesksToLeft(index);
			}
