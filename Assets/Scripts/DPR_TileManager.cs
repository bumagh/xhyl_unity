...
public class DPR_TileManager : MonoBehaviour
{
	...
	public IEnumerator StartChasing(int targetIndex, Action endAction)
	{
		...
		DPR_XWait xwait = new DPR_XWait(this).Begin();
		int count = 0;
		float timeCount = 0f;
	...
	public IEnumerator WaitSecondsAction(float seconds, Action<int> action, int param1, DPR_XWait xwait = null)
	{
		if (xwait == null)
		{
			yield return new WaitForSeconds(seconds);
