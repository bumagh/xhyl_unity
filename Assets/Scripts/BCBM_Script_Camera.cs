...
public class BCBM_Script_Camera : MonoBehaviour
{
	...
	public void PlayAnimation(bool isPlay)
	{
		if (isPlay)
		{
			iTween.MoveTo(_curLookAtTransform.gameObject, iTween.Hash("position", _targetPoint, "time", 2f, "delay", BCBM_Parameter.G_fAnimalBounceTime + 0.8f, "easetype", iTween.EaseType.easeInOutSine));
			iTween.MoveTo(base.gameObject, iTween.Hash("position", _doPrizeDestPos, "time", 2f, "delay", BCBM_Parameter.G_fAnimalBounceTime + 0.8f, "easetype", iTween.EaseType.easeInOutSine));
		}
		else
		{
			_Reset();
