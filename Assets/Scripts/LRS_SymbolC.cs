...
public class LRS_SymbolC : MonoBehaviour
{
	...
	private string aniname = "img{0:0}_";

	private string aniname2 = "img{0:0}_";
	...
	private void Awake()
	{
		...
		symlen = Cinstance<LRS_Gcore>.Instance.Symcont;
	...
	public void Init(int idx, int symid, LRS_Line line = null)
	{
		symidx = idx;
		lineroll = line;
	...
	{
		iconimg.sprite = Cinstance<LRS_Gcore>.Instance.SymSprlist[idx];
	}
	...
	public void StopRoll()
	{
		...
		if (isroll)
		{
			...
			if (Inlineidx > 2)
			{
				...
				int idx = Cinstance<LRS_Gcore>.Instance.Result[index];
				SetSymbolimg(idx, 0L);
			}
		}
		LRS_SoundManager.Instance.PlayStopAndio();
		speed = normalspeed;
	}
	...
	private void Setrollnum(List<int> randomlist, int rolllen)
	{
		...
		if (Inlineidx > 2)
		{
			...
			int value = Cinstance<LRS_Gcore>.Instance.Result[index2];
	...
	public void Playani(int winline = -1)
	{
		...
		if (SymbolId == 10)
		{
			try
			{
				Seq_ani = iconimg.LRS_PlayAni(string.Format(aniname, SymbolId + 1) + "{0:0}", string.Format(aniname, SymbolId + 1) + "{0:0}", 1, frames);
				Seq_ani.Restart();
			}
			catch (Exception arg)
			{
