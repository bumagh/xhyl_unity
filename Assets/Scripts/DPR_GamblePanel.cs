...
public class DPR_GamblePanel : MonoBehaviour
{
	...
	public static DPR_GamblePanel ins;
	...
	{
		DPR_MB_Singleton<DPR_DiceGameController2>.GetInstance().Send_MultipleInfo(DPR_BetDiceType.Half);
		SetButton(canclick: false);
	}

	private void ClickBlack()
	{
		DPR_MB_Singleton<DPR_DiceGameController2>.GetInstance().Send_MultipleInfo(DPR_BetDiceType.Risk);
		SetButton(canclick: false);
	}
	...
	private Sprite Getsprite()
	{
		try
		{
			uint num = Getnum(DPR_MB_Singleton<DPR_DiceGameController2>.GetInstance().Gamelewinid);
	...
	{
		balancescore = DPR_MySqlConnection.winScorce;
		doublescore = DPR_MySqlConnection.winScorce * 2;
	...
	{
		DPR_MB_Singleton<DPR_DiceGameController2>.GetInstance().OnBtnReady_Click();
		DPR_MB_Singleton<DPR_DiceGameController2>.GetInstance().Hide();
		int credit = (!isGamble) ? (DPR_MySqlConnection.winScorce + DPR_MySqlConnection.credit) : AllScroce;
		DPR_MB_Singleton<DPR_MajorGameController>.GetInstance().SetCredit(credit);
