		...
		public TupleSerializer(RuntimeTypeModel model, ConstructorInfo ctor, MemberInfo[] members)
		{
			...
			while (true)
			{
				if (num < members.Length)
				{
					...
					type = ((!(itemType == null)) ? itemType : parameterType);
					bool asReference = false;
					int num2 = model.FindOrAddAuto(type, demand: false, addWithContractOnly: true, addEvenIfAutoDisabled: false);
					if (num2 >= 0)
					{
