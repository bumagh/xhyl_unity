...
namespace ProtoBuf
{
	public abstract class Extensible : IExtensible
	{
		...
		public static bool TryGetValue(TypeModel model, Type type, IExtensible instance, int tag, DataFormat format, bool allowDefinedTag, out object value)
		{
			value = null;
			bool result = false;
			IEnumerator enumerator = ExtensibleUtil.GetExtendedValues(model, type, instance, tag, format, singleton: true, allowDefinedTag).GetEnumerator();
			try
			{
				while (enumerator.MoveNext())
				{
					object obj = value = enumerator.Current;
					result = true;
				}
				return result;
			}
