	...
	public sealed class RuntimeTypeModel : TypeModel
	{
		...
		public override string GetSchema(Type type)
		{
			...
			if (!flag)
			{
				...
				try
				{
					...
					{
						MetaType metaType3 = (MetaType)enumerator2.Current;
						if (!metaType3.IsList)
						{
		...
		internal IProtoSerializer TryGetBasicTypeSerializer(Type type)
		{
			...
			lock (basicTypes)
			{
				...
				WireType defaultWireType;
				IProtoSerializer protoSerializer = (MetaType.GetContractFamily(this, type, null) != 0) ? null : ValueMember.TryGetCoreSerializer(this, DataFormat.Default, type, out defaultWireType, asReference: false, dynamicType: false, overwriteList: false, allowComplexTypes: false);
				if (protoSerializer != null)
				{
