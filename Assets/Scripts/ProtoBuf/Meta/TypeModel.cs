...
namespace ProtoBuf.Meta
{
	...
	{
		private sealed class DeserializeItemsIterator<T> : DeserializeItemsIterator, IEnumerator<T>, IEnumerable<T>, IEnumerator, IDisposable, IEnumerable
		{
				...
				if (enumerable != null)
				{
					...
					try
					{
						while (enumerator.MoveNext())
						{
							...
							if (!TrySerializeAuxiliaryType(writer, null, format, tag, current, isInsideList: true))
							{
								ThrowUnexpectedType(current.GetType());
							}
						}
					}
					finally
		...
		private bool TryDeserializeList(TypeModel model, ProtoReader reader, DataFormat format, int tag, Type listType, Type itemType, ref object value)
		{
			...
			object[] array = (!isList) ? new object[1] : null;
		...
		private bool CanSerialize(Type type, bool allowBasic, bool allowContract, bool allowLists)
		{
			...
			ProtoTypeCode typeCode = Helpers.GetTypeCode(type);
			if (typeCode != 0 && typeCode != ProtoTypeCode.Unknown)
			{
				return allowBasic;
			}
			int key = GetKey(ref type);
