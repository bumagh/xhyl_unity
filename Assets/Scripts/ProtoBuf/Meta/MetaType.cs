...
namespace ProtoBuf.Meta
{
	public class MetaType : ISerializerProxy
	{
		...
		internal bool IsList
		{
			get
			{
				Type left = (!IgnoreListHandling) ? TypeModel.GetListItemType(model, type) : null;
				return left != null;
			}
		}
		...
		internal void ApplyDefaultBehaviour()
		{
			...
			if (left != null && model.FindWithoutAdd(left) == null && GetContractFamily(model, left, null) != 0)
			{
				model.FindOrAddAuto(left, demand: true, addWithContractOnly: false, addEvenIfAutoDisabled: false);
			}
			...
			foreach (AttributeMap attributeMap in array)
			{
				...
				if (!flag && fullName == "ProtoBuf.ProtoIncludeAttribute")
				{
					...
					try
					{
						...
						{
							type = model.GetType((string)value, this.type.Assembly);
						}
