...
public class TF_PrefabPool
{
	...
	public TF_PrefabPool(Transform prefab)
	{
		this.prefab = prefab;
		prefabGO = prefab.gameObject;
	}

	public TF_PrefabPool()
	{
	}
	...
	internal bool DespawnInstance(Transform xform)
	{
		...
		TF_PoolManagerUtils.SetActive(xform.gameObject, state: false);
	...
	internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
	{
		...
		else
		{
			...
			TF_PoolManagerUtils.SetActive(transform2.gameObject, state: true);
		}
		if (transform2 != null)
		{
	...
	internal void AddUnpooled(Transform inst, bool despawn)
	{
		...
		if (despawn)
		{
			TF_PoolManagerUtils.SetActive(inst.gameObject, state: false);
			_despawned.Add(inst);
		}
		else
		{
