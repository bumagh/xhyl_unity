...
public class PTM_LoadingViewController : PTM_MB_Singleton<PTM_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (PTM_MB_Singleton<PTM_LoadingViewController>._instance == null)
		{
			PTM_MB_Singleton<PTM_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (PTM_MB_Singleton<PTM_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!PTM_MB_Singleton<PTM_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
