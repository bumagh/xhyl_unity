...
public class BCBM_GameMagert : MonoBehaviour
{
	...
	private static BCBM_GameMagert _instance;
	...
	public BCBM_MessageBoxPopup messg;

	public static bool swithonislistchange;

	public BCBM_countdowdata nowdata;

	public BCBM_playstate ChangeState;
	...
	private BCBM_playstate NowState;
	...
	private List<BCBM_poketstate> severpokethuase;

	private List<BCBM_poketstate> severpokethuase2;
	...
	public static BCBM_GameMagert Instance
	{
		get
		{
			return _instance;
	...
	private void init()
	{
		mainText.text = "切换 " + BCBM_LoginInfo.Instance().mylogindata.roomcount;
		vecFir = (gonggao.rectTransform.localPosition += new Vector3(gonggao.preferredWidth, 0f, 0f));
		ChangeState = BCBM_playstate.betready;
		isfrist = true;
		...
		betinlist = new int[5];
		gonggaocount = 0;
		BCBM_LoginInfo.Instance();
		servicesPanelText.text = BCBM_LoginInfo.Instance().mylogindata.servicesInfo;
		aGrid = 0;
		fristopen = true;
		fristuserin = true;
		isbetincheak = true;
		...
		BCBM_LoginInfo.Instance().mylogindata.isOpenError = true;
		countouit = 0;
		rushin = 0;
		BCBM_LoginInfo.Instance().mylogindata.coindown = int.Parse(BCBM_LoginInfo.Instance().mylogindata.roomcount);
		historyuserin = new int[5];
		swithonislistchange = true;
		severpoketcode = new List<int>();
		severpokethuase = new List<BCBM_poketstate>();
		severpokethuase2 = new List<BCBM_poketstate>();
		nowdata = new BCBM_countdowdata();
		buttonrate = new List<string>();
		buttonIDcode = new List<int>();
		erroropendatacout = 0;
		titlettextlist[0].text = BCBM_LoginInfo.Instance().mylogindata.username;
		titlettextlist[1].text = "0";
		titlettextlist[2].text = "0";
		titlettextlist[3].text = "0";
		titlettextlist[4].text = "0";
		titlettextlist[5].text = "0";
		limittext.text = BCBM_LoginInfo.Instance().mylogindata.roomlitmit;
		lessdown.text = BCBM_LoginInfo.Instance().mylogindata.roomcount;
		gotohallint = 0;
	...
	private void Start()
	{
		...
		getpoint(Convert.ToDouble(BCBM_LoginInfo.Instance().mylogindata.ALLScroce));
		setUserUIinfo();
		addlistenrt();
	...
	private void changecountall(List<BCBM_poketstate> list)
	{
		for (int i = 0; i < list.Count; i++)
		{
			if (list[i] > BCBM_poketstate.Diamond)
			{
	...
	private void changecountallright(List<BCBM_poketstate> list)
	{
		for (int i = 0; i < list.Count; i++)
		{
	...
	private void changecountall(BCBM_poketstate state)
	{
		if (state > BCBM_poketstate.Diamond)
		{
	...
	private void changecountallright(BCBM_poketstate state)
	{
	...
	private IEnumerator cheakwillfixed(string URL)
	{
		...
		if (www.error == null && NowState != BCBM_playstate.betclearing && !isopendatenocheakswith && jd["code"].ToString() == "200")
		{
			if (jd["ArrList"].Count < countsize)
			{
				startclosepoket(0);
				yield return StartCoroutine(initpoketlist(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.winlistAPI, 1));
			}
			yield return new WaitForSeconds(1f);
			if (NowState != BCBM_playstate.betclearing && jd["ArrList"].Count > countsize)
			{
				startclosepoket(0);
				yield return StartCoroutine(initpoketlist(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.winlistAPI, 1));
			}
	...
	private IEnumerator polling()
	{
		while (true)
		{
			...
			try
			{
				StartCoroutine(getdatainitformsever(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.gameinfoPollingAPI + "user_id=" + BCBM_LoginInfo.Instance().mylogindata.user_id + "&room_id=" + BCBM_LoginInfo.Instance().mylogindata.room_id + "&unionuid=" + BCBM_LoginInfo.Instance().mylogindata.token));
				StartCoroutine(BCBM_LoginInfo.Instance().wwwinstance.sendcoutdown(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.counwtAPI));
				if (!isfrist)
				{
					StartCoroutine(cheakwillfixed(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.winlistAPI));
				}
				StartCoroutine(GetAnnouncementInfo(BCBM_LoginInfo.Instance().mylogindata.URL + "api/notice?user_id=" + BCBM_LoginInfo.Instance().mylogindata.user_id + "&game_id=" + BCBM_LoginInfo.Instance().mylogindata.choosegame));
				...
				{
					StartCoroutine(rewww(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.usercoininhistoryAPI + "plate=1&user_id=" + BCBM_LoginInfo.Instance().mylogindata.user_id + "&room_id=" + BCBM_LoginInfo.Instance().mylogindata.room_id, true));
					StartCoroutine(rewww(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.usercoininhistoryAPI + "plate=2&user_id=" + BCBM_LoginInfo.Instance().mylogindata.user_id + "&room_id=" + BCBM_LoginInfo.Instance().mylogindata.room_id, false));
	...
	private IEnumerator rewww(string url, bool leftorright)
	{
		...
		if (www.error == null)
		{
			useropenGO.GetComponent<BCBM_outtextstlye>().addtexttoui(www.downloadHandler.text, leftorright);
		}
		else
		{
	...
	private IEnumerator getdatainitformsever(string url)
	{
		...
		if (www.error == null)
		{
			...
			if (jsonData["code"].ToString() == "200")
			{
				...
				BCBM_LoginInfo.Instance().mylogindata.ALLScroce = num.ToString();
			...
			messg.Show("异常", jsonData["msg"].ToString(), BCBM_ButtonStyle.Confirm, delegate(BCBM_MessageBoxResult call)
			{
				switch (call)
				{
				case BCBM_MessageBoxResult.Cancel:
				{
					BCBM_LoginInfo.Instance().cleanmylogindata();
					AsyncOperation asyncOperation6 = Application.LoadLevelAsync(0);
					asyncOperation6.allowSceneActivation = true;
					break;
				}
				case BCBM_MessageBoxResult.Confirm:
				{
					BCBM_LoginInfo.Instance().cleanmylogindata();
				...
				case BCBM_MessageBoxResult.Timeout:
				{
					BCBM_LoginInfo.Instance().cleanmylogindata();
		...
		gotohallint = 0;
		messg.Show("异常", "网络异常，即将退出游戏", BCBM_ButtonStyle.Confirm, delegate(BCBM_MessageBoxResult call)
		{
			switch (call)
			{
			case BCBM_MessageBoxResult.Cancel:
	...
	private void FixedUpdate()
	{
		...
		if (BCBM_LoginInfo.Instance().wwwinstance.listformwwwinfo.Count <= 0)
		{
			return;
		}
		if (BCBM_LoginInfo.Instance().wwwinstance.listformwwwinfo.Count == 1)
		{
			if (swithonislistchange)
			{
				getinfoandsetstate(BCBM_LoginInfo.Instance().wwwinstance.listformwwwinfo.Peek());
				swithonislistchange = false;
			}
		}
		else if (BCBM_LoginInfo.Instance().wwwinstance.listformwwwinfo.Count > 1)
		{
			BCBM_wwwinfo bCBM_wwwinfo = BCBM_LoginInfo.Instance().wwwinstance.listformwwwinfo.Dequeue();
			swithonislistchange = true;
		}
	}

	private void getinfoandsetstate(BCBM_wwwinfo info)
	{
		switch (info.statetype)
		{
		case 0:
			getdatatojson(info.textinfo);
			changestate(BCBM_playstate.betin);
			break;
		case 1:
			getdatatojson(info.textinfo);
			changestate(BCBM_playstate.betover);
			break;
		case 2:
			getdatatojson(info.textinfo);
			changestate(BCBM_playstate.betclearing);
			break;
		}
	}
	...
	private void getdatatojson(string temp)
	{
		...
		{
			nowdata.wintype1 = BCBM_poketstate.isnull;
			nowdata.wintype2 = BCBM_poketstate.isnull;
		...
		if (isfristopennumber)
		{
			...
			{
				BCBM_NumSpriteControl.Instances.SetImage(changestringtointlist(nowdata.win_number));
			}
			else
			{
				BCBM_NumSpriteControl.Instances.SetImage(changestringtointlist(nowdata.last_winnumber));
			}
	...
	private void changeState(BCBM_playstate state)
	{
		switch (state)
		{
		case BCBM_playstate.betnull:
			break;
		case BCBM_playstate.betready:
			betready();
			break;
		case BCBM_playstate.betin:
			betin();
			break;
		case BCBM_playstate.betover:
			betover();
			break;
		case BCBM_playstate.betopen:
			betwaitopen();
			break;
		case BCBM_playstate.betclearing:
			betcleaning();
			break;
		case BCBM_playstate.bettime:
			bettime();
			break;
		}
	}
	...
	private void betready()
	{
		if (isfrist)
		{
			startclosepoket(0);
			isfrist = false;
			StartCoroutine(initpoketlist(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.winlistAPI, 1));
		}
		else
		{
	...
	private void betin()
	{
		if (!isChangeXiaZhu)
		{
			BCBM_Audiomanger._instenc.ChangeBGMusic();
	...
	{
		StartCoroutine(initpoketlist(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.winlistAPI, 0));
	}
	...
	private void erroroutgame()
	{
		...
		erroropendatacout = 0;
		messg.Show("异常", "您由于在游戏期间离开次数过多，请退出房间后再进入", BCBM_ButtonStyle.Confirm, delegate(BCBM_MessageBoxResult call)
		{
			switch (call)
			{
			case BCBM_MessageBoxResult.Cancel:
	...
	private void betover()
	{
		...
		thistimecoutText.text = "本局总得分：0";
		BCBM_NumSpriteControl.Instances.StartImage();
		...
		if (!isChangeKaiJiang)
		{
			BCBM_Audiomanger._instenc.ChangeBGMusic_KaiJiang();
	...
	private IEnumerator opentolist(int toggle)
	{
		...
		{
		case 1:
			yield return StartCoroutine(initpoketlist(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.winlistAPI, 0));
			BCBM_NumSpriteControl.Instances.StopImage(changestringtointlist(nowdata.win_number));
			isopendatafromsever = true;
			break;
		case 0:
			...
			BCBM_NumSpriteControl.Instances.SetImage(changestringtointlist(nowdata.win_number));
			yield return new WaitForSeconds(3f);
			if (nowdata.wintype1 >= BCBM_poketstate.spdaesJR)
			{
			...
			StartCoroutine(getwindataandopenpanel(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.wingetforinterfroAPI + "user_id=" + BCBM_LoginInfo.Instance().mylogindata.user_id + "&periods=" + nowdata.periods));
			break;
		}
		updatenowcount = 4;
	...
	private IEnumerator addpoket()
	{
		...
		if (nowdata.wintype2 < BCBM_poketstate.HeartsJR)
		{
	...
	private IEnumerator waitsecnd(BCBM_playstate state)
	{
		yield return new WaitForSeconds(1f);
		ChangeState = state;
	}
	...
	private void poketActivetolist(BCBM_poketstate BCBM_poketstate, int size, GameObject poketlistGO)
	{
		if (size >= 48)
		{
		...
		GameObject gameObject = poketGOtoinit.transform.GetChild(0).GetChild((int)BCBM_poketstate).gameObject;
		gameObject.SetActive(true);
		GameObject gameObject2 = ((BCBM_poketstate != 0 && BCBM_poketstate != BCBM_poketstate.Culb) ? poketGOtoinit.transform.GetChild(1).GetChild(1).gameObject : poketGOtoinit.transform.GetChild(1).GetChild(0).gameObject);
		if (gameObject2 != null)
		{
	...
	{
		if (BCBM_LoginInfo.Instance().mylogindata.isOpenError)
		{
			if (goLeft != null)
			{
	...
	private void addlistenrt()
	{
		...
		{
			canelallin(BCBM_LoginInfo.Instance().mylogindata.user_id, nowdata.periods, 1);
		});
		cnealGO.onClick.AddListener(BCBM_Audiomanger._instenc.clickvoice);
		cnealGO2.onClick.AddListener(delegate
		{
			canelallin(BCBM_LoginInfo.Instance().mylogindata.user_id, nowdata.periods, 2);
		});
		cnealGO2.onClick.AddListener(BCBM_Audiomanger._instenc.clickvoice);
		quit.onClick.AddListener(tohall);
		quit.onClick.AddListener(BCBM_Audiomanger._instenc.clickvoice);
	}
	...
	{
		BCBM_LoginInfo.Instance().mylogindata.coindown = value;
	}

	private void btnaddclick(int value, int id, string huase)
	{
		if (NowState != BCBM_playstate.betin)
		{
			if (value <= 4)
			{
				senderror("未到下注时间");
		...
		if (allScroceForUi < (double)int.Parse(BCBM_LoginInfo.Instance().mylogindata.roomcount))
		{
			changeallsroce(0);
			if (value <= 4)
			{
		...
		try
		{
			if (allScroceForUi < 100.0 && BCBM_LoginInfo.Instance().mylogindata.coindown == 100)
			{
				StartCoroutine(betincoin(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.betinAPI + "user_id=" + BCBM_LoginInfo.Instance().mylogindata.user_id + "&num=" + allScroceForUi + "&room_id=" + BCBM_LoginInfo.Instance().mylogindata.room_id + "&drop_content=" + huase + "&id=" + id, value, allScroceForUi));
			}
			else
			{
				StartCoroutine(betincoin(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.betinAPI + "user_id=" + BCBM_LoginInfo.Instance().mylogindata.user_id + "&num=" + BCBM_LoginInfo.Instance().mylogindata.coindown + "&room_id=" + BCBM_LoginInfo.Instance().mylogindata.room_id + "&drop_content=" + huase + "&id=" + id, value, BCBM_LoginInfo.Instance().mylogindata.coindown));
			}
		}
		catch (Exception)
		{
	...
	private void canelallin(string user_id, string ratedate, int plate)
	{
		...
		try
		{
			StartCoroutine(canelbetin(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.caneldownAPI + "user_id=" + user_id + "&drop_date=" + ratedate + "&room_id=" + BCBM_LoginInfo.Instance().mylogindata.room_id + "&plate=" + text, plate));
		}
		catch (Exception)
		{
		}
	...
	private IEnumerator betincoin(string URL, int value, double invalue)
	{
		...
		if (jsonData["code"].ToString() == "200")
		{
			...
			{
				userscorein[value].text = BCBM_LoginInfo.Instance().mylogindata.coindown.ToString();
			}
			else
			{
			...
			{
				uiallintext[value].text = BCBM_LoginInfo.Instance().mylogindata.coindown.ToString();
			}
			else
			{
	...
	{
		BCBM_Audiomanger._instenc.clickvoice();
	...
	private IEnumerator initpoketlist(string URL, int toggle)
	{
		lock (base.gameObject)
		{
			...
			if (temp.error == null && temp.isDone)
			{
				...
				{
					messg.Show("异常", "无法获取以往期数，请重新进入", BCBM_ButtonStyle.Confirm, delegate(BCBM_MessageBoxResult call)
					{
						switch (call)
						{
						case BCBM_MessageBoxResult.Cancel:
	...
	private IEnumerator starsetwinnumber(int[] winnumber)
	{
		...
		{
			BCBM_NumSpriteControl.Instances.StopImage(winnumber[i]);
	...
	{
		BCBM_Audiomanger._instenc.GetComponent<AudioSource>().volume = value;
	}
	...
	public BCBM_poketstate changeTYPEtoint(string temp)
	{
		if (temp.Equals("A1"))
		{
			return BCBM_poketstate.spdaes;
		}
		if (temp.Equals("B1"))
		{
			return BCBM_poketstate.Hearts;
		}
		if (temp.Equals("C1"))
		{
			return BCBM_poketstate.Culb;
		}
		if (temp.Equals("D1"))
		{
			return BCBM_poketstate.Diamond;
		}
		if (temp.Equals("A2"))
		{
			return BCBM_poketstate.spdaesJR;
		}
		if (temp.Equals("B2"))
		{
			return BCBM_poketstate.HeartsJR;
		}
		if (temp.Equals("C2"))
		{
			return BCBM_poketstate.CulbJR;
		}
		if (temp.Equals("D2"))
		{
			return BCBM_poketstate.DiamondJR;
		}
		return BCBM_poketstate.spdaes;
	}
	...
	{
		BCBM_LoginInfo.Instance().wwwinstance.listformwwwinfo.Clear();
		BCBM_LoginInfo.Instance().mylogindata.room_id = 0;
		cheak -= cheakpoketpoolisture;
	}

	private void OnShowTuiShui()
	{
		StartCoroutine(ShowTuiShui(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.userCut + "user_id=" + BCBM_LoginInfo.Instance().mylogindata.user_id));
	}
	...
	private void OnTuiShui()
	{
		...
		{
			StartCoroutine(TuiShui(BCBM_LoginInfo.Instance().mylogindata.URL + "api/" + BCBM_LoginInfo.Instance().mylogindata.userCutSend + "user_id=" + BCBM_LoginInfo.Instance().mylogindata.user_id));
		}
		else
		{
