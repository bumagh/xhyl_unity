...
public class TF_FishPathMngr : MonoBehaviour
{
	public static TF_FishPathMngr G_FishPathMngr;

	private TF_PathManager[] allFishPath;

	private TF_PathManager[] smallFishPath;
	...
	public static TF_FishPathMngr GetSingleton()
	{
	...
	private void Awake()
	{
		...
		int num = 0;
		_AllPathLength = 88;
		allFishPath = new TF_PathManager[_AllPathLength];
		...
		for (int i = 0; i < _AllPathLength; i++)
		{
			...
			allFishPath[i] = new TF_PathManager();
		...
		smallFishPath = new TF_PathManager[_AllPathLength * 4];
	...
	public TF_PathManager GetFishPath(int type)
	{
		return allFishPath[type];
	}

	public TF_PathManager GetSmallFishPath(int type)
	{
		return smallFishPath[type];
	}

	private TF_PathManager ChangePath(TF_PathManager path, Vector3 vec)
	{
		int num = path.vecs.Length;
		TF_PathManager tF_PathManager = new TF_PathManager();
		tF_PathManager.vecs = new Vector3[num];
		for (int i = 0; i < num; i++)
		{
			tF_PathManager.vecs[i] = path.vecs[i] + vec;
		}
		return tF_PathManager;
	}

	public void GetAllPath(int type)
	{
		TF_PathManager tF_PathManager = smallFishPath[type];
		int num = tF_PathManager.vecs.Length;
		string text = string.Empty;
		for (int i = 0; i < num; i++)
		{
			string text2 = text;
			text = text2 + tF_PathManager.vecs[i][0].ToString() + "," + tF_PathManager.vecs[i][1].ToString() + "," + tF_PathManager.vecs[i][2].ToString() + " ";
		}
