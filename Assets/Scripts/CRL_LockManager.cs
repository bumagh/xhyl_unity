...
public class CRL_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {CRL_LogHelper.Key(btnKey)} value {CRL_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {CRL_LogHelper.Key(btnKey)} value {CRL_LogHelper.Key(GetValue(btnKey))}");
			}
