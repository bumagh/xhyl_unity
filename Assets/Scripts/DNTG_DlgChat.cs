...
public class DNTG_DlgChat : MonoBehaviour
{
	[SerializeField]
	...
	private DNTG_ErrorTipAnim sptErrorTipAnim;
	...
	[HideInInspector]
	public DNTG_ChatCtrl sptChatCtrl;
	...
	private void GetAndInitCompenent()
	{
		...
		sptErrorTipAnim = imgError.GetComponent<DNTG_ErrorTipAnim>();
	...
	private void ClickBtnCommonUse(int index)
	{
		...
		DNTG_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, ifChart.text);
	...
	private void ClickBtnSend()
	{
		if (ifChart.text != string.Empty)
		{
			...
			DNTG_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, chatMessage);
			base.gameObject.SetActive(value: false);
		}
		else
		{
	...
	public void ShowChat()
	{
		...
		if (gameInfo.IsGameShuUp || gameInfo.IsUserShutUp)
		{
			...
			btnSend.GetComponent<DNTG_ButtonImgAni>().enabled = false;
			btnSend.image.sprite = spiBtnSend[language * 2 + 1];
			SetBtnCommonUse(b: false);
		}
		else
		{
			btnSend.GetComponent<DNTG_ButtonImgAni>().enabled = true;
