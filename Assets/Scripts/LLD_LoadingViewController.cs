...
public class LLD_LoadingViewController : LLD_MB_Singleton<LLD_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (LLD_MB_Singleton<LLD_LoadingViewController>._instance == null)
		{
			LLD_MB_Singleton<LLD_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (LLD_MB_Singleton<LLD_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!LLD_MB_Singleton<LLD_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
