...
public class USW_GamblePanel : MonoBehaviour
{
	...
	public static USW_GamblePanel ins;
	...
	private void Awake()
	{
		...
		douletext = base.transform.Find<Text>("double/double");
		scoretext = base.transform.Find<Text>("score");
		balance = base.transform.Find<Text>("balamce/balamce");
	...
	{
		USW_MB_Singleton<USW_DiceGameController2>.GetInstance().Send_MultipleInfo(USW_BetDiceType.Half);
		SetButton(canclick: false);
	}

	private void ClickBlack()
	{
		USW_MB_Singleton<USW_DiceGameController2>.GetInstance().Send_MultipleInfo(USW_BetDiceType.Risk);
		SetButton(canclick: false);
	}
	...
	private Sprite Getsprite()
	{
		try
		{
			uint num = Getnum(USW_MB_Singleton<USW_DiceGameController2>.GetInstance().Gamelewinid);
	...
	{
		balancescore = USW_GVars.winScorce;
		doublescore = USW_GVars.winScorce * 2;
	...
	{
		USW_MB_Singleton<USW_DiceGameController2>.GetInstance().OnBtnReady_Click();
		USW_MB_Singleton<USW_DiceGameController2>.GetInstance().Hide();
		int credit = (!isGamble) ? (USW_GVars.winScorce + USW_GVars.credit) : AllScroce;
		USW_MB_Singleton<USW_MajorGameController>.GetInstance().SetCredit(credit);
