...
public class LKB_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {LKB_LogHelper.Key(btnKey)} value {LKB_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {LKB_LogHelper.Key(btnKey)} value {LKB_LogHelper.Key(GetValue(btnKey))}");
			}
