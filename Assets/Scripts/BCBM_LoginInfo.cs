...
public class BCBM_LoginInfo : MonoBehaviour
{
	private static BCBM_LoginInfo instance;

	private static GameObject loginGO;

	public BCBM_WWWstatic wwwinstance;

	public int mytest;

	public BCBM_LoginData mylogindata;

	public static BCBM_LoginInfo Instance()
	{
		if (instance == null)
		{
			loginGO = new GameObject("LoginInfo");
			instance = (loginGO.AddComponent(typeof(BCBM_LoginInfo)) as BCBM_LoginInfo);
			instance.wwwinstance = (loginGO.AddComponent(typeof(BCBM_WWWstatic)) as BCBM_WWWstatic);
	...
	{
		if (!BCBM_LoginData.IsLogin)
		{
			return;
		}
		BCBM_LoginData.OverTime += Time.deltaTime;
		if (BCBM_LoginData.OverTime >= 3f)
		{
			BCBM_NewTcpNet.GetInstance().SocketQuit();
			if (mylogindata.choosegame == 19)
			{
				BCBM_NewTcpNet.GetInstance2();
			}
			else
			{
				BCBM_NewTcpNet.GetInstance();
			}
		}
	}
		...
		while (true)
		{
			...
			if (www.error == null)
			{
				try
				{
					...
					{
						if (BCBM_LoginData.IsConnect)
						{
							BCBM_DisconnectPanel.GetInstance().Show();
							BCBM_DisconnectPanel.GetInstance().Modification(string.Empty, "长时间未操作，你已被移除房间");
	...
	private IEnumerator OnWebGet2()
	{
		...
		if (www.error == null)
		{
			...
			{
				JsonData jsonData = JsonMapper.ToObject(www.downloadHandler.text);
				if (jsonData["code"].ToString() == "200" && BCBM_NewTcpNet.instance != null)
				{
					BCBM_NewTcpNet.GetInstance().SocketQuit();
				}
			}
