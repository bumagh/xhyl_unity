...
public class DPR_GameRoot : DPR_MB_Singleton<DPR_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(DPR_MB_Singleton<DPR_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            DPR_MB_Singleton<DPR_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : DPR_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : DPR_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
