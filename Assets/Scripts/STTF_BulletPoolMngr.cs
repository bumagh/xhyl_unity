...
public class STTF_BulletPoolMngr : MonoBehaviour
{
	public static STTF_BulletPoolMngr G_BulletPoolMngr;
	...
	public static STTF_BulletPoolMngr GetSingleton()
	{
	...
	public Transform LanchBullet(int nPlayer, float fAgl, int nPower, float fLiziTime, bool isTellSvr)
	{
		...
		STTF_Bullet component = transform.GetComponent<STTF_Bullet>();
		for (int i = 0; i < STTF_GameInfo.getInstance().UserList.Count; i++)
		{
			if (STTF_GameInfo.getInstance().UserList[i].SeatIndex == nPlayer)
			{
				STTF_Lock currentGun = STTF_GameInfo.getInstance().GameScene.GetCurrentGun(STTF_GameInfo.getInstance().UserList[i].SeatIndex);
				if (currentGun.lockProcess != STTF_ELOCK.MoveToOtherFish && STTF_GameInfo.getInstance().UserList[i].LockFish != null && STTF_GameInfo.getInstance().UserList[i].Lock)
				{
		...
		if (nPower <= 50 && nPower >= 1)
		{
			if (fLiziTime > 0f)
			{
				...
				component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_NORMAL_LIZI);
				STTF_MusicMngr.GetSingleton().PlayGameSound(STTF_MusicMngr.GAME_SOUND.SOUND_LIZI_SHOOT);
			}
			else
			{
				component.mIsLizi = false;
				STTF_MusicMngr.GetSingleton().PlayGameSound(STTF_MusicMngr.GAME_SOUND.SOUND_NORMAL_SHOOT);
				switch (nPlayer)
				{
				case 1:
					component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_NORMAL1);
					break;
				case 2:
					component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_NORMAL2);
					break;
				case 3:
					component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_NORMAL3);
					break;
				case 4:
					component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_NORMAL4);
					break;
				}
			}
		}
		else if (nPower < 1000 && nPower >= 51)
		{
			...
			{
				component.mIsLizi = true;
				component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_SUPER_LIZI);
				STTF_MusicMngr.GetSingleton().PlayGameSound(STTF_MusicMngr.GAME_SOUND.SOUND_LIZI_SHOOT);
			}
			else
			{
				...
				{
				case 1:
					component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_SUPER1);
					break;
				case 2:
					component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_SUPER2);
					break;
				case 3:
					component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_SUPER3);
					break;
				case 4:
					component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_SUPER4);
					break;
				}
			}
		}
		...
		{
			component.mIsLizi = true;
			component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_SUPER_LIZI4);
			STTF_MusicMngr.GetSingleton().PlayGameSound(STTF_MusicMngr.GAME_SOUND.SOUND_LIZI_SHOOT);
		}
		else
		{
			...
			{
			case 1:
				component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_SUPER4_1);
				break;
			case 2:
				component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_SUPER4_2);
				break;
			case 3:
				component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_SUPER4_3);
				break;
			case 4:
				component.SetBulletType(STTF_BULLET_TYPE.BULLET_TYP_SUPER4_4);
				break;
			}
		}
		...
		if (isTellSvr)
		{
			STTF_NetMngr.GetSingleton().MyCreateSocket.SendFired(component.mBulletID, _nPlayerIndex, fAgl, nPower, component.mIsLizi);
		}
		return transform;
	}
	...
	private void Update()
	{
		...
		if (STTF_GameParameter.G_bTest && Input.GetMouseButtonDown(0))
		{
	...
	public void LockingBulletToNormal()
	{
		...
		{
			mCurrentPlayerBullet[i].GetComponent<STTF_Bullet>().IsLocking = false;
		}
	}
	...
	public void OtherLockingBulletToNormal(int player)
	{
		...
		{
			STTF_Bullet component = mOtherPlayerBullet[i].GetComponent<STTF_Bullet>();
