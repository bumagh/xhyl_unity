...
public class CRL_TileManager : MonoBehaviour
{
	...
	public IEnumerator StartChasing(int targetIndex, Action endAction)
	{
		...
		CRL_XWait xwait = new CRL_XWait(this).Begin();
		int count = 0;
		float timeCount = 0f;
	...
	public IEnumerator WaitSecondsAction(float seconds, Action<int> action, int param1, CRL_XWait xwait = null)
	{
		if (xwait == null)
		{
			yield return new WaitForSeconds(seconds);
