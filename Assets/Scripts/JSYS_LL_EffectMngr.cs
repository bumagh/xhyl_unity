...
public class JSYS_LL_EffectMngr : MonoBehaviour
{
	public static JSYS_LL_EffectMngr G_EffectMngr;
	...
	private JSYS_LL_BackWallAnim _BackWallAnim;

	private GameObject backwall;

	private JSYS_LL_Script_AnimalPtc2 _AnimalPtcCtrl2;

	private JSYS_LL_Script_AnimalPtc3 _AnimalPtcCtrl3;
	...
	public static JSYS_LL_EffectMngr GetSingleton()
	{
		return G_EffectMngr;
	}
	...
	private void Awake()
	{
		...
		_BackWallAnim = GameObject.Find("Wall").GetComponent<JSYS_LL_BackWallAnim>();
		...
		_AnimalPtcCtrl2 = _AnimalPtcObj2.GetComponent<JSYS_LL_Script_AnimalPtc2>();
		_AnimalPtcObj3 = GameObject.Find("AnimalPrizePtc3_6Agl");
		_AnimalPtcCtrl3 = _AnimalPtcObj3.GetComponent<JSYS_LL_Script_AnimalPtc3>();
	...
	public void ShowFireWorksRandom(bool isShow)
	{
		...
		if (isShow)
		{
			JSYS_LL_MusicMngr.GetSingleton().PlayEffectSound(JSYS_LL_MusicMngr.MUSIC_EFFECT_MUSIC.EFFECT_FIREWORKS);
			_backFireWorks.SetActive(value: true);
	...
	public void SetWallColorState(JSYS_LL_BackWallAnim.WALL_COLOR_TYPE typ)
	{
	...
	public void SetSongDengTVShow(bool isShow, int number, bool withAnim = false)
	{
		...
		if (isShow)
		{
			_AllSongDengTV.GetComponent<JSYS_LL_All_SongDeng>().ShowSongDeng(isShow, number, withAnim);
		}
	}
		...
		if (isShow && isDouble)
		{
			_AllShanDian.transform.Find("shandian").Find("X2").GetComponent<Renderer>()
				...
				else if (isShow && !isDouble)
				{
					...
					{
						_AllDaSiXi.GetComponent<JSYS_LL_All_DaSiXi>().SetColor(color);
					}
					...
					public void SetAllDaSanYuanShowOneAnimal(int nAnimalIndex)
					{
						...
						_AllDaSanYuan.GetComponent<JSYS_LL_All_DaSanYuan>().ShowOneAnimal(nAnimalIndex);
					}

					public void SetLuckyPrizeShow(int nLuckyNumber, int nLuckyTyp)
					{
						_LuckyPrizeEffect.SetActive(value: true);
						_LuckyPrizeEffect.GetComponent<JSYS_LL_LuckyNumber>().ShowAnimation(nLuckyNumber, nLuckyTyp);
					}

					public void SetLuckyPrizeHide()
					{
						_LuckyPrizeEffect.GetComponent<JSYS_LL_LuckyNumber>().Reset();
					...
					public void SetLuckyBonusShow(bool isShow, int bonusNum = 1000)
					{
						if (isShow)
						{
							...
							_LuckyBonus.GetComponent<JSYS_LL_LuckyBonus>().SetBonusNum(bonusNum);
							JSYS_LL_MusicMngr.GetSingleton().PlayEffectSound(JSYS_LL_MusicMngr.MUSIC_EFFECT_MUSIC.Effect_BONUS_COINS);
						}
						else
						{
