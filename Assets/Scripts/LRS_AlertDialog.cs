...
public class LRS_AlertDialog : LRS_MB_Singleton<LRS_AlertDialog>
{
	...
	private void Awake()
	{
		...
		InitFinGame();
		if (LRS_MB_Singleton<LRS_AlertDialog>._instance == null)
		{
			LRS_MB_Singleton<LRS_AlertDialog>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void ShowDialog(string content, bool showOkCancel = false, Action callback = null)
	{
		...
		LRS_MB_Singleton<LRS_GameRoot>.GetInstance().StartCoroutine(LRS_Utils.DelayCall(delay, delegate
		{
	...
	{
		isTouchForbidden = !LRS_MB_Singleton<LRS_GameManager>.GetInstance().GetTouchEnable();
		if (isTouchForbidden)
		{
			LRS_MB_Singleton<LRS_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "alert", includeEsc: false);
		}
		...
		if (LRS_MB_Singleton<LRS_OptionsController>.GetInstance() != null)
		{
	...
	public void Hide()
	{
		...
		if (isTouchForbidden)
		{
			LRS_MB_Singleton<LRS_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "alert done, reforbidden", includeEsc: false);
		}
	}
	...
	public void OnBtnCancel_Click()
	{
		if (!LRS_GVars.tryLockOnePoint)
		{
			if (LRS_SoundManager.Instance != null)
			{
