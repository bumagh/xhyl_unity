...
public class SHT_AlertDialog : SHT_MB_Singleton<SHT_AlertDialog>
{
	...
	private void Awake()
	{
		...
		InitFinGame();
		if (SHT_MB_Singleton<SHT_AlertDialog>._instance == null)
		{
			SHT_MB_Singleton<SHT_AlertDialog>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void ShowDialog(string content, bool showOkCancel = false, Action callback = null)
	{
		...
		SHT_MB_Singleton<SHT_GameRoot>.GetInstance().StartCoroutine(SHT_Utils.DelayCall(delay, delegate
		{
	...
	{
		isTouchForbidden = !SHT_MB_Singleton<SHT_GameManager>.GetInstance().GetTouchEnable();
		if (isTouchForbidden)
		{
			SHT_MB_Singleton<SHT_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "alert", includeEsc: false);
		}
		...
		if (SHT_MB_Singleton<SHT_OptionsController>.GetInstance() != null)
		{
			SHT_MB_Singleton<SHT_OptionsController>.GetInstance().HideAllPanel();
		}
	}
	...
	public void Hide()
	{
		...
		if (isTouchForbidden)
		{
			SHT_MB_Singleton<SHT_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "alert done, reforbidden", includeEsc: false);
		}
	}
	...
	public void OnBtnCancel_Click()
	{
		if (!SHT_GVars.tryLockOnePoint)
		{
			if (SHT_SoundManager.Instance != null)
			{
