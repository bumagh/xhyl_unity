...
public class STMF_Sockets : MonoBehaviour
{
	public bool isReconnect = true;

	private static STMF_Sockets _MyCreateSocket;
	...
	private STMF_DataEncrypt m_DataEncrypt;
	...
	protected static int s_idIndex;

	protected STMF_FlagObject _lastFlagObj;
	...
	public static STMF_Sockets GetSingleton()
	{
	...
	public void CreateSocketGetPoint(STMF_MessageControl MyMessageControl, STMF_DataEncrypt MyDataEncrypt)
	{
		CreateReceiveThread();
	...
	public void CreateReceiveThread()
	{
		...
		if (_lastFlagObj != null)
		{
			_lastFlagObj.isShutDown = true;
		}
		_lastFlagObj = new STMF_FlagObject();
		_lastFlagObj.id = s_idIndex++;
		_lastFlagObj.isShutDown = false;
		if (coRestrictReconnect != null)
		{
			StopCoroutine("coRestrictReconnect");
		}
		coRestrictReconnect = StartCoroutine(RestrictReconnect());
		m_nReceiveThread = new Thread(SocketConnect2);
		m_nReceiveThread.Start(_lastFlagObj);
	}
	...
	public void SocketConnect2(object flagObj)
	{
		UnityEngine.Debug.Log("====SocketConnect2====");
		Thread.Sleep(50);
		...
		if (m_nSocketStartFlag)
		{
			if (!sendCheckVersion)
			{
				SendCheckVersion(STMF_Constants.VERSION_CODE);
			...
			_myReceiveControl(flagObj as STMF_FlagObject);
		}
	}

	public void SocketConnect()
	{
		UnityEngine.Debug.Log("SocketConnect");
		Thread.Sleep(50);
		m_nSocketStartFlag = false;
		m_nSocketStartFlag = _socketStart();
		if (m_nSocketStartFlag)
	...
	private bool _socketStart()
	{
		...
		IPEndPoint remoteEP = new IPEndPoint(IPAddress.Parse(STMF_Constants.GetSvrIP()), 10011);
		try
		{
	...
	private void _myReceiveControl(STMF_FlagObject flagObj)
	{
		int num = 0;
		int num2 = id;
		...
		while (true)
		{
			...
			try
			{
				...
				if (flagObj.isShutDown)
				{
					UnityEngine.Debug.Log("Stop receive socket " + flagObj.id + " 's message!!!!!!!!!!!!");
					return;
				}
			}
			catch (Exception ex)
			...
			while (memoryStream.Length >= 4)
			{
				...
				int num4 = (int)(memoryStream.Length - num3 - 4);
				...
				_receiveNetMessage(array2, flagObj);
			}
		}
	...
	private void _receiveNetMessage(byte[] msgBytes, STMF_FlagObject flagObj)
	{
		if (m_DataEncrypt.GetKey() != "none")
		{
			try
            {
                ...
                Debug.LogError("准备解码，正确: " + Encoding.Default.GetString(msgBytes));
            }
			catch(Exception ex)
			{
				Debug.LogError("准备解码，错误: " + Encoding.Default.GetString(msgBytes));
			}
		}
		...
		else
		{
			try
			{
				hashtable = STMF_ReadFileTool.JsonToClass<Hashtable>(@string);
			}
			catch (Exception)
			{
				msgBytes = m_DataEncrypt.DecompressBytes(msgBytes);
				Console.WriteLine(Encoding.UTF8.GetString(msgBytes));
				hashtable = STMF_ReadFileTool.JsonToClass<Hashtable>(Encoding.UTF8.GetString(msgBytes));
	...
	public bool _sendMsg(string strMethod, object[] args)
	{
		...
		if (args == null)
		{
			try
			{
				UnityEngine.Debug.LogError("---" + strMethod + "---" + JsonMapper.ToJson(args));
			}
			catch (Exception)
			{
			}
		...
		else
		{
			...
			hashtable.Add("version", STMF_Constants.Version);
			hashtable.Add("time", m_DataEncrypt.GetUnixTime());
			string s = STMF_ReadFileTool.JsonByObject(hashtable);
	...
	{
		UnityEngine.Debug.Log("SendCheckVersion: " + versionCode);
	...
	public bool SendCheckLogin(string userName, string passWord, int uiid, string versionCode = "")
	{
		...
		Console.WriteLine("userService/checklogin");
		return _sendMsg(strMethod, args);
	}
	...
	public bool SendEnterRoom(int roomid)
	{
		...
		UnityEngine.Debug.LogError("enterRoom " + JsonMapper.ToJson(array));
	...
	public bool SendLeaveDesk(int deskid)
	{
		...
		Console.WriteLine("userService/leaveDesk");
		return _sendMsg(strMethod, args);
	}
	...
	{
		string strMethod = "fired," + gunid + "," + (double)fRot + "," + gunValue + "," + bX2Gun;
		...
		Console.WriteLine("userService/fired");
		return _sendMsg(strMethod, null);
	}

	public bool SendGunHitfish(int gunid, STMF_HitFish[] hitfish1, bool bHaveKnife, STMF_HitFish[] hitfish2)
	{
		string empty = string.Empty;
