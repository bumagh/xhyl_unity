...
public class ESP_Gcore : Cinstance<ESP_Gcore>
{
	...
	public void Init()
	{
		try
		{
			...
			{
				string text = string.Format("EleWin{0}_01_png", i.ToString("00"));
				Sprite sprite = ESP_MB_Singleton<ESP_GameManager>.GetInstance().Getanisprite(text);
			...
			{
				List<int> item = ESP_LinePosIdx.Getwinlinepos(j, 5);
				AllLinePos.Add(item);
			}
	...
	{
		IsRoll = false;
		ESP_MB_Singleton<ESP_MajorGameController>.GetInstance().AllStop();
	}
	...
	public void QuickStop()
	{
		if (IsRoll)
		{
			ESP_MB_Singleton<ESP_MajorGameController>.GetInstance().StopRoll();
			IsRoll = false;
		}
	}
	...
	{
		IsRoll = true;
		ESP_MB_Singleton<ESP_MajorGameController>.GetInstance().StartRollSym();
	}
