...
public class LKB_NetManager : LKB_MB_Singleton<LKB_NetManager>
{
	public delegate void NetMessageHandler(object[] args);

	private LKB_ClientSocket _pClient;

	private LKB_DataEncrypt _pCrypto;
	...
	private Queue<LKB_NetMessage> _netMsgQueue = new Queue<LKB_NetMessage>();

	private Dictionary<string, NetMessageHandler> _netMsgHandlerDic = new Dictionary<string, NetMessageHandler>();

	private void Awake()
	{
		if (LKB_MB_Singleton<LKB_NetManager>._instance == null)
		{
			LKB_MB_Singleton<LKB_NetManager>.SetInstance(this);
		}
	...
	private void _initClientSocket()
	{
		...
		_pClient = new LKB_ClientSocket(useTcpClient: true);
		_pCrypto = new LKB_DataEncrypt();
	...
	private void _handleException(Exception ex)
	{
		...
		isReady = false;
		isLogined = false;
		Queue<LKB_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
			_netMsgQueue.Enqueue(new LKB_NetMessage("netDown", new object[1]
			{
				ex
			}));
		}
	...
	private void _handleConnected()
	{
		try
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new LKB_NetMessage("netConnected", new object[0]));
			}
		}
		catch (Exception ex)
		{
	...
	{
		UnityEngine.Debug.Log("ZH2_GVars.versionCode: " + LKB_GVars.versionCode);
		Send("userService/checkVersion", new object[1]
		{
			LKB_GVars.versionCode
		});
	}
	...
	protected void _handleData(byte[] msgBuffer, int packetId, int endIndex)
	{
		...
		lock (netMsgQueue)
		{
			LKB_NetMessage lKB_NetMessage = new LKB_NetMessage(text, args);
			lKB_NetMessage.jsonString = @string;
			lKB_NetMessage.packetId = packetId;
			lKB_NetMessage.endIndex = endIndex;
			_netMsgQueue.Enqueue(lKB_NetMessage);
		}
	}

	private IEnumerator _checkTimeout(int id, int timeout, LKB_ClientSocket cs)
	{
		UnityEngine.Debug.Log(LKB_LogHelper.Aqua("_checkTimeout start id: {0}", id));
		...
		if (cs != null && cs.doneList.Count > id && !cs.doneList[id])
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new LKB_NetMessage("timeout", new object[1]
				{
					id
				}));
				isTimeout = true;
		...
		if (isTimeout)
		{
			UnityEngine.Debug.Log(LKB_LogHelper.Aqua("_checkTimeout start id: {0} timeout: {1}", id, isTimeout));
		}
	}
	...
	private IEnumerator _checkHeartTimeout(int timeout)
	{
		...
		if (isReady && !isHeartPong)
		{
			...
			lock (netMsgQueue)
			{
				...
				isReady = false;
				isLogined = false;
				_netMsgQueue.Enqueue(new LKB_NetMessage("heart timeout", new object[0]));
				isTimeout = true;
			}
		}
		if (isTimeout)
		{
			UnityEngine.Debug.Log(LKB_LogHelper.Aqua("_checkHeartTimeout is timeout"));
		}
	}

	public void _checkTimeoutStart(int id, int timeout, LKB_ClientSocket cs)
	{
	...
	public void Send(string method, object[] args)
	{
		...
		{
			UnityEngine.Debug.Log(LKB_LogHelper.Magenta("drop send>>method: {0}", method));
			return;
		}
		Hashtable hashtable = new Hashtable();
		hashtable.Add("method", method);
		hashtable.Add("args", args);
		hashtable.Add("version", LKB_GVars.Version);
		if (isReady)
		{
	...
	private bool GetNetMessage(out LKB_NetMessage netMsg)
	{
		Queue<LKB_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
	...
	private void DispatchNetMessage(LKB_NetMessage netMsg)
	{
		if (dropDispatch)
		{
			UnityEngine.Debug.Log(LKB_LogHelper.Magenta("drop send>>method: {0}", netMsg.method));
		}
