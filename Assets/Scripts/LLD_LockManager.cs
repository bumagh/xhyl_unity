...
public class LLD_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {LLD_LogHelper.Key(btnKey)} value {LLD_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {LLD_LogHelper.Key(btnKey)} value {LLD_LogHelper.Key(GetValue(btnKey))}");
			}
