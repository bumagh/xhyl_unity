...
public class WHN_SymbolC : MonoBehaviour
{
	...
	private string aniname = "Ele{0:0}_";

	private string aniname2 = "icon{0:0}_";
	...
	private void Awake()
	{
		...
		symlen = Cinstance<WHN_Gcore>.Instance.Symcont;
	...
	public void Init(int idx, int symid, WHN_Line line = null)
	{
		symidx = idx;
		lineroll = line;
	...
	{
		iconimg.sprite = Cinstance<WHN_Gcore>.Instance.SymSprlist[idx];
	}
	...
	public void StopRoll()
	{
		...
		if (isroll)
		{
			...
			if (Inlineidx > 2)
			{
				...
				int idx = Cinstance<WHN_Gcore>.Instance.Result[index];
				SetSymbolimg(idx, 0L);
			}
		}
		WHN_SoundManager.Instance.PlayStopAndio();
		speed = normalspeed;
	}
	...
	private void Setrollnum(List<int> randomlist, int rolllen)
	{
		...
		if (Inlineidx > 2)
		{
			...
			int value = Cinstance<WHN_Gcore>.Instance.Result[index2];
	...
	public void Playani(int winline = -1)
	{
		...
		try
		{
			...
			Seq_ani = iconimg.WHN_PlayAni(string.Format(aniname, (SymbolId != 9) ? (SymbolId + 1).ToString("00") : string.Empty) + "{0:0}_png", string.Format(aniname, (SymbolId != 9) ? (SymbolId + 1).ToString("00") : string.Empty) + "{0:0}_png", 2, frames);
			Seq_ani.Restart();
		}
		catch (Exception arg)
		{
