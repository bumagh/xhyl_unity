...
public class JSYS_LL_AnimalScript : JSYS_LL_RoleAnimationScript
{
	...
	private void Start()
	{
		...
		_centerPositon = new Vector3(0f, JSYS_LL_Parameter.G_fCenterAnimalHeight, 0f);
		_optimize();
	...
	public void SetAnimalActionState(Animal_Action_State state)
	{
		...
		{
			JSYS_LL_EffectMngr.GetSingleton().ShowAnimalParticle(isShow: true);
			int typ = 0;
			if (base.gameObject.CompareTag("lion"))
			{
				typ = 3;
			...
			JSYS_LL_MusicMngr.GetSingleton().PlayAnimalWin((AnimalType)typ);
	...
	private void _logicCtrl()
	{
		...
		if (_Animal_Action == Animal_Action_State.Animal_Bounce_And_GoCenter)
		{
			if (_lastAnimal_Action != Animal_Action_State.Animal_Bounce_And_GoCenter)
			{
				...
				iTween.ScaleTo(base.gameObject, iTween.Hash("scale", JSYS_LL_Parameter.G_AnimalMaxScale2, "delay", 0.3f, "time", 0.1f, "easetype", iTween.EaseType.easeInOutSine));
				iTween.ScaleTo(base.gameObject, iTween.Hash("scale", JSYS_LL_Parameter.G_AnimalMinScale, "delay", 0.4f, "time", 0.1f, "easetype", iTween.EaseType.easeInOutSine));
				iTween.ScaleTo(base.gameObject, iTween.Hash("scale", JSYS_LL_Parameter.G_AnimalMaxScale, "delay", 0.5f, "time", 0.1f, "easetype", iTween.EaseType.easeOutBack));
				...
				JSYS_LL_MusicMngr.GetSingleton().PlayAnimalOut();
			}
		}
	...
	public void SetColorOfAnimalRing(AnimalColor color)
	{
		...
		{
			JSYS_LL_ErrorManager.GetSingleton().AddError("---Error Client3D---:SetColorOfAnimalRing wrong color");
		}
	}
	...
	public void Reset()
	{
		...
		base.transform.localScale = JSYS_LL_Parameter.G_AnimalMinScale;
		SetAnimation(AnimationType.AT_WAIT);
		ShowGoldShine(isShow: false);
		ShowGold(isShow: false);
	}
