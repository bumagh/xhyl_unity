...
public class CSF_GameRoot : CSF_MB_Singleton<CSF_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(CSF_MB_Singleton<CSF_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            CSF_MB_Singleton<CSF_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : CSF_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : CSF_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
