...
public class STTF_DataEncrypt : MonoBehaviour
{
	private string key = "none";

	private string IVKey = "Qzh0Qxebwie8Kvse";

	private static STTF_DataEncrypt _MyDataEncrypt;
	...
	private string[] PublicKeyPool = new string[30];

	private string[] PrivateKeyPool = new string[30];

	public STTF_DataEncrypt()
	{
	...
	public static STTF_DataEncrypt GetSingleton()
	{
		return _MyDataEncrypt;
	}

	private void Awake()
	{
		if (_MyDataEncrypt == null)
		{
			UnityEngine.Debug.Log("_MyDataEncrypt");
			_MyDataEncrypt = this;
		}
	}
	...
	public byte[] Encrypt(byte[] toEncrypt)
	{
		...
		rijndaelManaged.IV = Encoding.UTF8.GetBytes(IVKey);
	...
	public void setServerTime(long setTime)
	{
		...
		serverTime = setTime;
		getServerTime = (long)Math.Round((now - d).TotalMilliseconds, MidpointRounding.AwayFromZero);
	}
	...
	public long GetUnixTime()
	{
		DateTime d = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1, 0, 0, 0, 0));
		DateTime now = DateTime.Now;
		long num = (long)Math.Round((now - d).TotalMilliseconds, MidpointRounding.AwayFromZero);
		return serverTime + (num - getServerTime);
	}
	...
	public string[] NetConnectGetRsaKey()
	{
		...
		array = RSAKey();
		m_privateKey = array[0];
		m_publicKey = array[1];
		XmlDocument xmlDocument = new XmlDocument();
		xmlDocument.LoadXml(m_publicKey);
		XmlNodeList xmlNodeList = xmlDocument.SelectNodes("/RSAKeyValue");
		string innerText = xmlNodeList[0].SelectSingleNode("//Modulus").InnerText;
		string innerText2 = xmlNodeList[0].SelectSingleNode("//Exponent").InnerText;
		return new string[2]
		{
			STTF_SafeTools.Adjust_String_1(innerText),
			innerText2
		};
	}
	...
	public byte[] DecompressBytes(byte[] SourceByte)
	{
		try
		{
			...
			Stream stream = DecompressStream(sourceStream);
	...
	public Stream DecompressStream(Stream SourceStream)
	{
		try
		{
			...
			ZOutputStream output = new ZOutputStream(memoryStream);
