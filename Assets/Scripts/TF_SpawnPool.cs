...
public sealed class TF_SpawnPool : MonoBehaviour, IList<Transform>, ICollection<Transform>, IEnumerable<Transform>, IEnumerable
{
	...
	public List<TF_PrefabPool> _perPrefabPoolOptions = new List<TF_PrefabPool>();
	...
	public TF_PrefabsDict prefabs = new TF_PrefabsDict();

	public Dictionary<object, bool> _editorListItemStates = new Dictionary<object, bool>();

	private List<TF_PrefabPool> _prefabPools = new List<TF_PrefabPool>();
	...
	public Dictionary<string, TF_PrefabPool> prefabPools
	{
		get
		{
			Dictionary<string, TF_PrefabPool> dictionary = new Dictionary<string, TF_PrefabPool>();
			foreach (TF_PrefabPool prefabPool in _prefabPools)
			{
	...
	private void Awake()
	{
		...
		foreach (TF_PrefabPool perPrefabPoolOption in _perPrefabPoolOptions)
		{
	...
	private void OnDestroy()
	{
		...
		TF_PoolManager.Pools.Remove(this);
		StopAllCoroutines();
		_spawned.Clear();
		foreach (TF_PrefabPool prefabPool in _prefabPools)
		{
	...
	public void CreatePrefabPool(TF_PrefabPool prefabPool)
	{
	...
	public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot)
	{
		...
		TF_PrefabPool tF_PrefabPool = new TF_PrefabPool(prefab);
		CreatePrefabPool(tF_PrefabPool);
		transform = tF_PrefabPool.SpawnInstance(pos, rot);
		transform.parent = group;
		_spawned.Add(transform);
		return transform;
	}
	...
	private IEnumerator ListenForEmitDespawn(ParticleSystem emitter)
	{
		...
		{
			if (!TF_PoolManagerUtils.activeInHierarchy(emitter.gameObject))
			{
