...
public class CSF_MajorHints : MonoBehaviour
{
	[SerializeField]
	...
	private CSF_MajorHintSprites sprites;
	...
	public void PlayHint(string state)
	{
		...
		if (!(_lastHintState == state) || !(state != "WinTip"))
		{
			...
			if (_coHint != null)
			{
				CSF_MB_Singleton<CSF_GameManager>.GetInstance().StopCoroutine(_coHint);
			...
			{
			case "Normal_Dice":
				_coHint = CSF_MB_Singleton<CSF_GameManager>.GetInstance().StartCoroutine(PlayNormal(hasDice: true));
				break;
			case "Normal":
				_coHint = CSF_MB_Singleton<CSF_GameManager>.GetInstance().StartCoroutine(PlayNormal(hasDice: false));
				break;
			case "Win_Dice":
				_coHint = CSF_MB_Singleton<CSF_GameManager>.GetInstance().StartCoroutine(PlayWin(hasDice: true));
				break;
			case "Win":
				_coHint = CSF_MB_Singleton<CSF_GameManager>.GetInstance().StartCoroutine(PlayWin(hasDice: false));
				break;
			case "Rolling":
				_coHint = CSF_MB_Singleton<CSF_GameManager>.GetInstance().StartCoroutine(PlayRolling());
				break;
			case "GainScore":
				PlayGainScore();
				break;
