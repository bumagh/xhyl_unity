...
public class TF_SpawnPoolsDict : IDictionary<string, TF_SpawnPool>, ICollection<KeyValuePair<string, TF_SpawnPool>>, IEnumerable<KeyValuePair<string, TF_SpawnPool>>, IEnumerable
{
	private Dictionary<string, TF_SpawnPool> _pools = new Dictionary<string, TF_SpawnPool>();

	bool ICollection<KeyValuePair<string, TF_SpawnPool>>.IsReadOnly => true;

	public int Count => _pools.Count;

	public TF_SpawnPool this[string key]
	{
		get
		{
			try
	...
	public ICollection<TF_SpawnPool> Values
	{
		get
		{
			string message = "If you need this, please request it.";
	...
	void ICollection<KeyValuePair<string, TF_SpawnPool>>.CopyTo(KeyValuePair<string, TF_SpawnPool>[] array, int arrayIndex)
	{
	...
	public TF_SpawnPool Create(string poolName)
	{
		GameObject gameObject = new GameObject(poolName + "Pool");
		return gameObject.AddComponent<TF_SpawnPool>();
	}

	public TF_SpawnPool Create(string poolName, GameObject owner)
	{
		...
		try
		{
			...
			return owner.AddComponent<TF_SpawnPool>();
		}
		finally
		{
	...
	{
		if (!_pools.TryGetValue(poolName, out TF_SpawnPool value))
		{
	...
	{
		foreach (KeyValuePair<string, TF_SpawnPool> pool in _pools)
		{
			UnityEngine.Object.Destroy(pool.Value);
		}
	}

	internal void Add(TF_SpawnPool spawnPool)
	{
	...
	public void Add(string key, TF_SpawnPool value)
	{
	...
	internal bool Remove(TF_SpawnPool spawnPool)
	{
		if (!ContainsKey(spawnPool.poolName))
		{
	...
	public bool TryGetValue(string poolName, out TF_SpawnPool spawnPool)
	{
		return _pools.TryGetValue(poolName, out spawnPool);
	}

	public bool Contains(KeyValuePair<string, TF_SpawnPool> item)
	{
		string message = "Use PoolManager.Pools.Contains(string poolName) instead.";
		throw new NotImplementedException(message);
	}

	public void Add(KeyValuePair<string, TF_SpawnPool> item)
	{
		string message = "SpawnPools add themselves to PoolManager.Pools when created, so there is no need to Add() them explicitly. Create pools using PoolManager.Pools.Create() or add a SpawnPool component to a GameObject.";
		throw new NotImplementedException(message);
	}
	...
	private void CopyTo(KeyValuePair<string, TF_SpawnPool>[] array, int arrayIndex)
	{
		string message = "PoolManager.Pools cannot be copied";
		throw new NotImplementedException(message);
	}

	public bool Remove(KeyValuePair<string, TF_SpawnPool> item)
	{
		string message = "SpawnPools can only be destroyed, not removed and kept alive outside of PoolManager. There are only 2 legal ways to destroy a SpawnPool: Destroy the GameObject directly, if you have a reference, or use PoolManager.Destroy(string poolName).";
		throw new NotImplementedException(message);
	}

	public IEnumerator<KeyValuePair<string, TF_SpawnPool>> GetEnumerator()
	{
		return _pools.GetEnumerator();
	}
}
