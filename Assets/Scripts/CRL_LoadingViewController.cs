...
public class CRL_LoadingViewController : CRL_MB_Singleton<CRL_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (CRL_MB_Singleton<CRL_LoadingViewController>._instance == null)
		{
			CRL_MB_Singleton<CRL_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (CRL_MB_Singleton<CRL_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public void Show()
	{
		...
		CRL_MySqlConnection.curView = "LoadingView";
	}
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!CRL_MB_Singleton<CRL_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
	...
	{
		percent += CRL_MySqlConnection.RandomTime;
		if (percent >= 1f)
		{
			percent = 1f;
		}
