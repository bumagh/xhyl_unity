...
public class STMF_FishNetpoolMngr : MonoBehaviour
{
	public static STMF_FishNetpoolMngr G_FishNetpoolMngr;

	public Transform mNet;

	private ArrayList _allNetArr = new ArrayList();

	public static STMF_FishNetpoolMngr GetSingleton()
	{
	...
	public void DestroyFishNet(GameObject fishNet)
	{
		...
		PoolManager.Pools["UIEffectPool"].Despawn(fishNet.transform);
	}

	public void RemoveAllFishNet()
	{
		UnityEngine.Debug.Log("RemoveAllFishNet");
		PoolManager.Pools["UIEffectPool"].DespawnAll();
	}
	...
	public void CreateFishNet(Vector3 pos, bool isLizi, int nPower, int nPlayerIndex)
	{
		...
		Transform transform = PoolManager.Pools["UIEffectPool"].Spawn(mNet);
		...
		component.SetPower(nPower, ZH2_GVars.GameType.fish_desk);
