...
public class TF_DlgChat : MonoBehaviour
{
	[SerializeField]
	...
	[HideInInspector]
	public TF_ChatCtrl sptChatCtrl;
	...
	private void ClickBtnCommonUse(int index)
	{
		...
		TF_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, ifChart.text);
	...
	private void ClickBtnSend()
	{
		if (ifChart.text != string.Empty)
		{
			...
			TF_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, chatMessage);
			base.gameObject.SetActive(value: false);
		}
		else
		{
	...
	public void ShowChat()
	{
		...
		if (gameInfo.IsGameShuUp || gameInfo.IsUserShutUp)
		{
			...
			btnSend.GetComponent<TF_ButtonImgAni>().enabled = false;
			btnSend.image.sprite = spiBtnSend[language * 2 + 1];
			SetBtnCommonUse(b: false);
		}
		else
		{
			btnSend.GetComponent<TF_ButtonImgAni>().enabled = true;
