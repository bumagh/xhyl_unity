...
public class JSYS_LL_All_DaSanYuan : MonoBehaviour
{
	...
	private void Start()
	{
		...
		for (int i = 0; i < mAnimalArr.Length; i++)
		{
			...
			mAnimalArr[i].GetComponent<JSYS_LL_AnimalScript>().SetAnimation(JSYS_LL_RoleAnimationScript.AnimationType.AT_STOP);
		}
		_isFirstRun = true;
	}
	...
	public void ShowOneAnimal(int index)
	{
		...
		mAnimalArr[index].GetComponent<JSYS_LL_AnimalScript>().SetAnimalActionState(Animal_Action_State.Animal_Play_Win);
	}
