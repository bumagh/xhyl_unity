...
public class SHT_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {SHT_LogHelper.Key(btnKey)} value {SHT_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {SHT_LogHelper.Key(btnKey)} value {SHT_LogHelper.Key(GetValue(btnKey))}");
			}
