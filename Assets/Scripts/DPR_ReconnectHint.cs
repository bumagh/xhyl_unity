...
public class DPR_ReconnectHint : DPR_MB_Singleton<DPR_ReconnectHint>
{
	...
	private void Awake()
	{
		if (DPR_MB_Singleton<DPR_ReconnectHint>._instance == null)
		{
			DPR_MB_Singleton<DPR_ReconnectHint>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void Show()
	{
		if (!_goContainer.activeSelf)
		{
			...
			UnityEngine.Debug.Log(DPR_LogHelper.Magenta("ReconnectHint Show"));
	...
	public void Hide()
	{
		UnityEngine.Debug.Log(DPR_LogHelper.Magenta("ReconnectHint Hide"));
		DPR_LockManager.UnLock("Esc");
	...
	private IEnumerator _textInfoAni()
	{
		...
		while (true)
		{
			string prefix = (DPR_MySqlConnection.language == "zh") ? "网络断开，正在重连 " : "Reconnecting to the network ";
