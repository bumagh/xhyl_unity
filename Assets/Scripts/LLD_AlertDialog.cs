...
public class LLD_AlertDialog : LLD_MB_Singleton<LLD_AlertDialog>
{
	...
	private void Awake()
	{
		...
		InitFinGame();
		if (LLD_MB_Singleton<LLD_AlertDialog>._instance == null)
		{
			LLD_MB_Singleton<LLD_AlertDialog>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void ShowDialog(string content, bool showOkCancel = false, Action callback = null)
	{
		...
		LLD_MB_Singleton<LLD_GameRoot>.GetInstance().StartCoroutine(LLD_Utils.DelayCall(delay, delegate
		{
	...
	{
		isTouchForbidden = !LLD_MB_Singleton<LLD_GameManager>.GetInstance().GetTouchEnable();
		if (isTouchForbidden)
		{
			LLD_MB_Singleton<LLD_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "alert", includeEsc: false);
		}
		...
		if (LLD_MB_Singleton<LLD_OptionsController>.GetInstance() != null)
		{
	...
	public void Hide()
	{
		...
		if (isTouchForbidden)
		{
			LLD_MB_Singleton<LLD_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "alert done, reforbidden", includeEsc: false);
		}
	}
	...
	public void OnBtnCancel_Click()
	{
		if (!LLD_GVars.tryLockOnePoint)
		{
			if (LLD_SoundManager.Instance != null)
			{
