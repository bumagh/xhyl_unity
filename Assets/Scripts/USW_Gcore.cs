...
public class USW_Gcore : Cinstance<USW_Gcore>
{
	...
	public void Init()
	{
		try
		{
			...
			{
				string text = string.Format("ELE{0}_01", i.ToString("00"));
			...
			{
				List<int> item = USW_LinePosIdx.Getwinlinepos(j, 5);
				AllLinePos.Add(item);
			}
	...
	{
		IsRoll = false;
		USW_MB_Singleton<USW_MajorGameController>.GetInstance().AllStop();
	}
	...
	public void QuickStop()
	{
		if (IsRoll)
		{
			USW_MB_Singleton<USW_MajorGameController>.GetInstance().StopRoll();
			IsRoll = false;
		}
	}
	...
	{
		IsRoll = true;
		USW_MB_Singleton<USW_MajorGameController>.GetInstance().StartRollSym();
	}
