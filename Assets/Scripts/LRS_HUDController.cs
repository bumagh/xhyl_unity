...
public class LRS_HUDController : LRS_MB_Singleton<LRS_HUDController>
{
	...
	private void Awake()
	{
		...
		if (LRS_MB_Singleton<LRS_HUDController>._instance == null)
		{
			LRS_MB_Singleton<LRS_HUDController>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void PreInit()
	{
		...
		{
			ruleController = base.transform.Find("Canvas/Mask/Rule").GetComponent<LRS_RulePicController>();
		}
	}

	public void Init()
	{
		LRS_MB_Singleton<LRS_OptionsController>.GetInstance().onItemRules = delegate
		{
			ruleController.Show();
		};
		ruleController.Hide();
	...
	{
		ping = new Ping(LRS_GVars.IPAddress);
		if (waitPing != null)
		{
	...
	public void OnBtnOptions_Click()
	{
		...
		{
			if (LRS_MB_Singleton<LRS_OptionsController>.GetInstance().isShow)
			{
