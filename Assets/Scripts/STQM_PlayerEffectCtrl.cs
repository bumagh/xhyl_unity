...
public class STQM_PlayerEffectCtrl : MonoBehaviour
{
	...
	public void ShowCoin(STQM_FISH_TYPE fishType, Vector3 pos)
	{
		if (fishType < STQM_FISH_TYPE.Fish_Turtle || (fishType < STQM_FISH_TYPE.Fish_Same_Turtle && fishType >= STQM_FISH_TYPE.Fish_Same_Shrimp))
		{
			_showSilver(STQM_EffectMngr.GetSingleton().GetFlyCoinNum(fishType), pos);
			STQM_MusicMngr.GetSingleton().PlayGameSound(STQM_MusicMngr.GAME_SOUND.SOUND_SILVER_COIN);
		}
		else
		{
			_showGold(STQM_EffectMngr.GetSingleton().GetFlyCoinNum(fishType), pos);
			STQM_MusicMngr.GetSingleton().PlayGameSound(STQM_MusicMngr.GAME_SOUND.SOUND_GOLD_COIN);
		}
	}

	public void ShowLiziCard(Vector3 pos)
	{
		Transform transform = STQM_EffectMngr.GetSingleton().CreateEffectObj(_Lizi.transform);
		if (pos.x < -6f)
		{
		...
		transform.GetComponent<STQM_FlyObj>().AnimatedFlyTo(_playerPos);
		_fLiziTime += 20f;
		STQM_GameInfo.getInstance().GameScene.PlayEnergyGun(mPlayerIndex, isPlay: true);
	}
	...
	public void ShowFishScore(Vector3 pos, int nScore)
	{
		if (nScore > 0)
		{
			Transform transform = STQM_EffectMngr.GetSingleton().CreateEffectObj(_Score.transform);
			...
			transform.GetComponent<STQM_FlyObj>().AnimatedShow(_playerPos);
		}
	}
	...
	private void _showGold(int nCoinNum, Vector3 pos)
	{
		...
		{
			Transform transform = STQM_EffectMngr.GetSingleton().CreateEffectObj(objCoinGold.transform);
			float d = -1f;
			...
			transform.GetComponent<STQM_FlyObj>().FlyTo(_playerPos);
		}
	}
	...
	private void _showSilver(int nCoinNum, Vector3 pos)
	{
		for (int i = 0; i < nCoinNum; i++)
		{
			Transform transform = STQM_EffectMngr.GetSingleton().CreateEffectObj(objCoinSilver.transform);
	...
	private void _updateBigPrizePlate()
	{
		...
		if (_prizePlateQue.Count >= 1 && _showTimePlate >= 1f)
		{
			...
			{
				_curPrizePlateObj.GetComponent<STQM_PrizePlate>().DestroyObj();
			}
			int nNum = _prizePlateQue.Dequeue();
			_curPrizePlateObj = STQM_EffectMngr.GetSingleton().CreateEffectObj(_prizePlate.transform).gameObject;
			_curPrizePlateObj.GetComponent<STQM_PrizePlate>().ShowScore(nNum, _playerPos, mPlayerIndex);
			_showTimePlate = 0f;
			STQM_MusicMngr.GetSingleton().PlayGameSound(STQM_MusicMngr.GAME_SOUND.SOUND_SCORE_PANEL);
		}
	}
	...
	private void _updateLiziBarBette()
	{
		if (_fLiziTime > 0f)
		{
			...
			if (_fLiziTime <= 0f)
			{
				STQM_GameInfo.getInstance().GameScene.PlayEnergyGun(mPlayerIndex, isPlay: false);
			}
		}
		else
		{
