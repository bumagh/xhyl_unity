...
[AddComponentMenu("Path-o-logical/PoolManager/STQM_SpawnPool")]
public sealed class STQM_SpawnPool : MonoBehaviour, IList<Transform>, ICollection<Transform>, IEnumerable<Transform>, IEnumerable
{
	...
	public List<STQM_PrefabPool> _perPrefabPoolOptions = new List<STQM_PrefabPool>();
	...
	public STQM_PrefabsDict prefabs = new STQM_PrefabsDict();

	public Dictionary<object, bool> _editorListItemStates = new Dictionary<object, bool>();

	private List<STQM_PrefabPool> _prefabPools = new List<STQM_PrefabPool>();
	...
	public Dictionary<string, STQM_PrefabPool> prefabPools
	{
		get
		{
			Dictionary<string, STQM_PrefabPool> dictionary = new Dictionary<string, STQM_PrefabPool>();
			foreach (STQM_PrefabPool prefabPool in _prefabPools)
			{
	...
	private void Awake()
	{
		...
		MonoBehaviour.print(group.name);
		...
		foreach (STQM_PrefabPool perPrefabPoolOption in _perPrefabPoolOptions)
		{
		...
		STQM_PoolManager.Pools.Add(this);
	}
	...
	private void OnDestroy()
	{
		...
		STQM_PoolManager.Pools.Remove(this);
		StopAllCoroutines();
		_spawned.Clear();
		foreach (STQM_PrefabPool prefabPool in _prefabPools)
		{
	...
	public void CreatePrefabPool(STQM_PrefabPool prefabPool)
	{
	...
	public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot)
	{
		...
		STQM_PrefabPool sTQM_PrefabPool = new STQM_PrefabPool(prefab);
		CreatePrefabPool(sTQM_PrefabPool);
		transform = sTQM_PrefabPool.SpawnInstance(pos, rot);
		transform.parent = group;
		_spawned.Add(transform);
		return transform;
	}
