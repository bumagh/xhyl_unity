...
public class USW_AlertDialog : USW_MB_Singleton<USW_AlertDialog>
{
	...
	private void Awake()
	{
		...
		InitFinGame();
		if (USW_MB_Singleton<USW_AlertDialog>._instance == null)
		{
			USW_MB_Singleton<USW_AlertDialog>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void ShowDialog(string content, bool showOkCancel = false, Action callback = null)
	{
		...
		USW_MB_Singleton<USW_GameRoot>.GetInstance().StartCoroutine(USW_Utils.DelayCall(delay, delegate
		{
	...
	{
		isTouchForbidden = !USW_MB_Singleton<USW_GameManager>.GetInstance().GetTouchEnable();
		if (isTouchForbidden)
		{
			USW_MB_Singleton<USW_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "alert", includeEsc: false);
		}
		...
		if (USW_MB_Singleton<USW_OptionsController>.GetInstance() != null)
		{
	...
	public void Hide()
	{
		...
		if (isTouchForbidden)
		{
			USW_MB_Singleton<USW_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "alert done, reforbidden", includeEsc: false);
		}
	}
	...
	public void OnBtnCancel_Click()
	{
		if (!USW_GVars.tryLockOnePoint)
		{
			if (USW_SoundManager.Instance != null)
			{
