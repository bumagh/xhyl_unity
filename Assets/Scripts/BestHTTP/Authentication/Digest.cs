			...
			try
			{
				...
				case AuthenticationTypes.Digest:
				{
					...
					else if (text3.Contains("auth-int"))
					{
						...
						string text4 = $"{request.MethodType.ToString().ToUpper()}:{request.CurrentUri.GetRequestPathAndQueryURL()}:{array.CalculateMD5Hash()}".CalculateMD5Hash();
					...
					else
					{
						...
						text3 = "auth";
						string text5 = (request.MethodType.ToString().ToUpper() + ":" + request.CurrentUri.GetRequestPathAndQueryURL()).CalculateMD5Hash();
						empty2 = $"{empty}:{Nonce}:{text2}:{text}:{text3}:{text5}".CalculateMD5Hash();
					}
					string text6 = $"Digest username=\"{credentials.UserName}\", realm=\"{Realm}\", nonce=\"{Nonce}\", uri=\"{request.Uri.GetRequestPathAndQueryURL()}\", cnonce=\"{text}\", response=\"{empty2}\"";
					if (text3 != null)
					{
