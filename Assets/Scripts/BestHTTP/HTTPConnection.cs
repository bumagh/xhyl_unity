							...
							if (base.CurrentRequest.Response != null)
							{
								...
								case 308:
									if (base.CurrentRequest.RedirectCount < base.CurrentRequest.MaxRedirects)
									{
										...
										{
											HTTPManager.Logger.Verbose("HTTPConnection", $"{base.CurrentRequest.CurrentUri.ToString()} - Redirected to {firstHeaderValue}");
										}
								...
								TryStoreInCache();
								if (!base.CurrentRequest.IsKeepAlive || base.CurrentRequest.Response == null || (!base.CurrentRequest.Response.IsClosedManually && base.CurrentRequest.Response.HasHeaderWithValue("connection", "close")))
								{
									Close();
								}
