...
public class BCBM_Sockets : MonoBehaviour
{
	public bool isReconnect = true;

	private static BCBM_Sockets _MyCreateSocket;
	...
	public static BCBM_Sockets GetSingleton()
	{
	...
	public void CreateSocketGetPoint(BCBM_MessageControl MyMessageControl, BCBM_DataEncrypt MyDataEncrypt)
	{
		CreateReceiveThread();
	...
	public void SocketConnect()
	{
		...
		if (m_nSocketStartFlag)
		{
			if (!sendCheckVersion)
			{
				SendCheckVersion(BCBM_Constants.VERSION_CODE);
	...
	private bool _socketStart()
	{
		...
		IPEndPoint iPEndPoint = new IPEndPoint(IPAddress.Parse(BCBM_GameInfo.getInstance().IP), 10039);
	...
	private void _receiveNetMessage(string msg)
	{
		...
		try
		{
			...
			xmlReader.Read();
			BCBM_SimpleReader bCBM_SimpleReader = new BCBM_SimpleReader();
			Hashtable hashtable = bCBM_SimpleReader.read(xmlReader) as Hashtable;
