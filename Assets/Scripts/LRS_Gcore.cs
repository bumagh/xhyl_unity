...
public class LRS_Gcore : Cinstance<LRS_Gcore>
{
	...
	public void Init()
	{
		try
		{
			...
			{
				string text = $"img{i.ToString()}";
				Sprite sprite = LRS_MB_Singleton<LRS_GameManager>.GetInstance().Getanisprite(text);
	...
	{
		IsRoll = false;
		LRS_MB_Singleton<LRS_MajorGameController>.GetInstance().AllStop();
	}
	...
	public void QuickStop()
	{
		if (IsRoll)
		{
			LRS_MB_Singleton<LRS_MajorGameController>.GetInstance().StopRoll();
			IsRoll = false;
		}
	}
	...
	public void GetWinsym()
	{
		...
		{
			List<int> list = Getwinlinepos(WinlineList[i], Winlinenumlist[i]);
			showLineList.Add(i);
			WinSymlist.Add(list);
	...
	{
		IsRoll = true;
		LRS_MB_Singleton<LRS_MajorGameController>.GetInstance().StartRollSym();
	}
