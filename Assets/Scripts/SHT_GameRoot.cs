...
public class SHT_GameRoot : SHT_MB_Singleton<SHT_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(SHT_MB_Singleton<SHT_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            SHT_MB_Singleton<SHT_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : SHT_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : SHT_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
