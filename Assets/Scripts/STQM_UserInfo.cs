...
public class STQM_UserInfo : STQM_BaseItem
{
	...
	public STQM_UserInfo(STQM_User netUser)
	{
		if (netUser != null)
		{
			mId = netUser.id;
	...
	public STQM_UserInfo(int seatIndex, int photoId, string nickName)
	{
	...
	public static STQM_UserInfo createOtherUser(STQM_Seat netSeat)
	{
		STQM_UserInfo sTQM_UserInfo = new STQM_UserInfo(netSeat.user);
		sTQM_UserInfo.mSeatIndex = netSeat.seatId;
		sTQM_UserInfo.mIsExist = !netSeat.isFree;
		sTQM_UserInfo.mGunValue = netSeat.gunValue;
		return sTQM_UserInfo;
	}

	public static int compareBySeat(STQM_UserInfo t1, STQM_UserInfo t2)
	{
