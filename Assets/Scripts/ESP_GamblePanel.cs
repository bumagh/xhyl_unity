...
public class ESP_GamblePanel : MonoBehaviour
{
	...
	public static ESP_GamblePanel ins;
	...
	{
		ESP_MB_Singleton<ESP_DiceGameController2>.GetInstance().Send_MultipleInfo(ESP_BetDiceType.Half);
		SetButton(canclick: false);
	}

	private void ClickBlack()
	{
		ESP_MB_Singleton<ESP_DiceGameController2>.GetInstance().Send_MultipleInfo(ESP_BetDiceType.Risk);
		SetButton(canclick: false);
	}
	...
	private Sprite Getsprite()
	{
		try
		{
			uint num = Getnum(ESP_MB_Singleton<ESP_DiceGameController2>.GetInstance().Gamelewinid);
	...
	{
		balancescore = ESP_MySqlConnection.winScorce;
		doublescore = ESP_MySqlConnection.winScorce * 2;
	...
	{
		ESP_MB_Singleton<ESP_DiceGameController2>.GetInstance().OnBtnReady_Click();
		ESP_MB_Singleton<ESP_DiceGameController2>.GetInstance().Hide();
		int credit = (!isGamble) ? (ESP_MySqlConnection.winScorce + ESP_MySqlConnection.credit) : AllScroce;
		ESP_MB_Singleton<ESP_MajorGameController>.GetInstance().SetCredit(credit);
