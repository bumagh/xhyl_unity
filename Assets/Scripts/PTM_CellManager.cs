...
public class PTM_CellManager : MonoBehaviour
{
	public static PTM_CellManager cellManager;
	...
	private PTM_OverallComputer overall;

	private PTM_HitLineComputer[] winLines;
	...
	private List<PTM_WinLineResult> lineResults;
	...
	public static int[,] LineMap = new int[15, 5]
	{
		{
			1,
			1,
	...
	{
		List<int> showchange = PTM_MB_Singleton<PTM_MajorGameController>.GetInstance().Showchange1;
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				...
				{
					Cells[i, j].PlayState(PTM_CellState.normal, PTM_CellType.HuDie);
				}
			}
		}
	}
	...
	public void PreInit()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				Cells[i, j] = base.transform.Find($"Cell{i}_{j}").GetComponent<PTM_Cell>();
			}
		}
	}
	...
	public void SetAllCellsState(PTM_CellState cSF_CellState = PTM_CellState.normal)
	{
	...
	public IEnumerator GridRollingControl(Action endAction)
	{
		...
		for (int col2 = 0; col2 < 5; col2++)
		{
			for (int row2 = 0; row2 < 3; row2++)
			{
				if (col2 == 4 && row2 == 2)
				{
					yield return StartCoroutine(Cells[row2, col2].RolllingEnd((PTM_CellType)grid[row2, col2]));
					break;
				}
				StartCoroutine(Cells[row2, col2].RolllingEnd((PTM_CellType)grid[row2, col2]));
				if (!forceStopRolling)
				{
	...
	public int CalHitResult2(PTM_MajorResult majorResult, bool isChange = false)
	{
		...
		int num2 = 0;
		num = 0;
		overall = new PTM_OverallComputer(majorResult.gameContent);
		overall.Calculate();
		overallWin = overall.rate * lineNum * singleStake;
		UnityEngine.Debug.Log($"[overall] type: {overall.overallType}, rate: {overall.rate}");
		num += overallWin;
		winLines = new PTM_HitLineComputer[lineNum];
		winCount = 0;
		ResetHitGrid();
		lineResults = new List<PTM_WinLineResult>();
		List<int[]> list = new List<int[]>();
		for (int i = 0; i < lineNum; i++)
		{
			int[] betLine = GetBetLine(majorResult.gameContent, i);
			PTM_HitLineComputer pTM_HitLineComputer = new PTM_HitLineComputer(i, betLine);
			list.Add(betLine);
			PTM_WinLineResult[] array = pTM_HitLineComputer.Calculate2();
			if (array.Length == 2)
			{
	...
	private void PlayFullAwardAudio()
	{
		...
		PTM_SoundManager.Instance.PlayMajorGameAwardAudio(GetOverallType());
	}
	...
	public IEnumerator WinLinesAni(bool isReplay, float delay, int lineNum, int singleStake, PTM_LinesManager lineManager, PTM_BallManager ballManager, Action<int> changeScoreHandler, Action celebrateAction)
	{
		yield return new WaitForSeconds(delay);
		if (overall.rate > 0)
		{
			UnityEngine.Debug.LogError("overall.rate: " + overall.rate);
			PTM_SoundManager.Instance.PlayDrawLineAudio(0);
			for (int j = 0; j < 5; j++)
			{
				for (int k = 0; k < 3; k++)
				{
					Cells[k, j].PlayState(PTM_CellState.flash);
		...
		if (winCount > 0 || overall.rate != 0)
		{
			...
			for (int l = 0; l < 5; l++)
			{
				for (int m = 0; m < 3; m++)
				{
					Cells[m, l].PlayState(PTM_CellState.gray);
				}
			}
		}
		int lineAudioIndex = 0;
		int maxRate = 0;
		PTM_CellType celltype = PTM_CellType.None;
		PTM_WinLineType winlinetype = PTM_WinLineType.None;
		for (int i = 0; i < lineResults.Count; i++)
		{
			if (lineResults[i].winType != 0)
			{
				...
				PTM_SoundManager.Instance.PlayDrawLineAudio(lineAudioIndex);
				lineAudioIndex++;
		...
		if (!isReplay && (winCount > 0 || overall.rate != 0))
		{
			...
			PTM_SoundManager.Instance.PlayMajorGameAwardAudio(celltype, PTM_HitLineComputer.CalculateHitNum(winlinetype));
			UnityEngine.Debug.LogError("中奖类型: " + celltype + " 中奖个数: " + PTM_HitLineComputer.CalculateHitNum(winlinetype));
			yield return StartCoroutine(WinCellsCelebrateAni(overall.rate != 0));
		}
		allFinishedAction();
	}

	private IEnumerator WaitHideBrightBall(PTM_BallManager ballManager, int index)
	{
		yield return new WaitForSeconds(3f);
		ballManager.HideBrightBall(index);
	}

	private void SetCells(int[,] grid, PTM_CellState cSF_CellState = PTM_CellState.normal)
	{
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				Cells[i, j].PlayState(cSF_CellState, (PTM_CellType)grid[i, j]);
			}
		}
	}
	...
	{
		int num = Enum.GetNames(PTM_CellType.None.GetType()).Length;
	...
	private int[,] SetGridSame(ref int[,] grid, PTM_CellType type = PTM_CellType.ZiHua)
	{
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
	...
	private void ShowSingleLineBegin(int index, PTM_WinLineType lineType)
	{
		int num = 0;
		int num2 = 4;
		string state = "green";
		...
		for (int i = num; i <= num2; i++)
		{
			...
			Cells[LineMap[index, i], i].PlayState(PTM_CellState.flash);
		}
	}
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				if (hitGrid[i, j] >= 2 || overall.rate > 0)
				{
					...
					Cells[i, j].PlayState(PTM_CellState.celebrate);
				}
				else
				{
					Cells[i, j].HideBorder();
					Cells[i, j].PlayState(PTM_CellState.gray);
				}
			}
		}
	}
	...
	private void ShowSingleLineEnd(int index)
	{
		for (int i = 0; i < 5; i++)
		{
			...
			Cells[LineMap[index, i], i].PlayState(PTM_CellState.gray);
		}
	}
	...
	private IEnumerator WaitForCelebrate(int num)
	{
		while (celebrateCount < num)
		{
			...
			yield return new WaitForSeconds(3f);
			coShowAllHitCell = StartCoroutine(PTM_Utils.DelayCall(0.5f, delegate
			{
	...
	public void OnOverallPlayed(string eventName)
	{
		...
		{
			celebrateCount++;
			coShowAllHitCell = StartCoroutine(PTM_Utils.DelayCall(2f, delegate
			{
				if (celebrateCount < num)
				{
					ShowAllHitCell();
			...
			UnityEngine.Debug.Log(PTM_LogHelper.Aqua("eventName: {0}, _celebrateCount: {1}", eventName, celebrateCount));
		}
	}
	...
	private void SetHitGridLine(int lineIndex, PTM_WinLineType winType)
	{
		int num = 0;
		int num2 = 2;
		int num3 = 2;
