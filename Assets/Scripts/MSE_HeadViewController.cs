...
public class MSE_HeadViewController : MSE_MB_Singleton<MSE_HeadViewController>
{
	...
	private void Awake()
	{
		...
		if (MSE_MB_Singleton<MSE_HeadViewController>._instance == null)
		{
			MSE_MB_Singleton<MSE_HeadViewController>.SetInstance(this);
			PreInit();
		}
	}
	...
	private void Start()
	{
		btnBack.onClick.AddListener(MSE_MB_Singleton<MSE_GameManager>.GetInstance().Handle_BtnReturn);
	}
	...
	public void UpdateView()
	{
		...
		_textNickName.text = MSE_GVars.user.nickname;
		_textGoldCoin.text = MSE_GVars.user.gameGold.ToString();
		_textExpeCoin.text = MSE_GVars.user.expeGold.ToString();
		SetLevelInfo(MSE_GVars.user.level);
		_imgUserIcon.sprite = personIcon.spiIcons[MSE_GVars.user.photoId - 1];
	}

	public void UpdateView(MSE_User user)
	{
		_textHeadInfo.text = _headInfo;
	...
	{
		_textLevelInfo.text = "LV. " + level + "   " + MSE_Utils.GetLevelName(level, MSE_GVars.language);
	}
