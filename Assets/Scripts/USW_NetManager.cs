...
public class USW_NetManager : USW_MB_Singleton<USW_NetManager>
{
	public delegate void NetMessageHandler(object[] args);

	private USW_ClientSocket _pClient;

	private USW_DataEncrypt _pCrypto;
	...
	private Queue<USW_NetMessage> _netMsgQueue = new Queue<USW_NetMessage>();

	private Dictionary<string, NetMessageHandler> _netMsgHandlerDic = new Dictionary<string, NetMessageHandler>();

	private void Awake()
	{
		if (USW_MB_Singleton<USW_NetManager>._instance == null)
		{
			USW_MB_Singleton<USW_NetManager>.SetInstance(this);
		}
	...
	private void _initClientSocket()
	{
		...
		_pClient = new USW_ClientSocket(useTcpClient: true);
		_pCrypto = new USW_DataEncrypt();
	...
	private void _handleException(Exception ex)
	{
		...
		isReady = false;
		isLogined = false;
		Queue<USW_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
			_netMsgQueue.Enqueue(new USW_NetMessage("netDown", new object[1]
			{
				ex
			}));
		}
	...
	private void _handleConnected()
	{
		try
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new USW_NetMessage("netConnected", new object[0]));
			}
		}
		catch (Exception ex)
		{
	...
	{
		UnityEngine.Debug.Log("ZH2_GVars.versionCode: " + USW_GVars.versionCode);
		Send("userService/checkVersion", new object[1]
		{
			USW_GVars.versionCode
		});
	}
	...
	protected void _handleData(byte[] msgBuffer, int packetId, int endIndex)
	{
		...
		lock (netMsgQueue)
		{
			USW_NetMessage uSW_NetMessage = new USW_NetMessage(text, args);
			uSW_NetMessage.jsonString = @string;
			uSW_NetMessage.packetId = packetId;
			uSW_NetMessage.endIndex = endIndex;
			_netMsgQueue.Enqueue(uSW_NetMessage);
		}
	}

	private IEnumerator _checkTimeout(int id, int timeout, USW_ClientSocket cs)
	{
		UnityEngine.Debug.Log(USW_LogHelper.Aqua("_checkTimeout start id: {0}", id));
		...
		if (cs != null && cs.doneList.Count > id && !cs.doneList[id])
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new USW_NetMessage("timeout", new object[1]
				{
					id
				}));
				isTimeout = true;
		...
		if (isTimeout)
		{
			UnityEngine.Debug.Log(USW_LogHelper.Aqua("_checkTimeout start id: {0} timeout: {1}", id, isTimeout));
		}
	}
	...
	private IEnumerator _checkHeartTimeout(int timeout)
	{
		...
		if (isReady && !isHeartPong)
		{
			...
			lock (netMsgQueue)
			{
				...
				isReady = false;
				isLogined = false;
				_netMsgQueue.Enqueue(new USW_NetMessage("heart timeout", new object[0]));
				isTimeout = true;
			}
		}
		if (isTimeout)
		{
			UnityEngine.Debug.Log(USW_LogHelper.Aqua("_checkHeartTimeout is timeout"));
		}
	}

	public void _checkTimeoutStart(int id, int timeout, USW_ClientSocket cs)
	{
	...
	public void Send(string method, object[] args)
	{
		...
		{
			UnityEngine.Debug.Log(USW_LogHelper.Magenta("drop send>>method: {0}", method));
			return;
		}
		Hashtable hashtable = new Hashtable();
		hashtable.Add("method", method);
		hashtable.Add("args", args);
		hashtable.Add("version", USW_GVars.Version);
		if (isReady)
		{
	...
	private bool GetNetMessage(out USW_NetMessage netMsg)
	{
		Queue<USW_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
	...
	private void DispatchNetMessage(USW_NetMessage netMsg)
	{
		if (dropDispatch)
		{
			UnityEngine.Debug.Log(USW_LogHelper.Magenta("drop send>>method: {0}", netMsg.method));
		}
