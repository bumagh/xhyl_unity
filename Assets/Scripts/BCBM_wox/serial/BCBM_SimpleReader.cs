...
{
	public class BCBM_SimpleReader : ObjectReader
	{
		private Hashtable map;

		public BCBM_SimpleReader()
		{
			map = new Hashtable();
		}
		...
		public object readPrimitiveArray(XmlReader xob, object id)
		{
			try
			{
				Type type = (Type)BCBM_Serial.mapWOXToCSharp[xob.GetAttribute("elementType")];
		...
		public object readClassArray(Type[] a, string[] s)
		{
			...
			for (int i = 0; i < a.Length; i++)
			{
				...
				Type type = (Type)BCBM_Serial.mapWOXToCSharp[s[i]];
				if (type == null)
				{
					type = (Type)BCBM_Serial.mapArrayWOXToCSharp[s[i]];
					if (type == null)
					{
						try
						{
		...
		{
			Type type = (Type)BCBM_Serial.mapWOXToCSharp[arrayTypeName];
			if (type != null)
			{
				return type;
			}
			type = (Type)BCBM_Serial.mapArrayWOXToCSharp[arrayTypeName];
			if (type == null)
			{
		...
		public object readStringObject(XmlReader xob, object id)
		{
			try
			{
				Type type = (Type)BCBM_Serial.mapWOXToCSharp[xob.GetAttribute("type")];
				if (type.Equals(typeof(Type)))
				{
					Type type2 = (Type)BCBM_Serial.mapWOXToCSharp[xob.GetAttribute("value")];
					if (type2 == null)
					{
						type2 = (Type)BCBM_Serial.mapArrayWOXToCSharp[xob.GetAttribute("value")];
						if (type2 == null)
						{
		...
		public object readObject(XmlReader xob, object id)
		{
			try
			{
				...
				ConstructorInfo cons = Util.forceDefaultConstructor(type);
