...
public class STTF_Sockets : MonoBehaviour
{
	public bool isReconnect = true;

	private static STTF_Sockets _MyCreateSocket;
	...
	private STTF_DataEncrypt m_DataEncrypt;
	...
	public static STTF_Sockets GetSingleton()
	{
	...
	public void CreateSocketGetPoint(STTF_MessageControl MyMessageControl, STTF_DataEncrypt MyDataEncrypt)
	{
		CreateReceiveThread();
	...
	{
		UnityEngine.Debug.Log("CreateReceiveThread");
		try
		{
	...
	private bool _socketStart()
	{
		...
		IPEndPoint remoteEP = new IPEndPoint(IPAddress.Parse(STTF_GameInfo.getInstance().IP), 10031);
		try
		{
	...
	private void _receiveNetMessage(byte[] msgBytes)
	{
		...
		else
		{
			try
			{
				hashtable = STTF_ReadFileTool.JsonToClass<Hashtable>(@string);
			}
			catch (Exception)
			{
				msgBytes = m_DataEncrypt.DecompressBytes(msgBytes);
				Console.WriteLine(Encoding.UTF8.GetString(msgBytes));
				hashtable = STTF_ReadFileTool.JsonToClass<Hashtable>(Encoding.UTF8.GetString(msgBytes));
	...
	public bool _sendMsg(string strMethod, object[] args)
	{
		...
		else
		{
			...
			hashtable.Add("version", STTF_Constants.Version);
			hashtable.Add("time", m_DataEncrypt.GetUnixTime());
			string text = STTF_ReadFileTool.JsonByObject(hashtable);
	...
	public bool SendCheckLogin(string userName, string passWord, int uiid, string versionCode = "")
	{
		...
		UnityEngine.Debug.LogError("userName " + userName + " passWord " + passWord + " uiid " + uiid + " versionCode " + versionCode);
		return _sendMsg(strMethod, args);
	}
	...
	public bool SendGunHitfish(int gunid, STTF_HitFish[] hitfish1, bool bHaveKnife, STTF_HitFish[] hitfish2)
	{
		string empty = string.Empty;
	...
	public bool SendPublicKey()
	{
		...
		string s = STTF_ReadFileTool.JsonByObject(hashtable);
