...
public class TF_Formation : MonoBehaviour
{
	...
	public static TF_Formation G_Formation;

	private TF_FISH_TYPE FishType;

	private TF_FORMATION FormationType;

	private Vector3 rotVector = default(Vector3);

	public static TF_Formation GetSingleton()
	{
		return G_Formation;
	}
	...
	private void CreateAFish(TF_FORMATION formationType, float x, float y, float rot, TF_FISH_TYPE FishType, int i, float index = 0f)
	{
		Transform transform = TF_FishPoolMngr.GetSingleton().CreateFishForBig(FishType, i);
		rotVector = transform.eulerAngles;
		rotVector.x = rot / (float)Math.PI * 180f;
		transform.eulerAngles = rotVector;
		transform.GetComponent<TF_NormalFish>().InitNormalFish(FormationType, x, y, rot, index, FishType, i);
		transform.GetComponent<TF_NormalFish>().StartMove(move: true);
	}

	public void ShowFormation(TF_FORMATION type)
	{
	...
	private void CreateFormationBigFishes()
	{
		...
		for (int i = 0; i < 64; i++)
		{
			...
			if (i < 8)
			{
				FishType = TF_FISH_TYPE.Fish_Toad;
			...
			{
				FishType = TF_FISH_TYPE.Fish_Beauty;
			...
			{
				FishType = TF_FISH_TYPE.Fish_Dragon;
			...
			{
				FishType = TF_FISH_TYPE.Fish_Butterfly;
			...
			{
				FishType = TF_FISH_TYPE.Fish_Arrow;
			...
			{
				FishType = TF_FISH_TYPE.Fish_GoldenShark;
			...
			{
				FishType = TF_FISH_TYPE.Fish_SilverShark;
			...
			{
				FishType = TF_FISH_TYPE.Fish_Bat;
			...
			num4 /= 110f;
			num5 /= 110f;
			FormationType = TF_FORMATION.Formation_BigFishes;
	...
	private void CreateFormation_Fluctuate()
	{
		...
		for (int i = 0; i < 250; i++)
		{
			if (i < 70)
			{
				FishType = TF_FISH_TYPE.Fish_Shrimp;
				if (i % 2 == 0)
				{
			...
			{
				FishType = TF_FISH_TYPE.Fish_Zebra;
				if (i % 2 == 0)
				{
			...
			{
				FishType = TF_FISH_TYPE.Fish_YellowSpot;
				if (i % 2 == 0)
				{
			...
			{
				FishType = TF_FISH_TYPE.Fish_Lamp;
				if (i % 2 == 0)
				{
			...
			{
				FishType = TF_FISH_TYPE.Fish_Trailer;
				if (i % 2 == 0)
				{
			...
			num /= 110f;
			num2 /= 110f;
			FormationType = TF_FORMATION.Formation_Fluctuate;
		...
		for (int j = 250; j < 500; j++)
		{
			if (j < 320)
			{
				FishType = TF_FISH_TYPE.Fish_Grass;
				if (j % 2 == 0)
				{
			...
			{
				FishType = TF_FISH_TYPE.Fish_BigEars;
				if (j % 2 == 0)
				{
			...
			{
				FishType = TF_FISH_TYPE.Fish_Ugly;
				if (j % 2 == 0)
				{
			...
			{
				FishType = TF_FISH_TYPE.Fish_Hedgehog;
				if (j % 2 == 0)
				{
			...
			{
				FishType = TF_FISH_TYPE.Fish_Turtle;
				if (j % 2 == 0)
				{
	...
	private void CreateFormation_NineCircle()
	{
		...
		for (int i = 0; i < 160; i++)
		{
			...
			num7 /= 110f;
			num8 /= 110f;
			FormationType = TF_FORMATION.Formation_NineCircle;
	...
	private void CreateFormation_Hexagonal()
	{
		...
		for (int i = 0; i < num4; i++)
		{
			...
			num8 /= 110f;
			num9 /= 110f;
			FormationType = TF_FORMATION.Formation_Hexagonal;
	...
	private void CreateFormationRedCarpet()
	{
		for (int i = 0; i < 160; i++)
		{
			...
			num2 /= 110f;
			int i2 = i;
			FormationType = TF_FORMATION.Formation_RedCarpet;
	...
	{
		TF_FishPoolMngr.GetSingleton().DestroyFish(fish);
	}
	...
	private void CreateFormation_TwoCircles()
	{
		...
		for (int i = 0; i < 216; i++)
		{
			if (i < 108)
			{
				...
				if (i < 1)
				{
					FishType = TF_FISH_TYPE.Fish_BigShark;
					num3 = 0f;
					num6 = 0;
				}
				else if (i < 14)
			...
			num5 /= 100f;
			int i2 = i;
			FormationType = TF_FORMATION.Formation_TwoCircles;
	...
	private void CrateFormation_Cars()
	{
		...
		for (int i = 0; i < 94; i++)
		{
			...
			{
				FishType = TF_FISH_TYPE.Fish_CoralReefs;
			...
			num3 = (num3 - 300f) / 100f;
			int i2 = i;
			FormationType = TF_FORMATION.Formation_Cars;
	...
	private void CrateFormation_Wedding()
	{
		...
		for (int i = 0; i < 282; i++)
		{
			...
			num5 = (num5 - 400f) / 100f;
			int i2 = i;
			FormationType = TF_FORMATION.Formation_Wedding;
	...
	private void CreateFormation_TwoArmies()
	{
		...
		for (int i = 0; i < 218; i++)
		{
			...
			num6 = (num6 - 400f) / 100f;
			int i2 = i;
			FormationType = TF_FORMATION.Formation_TwoArmies;
	...
	private void CreateFormationSymmetry()
	{
		...
		for (int i = 0; i < 264; i++)
		{
			...
			{
				FishType = TF_FISH_TYPE.Fish_BlueAlgae;
			...
			num5 /= 100f;
			f = 0f;
			FormationType = TF_FORMATION.Formation_Symmetry;
	...
	private void CreateFormationSixCircles()
	{
		...
		for (int i = 0; i < 185; i++)
		{
			...
			f = 0f;
			FormationType = TF_FORMATION.Formation_SixCircles;
