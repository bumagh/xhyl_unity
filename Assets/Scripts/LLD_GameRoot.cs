...
public class LLD_GameRoot : LLD_MB_Singleton<LLD_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(LLD_MB_Singleton<LLD_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            LLD_MB_Singleton<LLD_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : LLD_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : LLD_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
