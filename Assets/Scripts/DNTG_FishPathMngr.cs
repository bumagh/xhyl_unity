...
public class DNTG_FishPathMngr : MonoBehaviour
{
	public static DNTG_FishPathMngr G_FishPathMngr;

	private DNTG_PathManager[] allFishPath;

	private DNTG_PathManager[] smallFishPath;
	...
	public static DNTG_FishPathMngr GetSingleton()
	{
	...
	private void Awake()
	{
		...
		int num = 0;
		_AllPathLength = 88;
		allFishPath = new DNTG_PathManager[_AllPathLength];
		...
		for (int i = 0; i < _AllPathLength; i++)
		{
			...
			allFishPath[i] = new DNTG_PathManager
			{
				vecs = new Vector3[num2]
			};
		...
		smallFishPath = new DNTG_PathManager[_AllPathLength * 4];
	...
	public DNTG_PathManager GetFishPath(int type)
	{
		return allFishPath[type];
	}

	public DNTG_PathManager GetSmallFishPath(int type)
	{
		return smallFishPath[type];
	}

	private DNTG_PathManager ChangePath(DNTG_PathManager path, Vector3 vec)
	{
		int num = path.vecs.Length;
		DNTG_PathManager dNTG_PathManager = new DNTG_PathManager();
		dNTG_PathManager.vecs = new Vector3[num];
		DNTG_PathManager dNTG_PathManager2 = dNTG_PathManager;
		for (int i = 0; i < num; i++)
		{
			dNTG_PathManager2.vecs[i] = path.vecs[i] + vec;
		}
		return dNTG_PathManager2;
	}

	public void GetAllPath(int type)
	{
		DNTG_PathManager dNTG_PathManager = smallFishPath[type];
		int num = dNTG_PathManager.vecs.Length;
		string text = string.Empty;
		for (int i = 0; i < num; i++)
		{
			string text2 = text;
			text = text2 + dNTG_PathManager.vecs[i][0].ToString() + "," + dNTG_PathManager.vecs[i][1].ToString() + "," + dNTG_PathManager.vecs[i][2].ToString() + " ";
		}
