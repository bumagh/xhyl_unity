...
public class STOF_DlgInOut : MonoBehaviour
{
	[SerializeField]
	...
	private STOF_ErrorTipAnim sptErrorTipAnim;
	...
	private void GetAndInitCompenent()
	{
		...
		sptErrorTipAnim = imgError.GetComponent<STOF_ErrorTipAnim>();
	...
	private void ClickBtnOut()
	{
		...
		{
			STOF_NetMngr.GetSingleton().MyCreateSocket.SendCoinOut(gameInfo.Table.DeltaCoin * gameInfo.Table.ScorePerCoin);
			STOF_SoundManage.getInstance().playButtonMusic(STOF_ButtonMusicType.coinUp);
			return;
		}
		int scoreCount = gameInfo.User.ScoreCount;
		if (scoreCount > 0)
		{
			STOF_NetMngr.GetSingleton().MyCreateSocket.SendCoinOut(scoreCount);
			STOF_SoundManage.getInstance().playButtonMusic(STOF_ButtonMusicType.coinUp);
		}
		else
		{
	...
	private void ClickBtnIn()
	{
		...
		if (num * gameInfo.Table.ScorePerCoin + gameInfo.User.ScoreCount <= 900000)
		{
			if (num > 0)
			{
				STOF_NetMngr.GetSingleton().MyCreateSocket.SendCoinIn(num);
				STOF_SoundManage.getInstance().playButtonMusic(STOF_ButtonMusicType.coinUp);
			}
			else
			{
