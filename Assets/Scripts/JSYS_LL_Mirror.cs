...
[ExecuteInEditMode]
public class JSYS_LL_Mirror : MonoBehaviour
{
	...
	public void OnWillRenderObject()
	{
		...
		{
			vector = base.transform.position - current.transform.position;
			vector.Normalize();
		}
		...
		float w = 0f - Vector3.Dot(vector, position) - m_ClipPlaneOffset;
		...
		{
			if (material.HasProperty("_Ref"))
			{
				material.SetTexture("_Ref", m_ReflectionTexture);
			}
		}
		if (m_DisablePixelLights)
		{
	...
	private void OnDisable()
	{
		...
		try
		{
			while (enumerator.MoveNext())
			{
				...
				UnityEngine.Object.DestroyImmediate(((Camera)((DictionaryEntry)current).Value).gameObject);
			}
		}
		finally
		{
