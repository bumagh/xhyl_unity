...
public class DNTG_Sockets : MonoBehaviour
{
	public bool isReconnect = true;

	private static DNTG_Sockets _MyCreateSocket;
	...
	private DNTG_DataEncrypt m_DataEncrypt;
	...
	public static DNTG_Sockets GetSingleton()
	{
	...
	public void CreateSocketGetPoint(DNTG_MessageControl MyMessageControl, DNTG_DataEncrypt MyDataEncrypt)
	{
		CreateReceiveThread();
	...
	public void SocketConnect()
	{
		...
		if (m_nSocketStartFlag)
		{
			if (!sendCheckVersion)
			{
				SendCheckVersion(DNTG_Constants.VERSION_CODE);
	...
	private bool _socketStart()
	{
		...
		IPEndPoint iPEndPoint = new IPEndPoint(IPAddress.Parse(DNTG_GameInfo.getInstance().IP), 10040);
	...
	private void _receiveNetMessage(byte[] msgBytes)
	{
		...
		else
		{
			try
			{
				hashtable = DNTG_ReadFileTool.JsonToClass<Hashtable>(@string);
			}
			catch
			{
				msgBytes = m_DataEncrypt.DecompressBytes(msgBytes);
				Console.WriteLine(Encoding.UTF8.GetString(msgBytes));
				hashtable = DNTG_ReadFileTool.JsonToClass<Hashtable>(Encoding.UTF8.GetString(msgBytes));
	...
	public bool _sendMsg(string strMethod, object[] args)
	{
		...
		else
		{
			...
			hashtable.Add("version", DNTG_Constants.Version);
			hashtable.Add("time", m_DataEncrypt.GetUnixTime());
			string text = DNTG_ReadFileTool.JsonByObject(hashtable);
	...
	public bool SendFired(int gunid, int seatid, float fRot, int gunValue, bool isSpeedGun, bool bX2Gun = false)
	{
		string strMethod = "fired," + gunid + "," + Math.Round(fRot, 2) + "," + gunValue + "," + bX2Gun + "," + isSpeedGun;
		return _sendMsg(strMethod, null);
	}

	public bool SendGunHitfish(int gunid, DNTG_HitFish[] hitfish1, bool bHaveKnife, DNTG_HitFish[] hitfish2)
	{
		string empty = string.Empty;
	...
	public bool SendPublicKey()
	{
		...
		string s = DNTG_ReadFileTool.JsonByObject(hashtable);
