...
public class PTM_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {PTM_LogHelper.Key(btnKey)} value {PTM_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {PTM_LogHelper.Key(btnKey)} value {PTM_LogHelper.Key(GetValue(btnKey))}");
			}
