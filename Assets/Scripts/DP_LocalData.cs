...
public class DP_LocalData
{
	private XmlDocument mXmlDoc = new XmlDocument();

	private string mFileName = Application.persistentDataPath + "/newLion.xml";

	private static DP_LocalData mSettedManager;

	public static DP_LocalData getInstance()
	{
		if (mSettedManager == null)
		{
			mSettedManager = new DP_LocalData();
		}
	...
	public void loadUserSetting()
	{
		...
		DP_GameInfo.getInstance().Setted = ReadUserSetting();
	}
	...
	{
		DP_SettedInfo setted = DP_GameInfo.getInstance().Setted;
	...
	private DP_SettedInfo ReadUserSetting()
	{
		DP_SettedInfo dP_SettedInfo = new DP_SettedInfo();
		dP_SettedInfo.bIsGameVolum = bool.Parse(_readData("/Data/UserSetting/GAMEVOLUM", "GameVolum"));
		dP_SettedInfo.bIsButtonVolum = bool.Parse(_readData("/Data/UserSetting/BUTTONVOLUM", "ButtonVolum"));
		dP_SettedInfo.bIsForbidPublicChat = bool.Parse(_readData("/Data/UserSetting/ISFORBIDPUBLICCHAT", "IsForbidPublicChat"));
		dP_SettedInfo.bIsForbidPrivateChat = bool.Parse(_readData("/Data/UserSetting/ISFORBIDPRIVATECHAT", "IsForbidPrivateChat"));
		dP_SettedInfo.bIsScreenNeverSleep = bool.Parse(_readData("/Data/UserSetting/ISSCREENNEVERSLEEP", "IsScreenNeverSleep"));
		return dP_SettedInfo;
	}
