...
public class LRS_GameRoot : LRS_MB_Singleton<LRS_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(LRS_MB_Singleton<LRS_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            LRS_MB_Singleton<LRS_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : LRS_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : LRS_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
