...
public class LL_HistoryRecord : MonoBehaviour
{
	...
	protected void _updateRecord()
	{
		...
		for (int i = 0; i < 21; i++)
		{
			if (i < mRecordCount)
			{
				...
				{
					LL_ErrorManager.GetSingleton().AddError("Error:记录结果庄和闲错误" + i + "_" + mHistoryList[i].zhuangXianHe);
				...
				if (mHistoryList[i].awardType == 0)
				{
					...
					{
						LL_ErrorManager.GetSingleton().AddError("Error:记录结果中奖动物错误" + i + "_" + mHistoryList[i].animalType);
				...
				if (mHistoryList[i].awardType == 3)
				{
					...
					{
						LL_ErrorManager.GetSingleton().AddError("Error:记录结果大三元动物错误" + i + "_" + mHistoryList[i].animalType);
				...
				if (mHistoryList[i].awardType == 4)
				{
					...
					{
						LL_ErrorManager.GetSingleton().AddError("Error:记录结果大四喜颜色错误" + i + "_" + mHistoryList[i].animalType);
				...
				if (mHistoryList[i].awardType == 5)
				{
					...
					{
						LL_ErrorManager.GetSingleton().AddError("Error:记录结果送灯错误" + i + "_" + mHistoryList[i].songDengCount);
				...
				if (num < 0 || num > 2)
				{
					LL_ErrorManager.GetSingleton().AddError("Error:记录结果幸运奖错误" + i + "_" + num);
					num = 0;
				}
				...
				{
					LL_ErrorManager.GetSingleton().AddError("Error:记录结果分机错误" + i + "_" + mHistoryList[i].luckNumber);
