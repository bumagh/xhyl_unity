...
public class Dzb_ClientSocket
{
	...
	public Action<int, int, Dzb_ClientSocket> checkTimeoutHandler;
	...
	public Dzb_ClientSocket(bool useTcpClient)
	{
	...
	{
		UnityEngine.Debug.Log("Connect||" + host + ":" + port);
		try
		{
	...
	private void _onConnected(IAsyncResult ar)
	{
		...
		lock (_lock)
		{
			...
			{
				UnityEngine.Debug.LogError("异常: " + ex.Message);
				exceptionHandler(ex);
			}
		}
	}
	...
	public void Send(byte[] msgBytes, bool useHead = true)
	{
		...
		lock (_lock)
		{
			...
			{
				UnityEngine.Debug.LogError("BeginSend: " + ex);
				exceptionHandler(ex);
			}
		}
	}
