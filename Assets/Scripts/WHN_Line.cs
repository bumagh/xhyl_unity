...
public class WHN_Line : MonoBehaviour
{
	private List<WHN_SymbolC> iconlist;
	...
	{
		result = new List<int>();
		iconlist = new List<WHN_SymbolC>();
		Rollsymnum = new List<int>();
		RandomIco = Cinstance<WHN_Gcore>.Instance.Symcont;
	...
	private void Initline()
	{
		...
		try
		{
			while (enumerator.MoveNext())
			{
				...
				component.Init(symid: (num > 2) ? UnityEngine.Random.Range(0, 6) : Cinstance<WHN_Gcore>.Instance.Normallist[index], idx: num++, line: this);
		...
		if (!stopline)
		{
			try
			{
				...
				if (LineIdx == 4)
				{
					...
					t.SetDelay(1f).AppendCallback(Cinstance<WHN_Gcore>.Instance.AllStop);
				}
			}
			catch (Exception arg)
			{
