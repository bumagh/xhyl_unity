...
[AddComponentMenu("Path-o-logical/DK_PoolManager/SpawnPool")]
public sealed class DK_SpawnPool : MonoBehaviour, IList<Transform>, ICollection<Transform>, IEnumerable<Transform>, IEnumerable
{
	...
	public List<DK_PrefabPool> _perPrefabPoolOptions = new List<DK_PrefabPool>();
	...
	public DK_PrefabsDict prefabs = new DK_PrefabsDict();

	public Dictionary<object, bool> _editorListItemStates = new Dictionary<object, bool>();

	private List<DK_PrefabPool> _prefabPools = new List<DK_PrefabPool>();
	...
	public Dictionary<string, DK_PrefabPool> prefabPools
	{
		get
		{
			Dictionary<string, DK_PrefabPool> dictionary = new Dictionary<string, DK_PrefabPool>();
			foreach (DK_PrefabPool prefabPool in _prefabPools)
			{
	...
	private void Awake()
	{
		...
		foreach (DK_PrefabPool perPrefabPoolOption in _perPrefabPoolOptions)
		{
			if (perPrefabPoolOption.prefab == null)
			{
				UnityEngine.Debug.LogWarning($"Initialization Warning: Pool '{poolName}' contains a DK_PrefabPool with no prefab reference. Skipping.");
			}
			else
			{
	...
	private void OnDestroy()
	{
		...
		DK_PoolManager.Pools.Remove(this);
		StopAllCoroutines();
		_spawned.Clear();
		foreach (DK_PrefabPool prefabPool in _prefabPools)
		{
	...
	public void CreatePrefabPool(DK_PrefabPool prefabPool)
	{
	...
	public void Add(Transform instance, string prefabName, bool despawn, bool parent)
	{
		foreach (DK_PrefabPool prefabPool in _prefabPools)
		{
			...
			{
				UnityEngine.Debug.LogError("Unexpected Error: DK_PrefabPool.prefabGO is null");
				return;
			}
		...
		UnityEngine.Debug.LogError($"SpawnPool {poolName}: DK_PrefabPool {prefabName} not found.");
	}
	...
	public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot)
	{
		...
		DK_PrefabPool dK_PrefabPool = new DK_PrefabPool(prefab);
		CreatePrefabPool(dK_PrefabPool);
		transform = dK_PrefabPool.SpawnInstance(pos, rot);
		transform.parent = group;
		_spawned.Add(transform);
		return transform;
	}
	...
	public Transform GetPrefab(Transform prefab)
	{
		foreach (DK_PrefabPool prefabPool in _prefabPools)
		{
			if (prefabPool.prefabGO == null)
			{
				UnityEngine.Debug.LogError($"SpawnPool {poolName}: DK_PrefabPool.prefabGO is null");
			}
			if (prefabPool.prefabGO == prefab.gameObject)
			{
	...
	private IEnumerator ListenForEmitDespawn(ParticleSystem emitter)
	{
		...
		{
			if (!DK_PoolManagerUtils.activeInHierarchy(emitter.gameObject))
			{
