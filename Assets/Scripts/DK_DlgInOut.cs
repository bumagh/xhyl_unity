...
public class DK_DlgInOut : MonoBehaviour
{
	[SerializeField]
	...
	private DK_ErrorTipAnim sptErrorTipAnim;
	...
	private void GetAndInitCompenent()
	{
		...
		sptErrorTipAnim = imgError.GetComponent<DK_ErrorTipAnim>();
	...
	private void ClickBtnOut()
	{
		...
		{
			DK_NetMngr.GetSingleton().MyCreateSocket.SendCoinOut(gameInfo.Table.DeltaCoin * gameInfo.Table.ScorePerCoin);
			DK_SoundManage.getInstance().playButtonMusic(DK_ButtonMusicType.coinUp);
			return;
		}
		int scoreCount = gameInfo.User.ScoreCount;
		if (scoreCount > 0)
		{
			DK_NetMngr.GetSingleton().MyCreateSocket.SendCoinOut(scoreCount);
			DK_SoundManage.getInstance().playButtonMusic(DK_ButtonMusicType.coinUp);
		}
		else
		{
	...
	private void ClickBtnIn()
	{
		...
		if (num * gameInfo.Table.ScorePerCoin + gameInfo.User.ScoreCount <= 900000)
		{
			if (num > 0)
			{
				DK_NetMngr.GetSingleton().MyCreateSocket.SendCoinIn(num);
				DK_SoundManage.getInstance().playButtonMusic(DK_ButtonMusicType.coinUp);
			}
			else
			{
