...
public class SPA_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {SPA_LogHelper.Key(btnKey)} value {SPA_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {SPA_LogHelper.Key(btnKey)} value {SPA_LogHelper.Key(GetValue(btnKey))}");
			}
