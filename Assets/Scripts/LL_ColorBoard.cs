...
public class LL_ColorBoard : MonoBehaviour
{
	...
	public LL_SpinScript m_LightPoiter;
	...
	private void Start()
	{
		...
		m_LightPoiter = GameObject.Find("BallPointer").GetComponent<LL_SpinScript>();
		Reset();
	}
	...
	{
		LL_MusicMngr.GetSingleton().PlayGameMusic(LL_MusicMngr.MUSIC_GAME_MUSIC.GAME_BG);
		LL_MusicMngr.GetSingleton().PlaySceneSound(LL_MusicMngr.MUSIC_SCENE_MUSIC.SCENE_RESET_COLOR);
	...
	public bool Test()
	{
		...
		if (!flag)
		{
			UnityEngine.Debug.Log("----Red Error!!----");
			return false;
		}
		flag = false;
		array2[0] = false;
		...
		if (!flag)
		{
			UnityEngine.Debug.Log("----Green Error!!----");
			return false;
		}
		flag = false;
		array2[0] = false;
		...
		if (!flag)
		{
			UnityEngine.Debug.Log("----Yellow Error!!----");
			return false;
		}
		return flag;
	}
	...
	public void GetLampArray(int[] lampArray)
	{
		...
		{
			UnityEngine.Debug.Log("GetLampArray lampArray length must be 24!");
		}
	}
