...
public class ClientSocket
{
	...
	public void Connect(string host, int port)
	{
		...
		{
			UnityEngine.Debug.LogError("链接异常: " + ex);
			exceptionHandler(ex);
		}
	}
	...
	private void _onConnected(IAsyncResult ar)
	{
		...
		lock (_lock)
		{
			try
			{
				...
				if (_pTcpClient == null)
				{
					UnityEngine.Debug.LogError("======pTcpClient为空=====");
				}
				if (ar == null)
				{
					UnityEngine.Debug.LogError("====ar未空====");
				}
				else
				{
					UnityEngine.Debug.LogError(ar.ToString());
				}
	...
	public void EndReceiveHandle(IAsyncResult ar)
	{
		...
		lock (_lock)
		{
			try
			{
				if (!ar.IsCompleted)
				{
					throw new Exception("EndReceiveHandle未完成");
				}
				if (!_pTcpClient.Connected)
				{
					exceptionHandler(new Exception("未链接成功"));
					return;
				}
	...
	public void Send(byte[] msgBytes, bool useHead = true)
	{
		...
		lock (_lock)
		{
			...
			catch (Exception ex)
			{
				...
				UnityEngine.Debug.Log("BeginSend: " + ex.StackTrace);
				exceptionHandler(ex);
			}
		}
	}
