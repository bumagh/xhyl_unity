...
public class SPA_GameRoot : SPA_MB_Singleton<SPA_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(SPA_MB_Singleton<SPA_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            SPA_MB_Singleton<SPA_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : SPA_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : SPA_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
