...
public class BCBM_HistoryRecord : MonoBehaviour
{
	...
	protected BCBM_HudManager mHudMngr;
	...
	private void Start()
	{
		mHudMngr = GameObject.Find("HudPanel").GetComponent<BCBM_HudManager>();
	...
	protected void _updateRecord()
	{
		...
		for (int i = 0; i < 21; i++)
		{
			if (i < mRecordCount)
			{
				...
				{
					BCBM_ErrorManager.GetSingleton().AddError("Error:记录结果庄和闲错误" + i + "_" + mHistoryList[i].zhuangXianHe);
				...
				if (mHistoryList[i].awardType == 0)
				{
					...
					{
						BCBM_ErrorManager.GetSingleton().AddError("Error:记录结果中奖动物错误" + i + "_" + mHistoryList[i].animalType);
				...
				if (mHistoryList[i].awardType == 3)
				{
					...
					{
						BCBM_ErrorManager.GetSingleton().AddError("Error:记录结果大三元动物错误" + i + "_" + mHistoryList[i].animalType);
				...
				if (mHistoryList[i].awardType == 4)
				{
					...
					{
						BCBM_ErrorManager.GetSingleton().AddError("Error:记录结果大四喜颜色错误" + i + "_" + mHistoryList[i].animalType);
				...
				if (mHistoryList[i].awardType == 5)
				{
					...
					{
						BCBM_ErrorManager.GetSingleton().AddError("Error:记录结果送灯错误" + i + "_" + mHistoryList[i].songDengCount);
				...
				if (num < 0 || num > 2)
				{
					BCBM_ErrorManager.GetSingleton().AddError("Error:记录结果幸运奖错误" + i + "_" + num);
					num = 0;
				}
				...
				{
					BCBM_ErrorManager.GetSingleton().AddError("Error:记录结果分机错误" + i + "_" + mHistoryList[i].luckNumber);
