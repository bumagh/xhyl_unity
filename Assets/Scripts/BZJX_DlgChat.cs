...
public class BZJX_DlgChat : MonoBehaviour
{
	[SerializeField]
	...
	[HideInInspector]
	public BZJX_ChatCtrl sptChatCtrl;
	...
	private void ClickBtnCommonUse(int index)
	{
		...
		BZJX_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, ifChart.text);
	...
	private void ClickBtnSend()
	{
		if (ifChart.text != string.Empty)
		{
			...
			BZJX_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, chatMessage);
			base.gameObject.SetActive(value: false);
		}
		else
		{
	...
	public void ShowChat()
	{
		...
		if (gameInfo.IsGameShuUp || gameInfo.IsUserShutUp)
		{
			...
			btnSend.GetComponent<BZJX_ButtonImgAni>().enabled = false;
			btnSend.image.sprite = spiBtnSend[language * 2 + 1];
			SetBtnCommonUse(b: false);
		}
		else
		{
			btnSend.GetComponent<BZJX_ButtonImgAni>().enabled = true;
