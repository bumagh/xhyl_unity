...
public abstract class DNTG_IMoveObj : MonoBehaviour
{
	...
	private float _fLimitLeft = 0f - DNTG_SetCanvas.Width * 6.2f / 1280f;

	private float _fLimitRight = DNTG_SetCanvas.Width * 6.2f / 1280f;
	...
	private void Start()
	{
		_fLimitLeft = 0f - DNTG_SetCanvas.Width * 6.2f / 1280f;
		_fLimitRight = DNTG_SetCanvas.Width * 6.2f / 1280f;
	}
	...
	public void MoveLogic()
	{
		...
		{
			if (DNTG_GameInfo.getInstance().UserList[i].SeatIndex != _nPlayerID)
			{
				continue;
			}
			if ((bool)DNTG_GameInfo.getInstance().UserList[i].LockFish)
			{
				DNTG_ISwimObj swimObjByTag = DNTG_FishPoolMngr.GetSingleton().GetSwimObjByTag(DNTG_GameInfo.getInstance().UserList[i].LockFish);
				if (DNTG_GameInfo.getInstance().UserList[i].Lock && !swimObjByTag.bFishDead)
				{
