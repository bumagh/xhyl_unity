...
public class PHG_SymbolC : MonoBehaviour
{
	...
	private string aniname = "icon{0:0}";
	...
	private void Awake()
	{
		...
		symlen = Cinstance<PHG_Gcore>.Instance.Symcont;
	...
	public void Init(int idx, int symid, PHG_Line line = null)
	{
		symidx = idx;
		lineroll = line;
	...
	{
		iconimg.sprite = Cinstance<PHG_Gcore>.Instance.SymSprlist[idx];
	}
	...
	public void StopRoll()
	{
		...
		if (isroll)
		{
			...
			if (Inlineidx > 2)
			{
				...
				int idx = Cinstance<PHG_Gcore>.Instance.Result[index];
				SetSymbolimg(idx, 0L);
			}
		}
		PHG_SoundManager.Instance.PlayStopAndio();
		speed = normalspeed;
	}
	...
	private void Setrollnum(List<int> randomlist, int rolllen)
	{
		...
		if (Inlineidx > 2)
		{
			...
			int value = Cinstance<PHG_Gcore>.Instance.Result[index2];
