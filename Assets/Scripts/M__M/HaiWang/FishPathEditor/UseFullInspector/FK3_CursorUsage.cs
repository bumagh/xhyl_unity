...
namespace M__M.HaiWang.FishPathEditor.UseFullInspector
{
	public class FK3_CursorUsage
	{
		...
		internal Action<FK3_CursorUsage> FreeAction;
		...
		{
			Transform prefab = FK3_BGPathMgr.Get().PathPool.prefabs["1"];
			Transform transform = FK3_BGPathMgr.Get().PathPool.Spawn(prefab, FK3_BGPathMgr.Get().PathPool.transform);
		...
		public void DespawnBullet(GameObject _bullet)
		{
			if (!(_bullet == null))
			{
				FK3_BGPathMgr.Get().PathPool.Despawn(_bullet.transform);
			}
		}
		...
		private void SetCursorUsageLinearSpeed(FK3_KeyValueInfo keyValueInfo)
		{
			linear.Speed = 4f;
		}
	}
