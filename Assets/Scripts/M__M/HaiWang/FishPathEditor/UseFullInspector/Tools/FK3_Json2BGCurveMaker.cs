...
{
	[fiInspectorOnly]
	public class FK3_Json2BGCurveMaker : MonoBehaviour
	{
		...
		private void LoadFromJson()
		{
			...
			FK3_PathPointTable fK3_PathPointTable = null;
			try
			{
				fK3_PathPointTable = JsonReader.Deserialize<FK3_PathPointTable>(textAsset.text);
			}
			catch (Exception arg)
			{
			...
			FK3_PathPointData[] data = fK3_PathPointTable.data;
			foreach (FK3_PathPointData fK3_PathPointData in data)
			{
				if (fK3_PathPointData.count > 9)
				{
					GameObject gameObject = new GameObject();
					gameObject.name = fK3_PathPointData.name;
					gameObject.transform.SetParent(transform);
					BGCurve bGCurve = gameObject.AddComponent<BGCurve>();
					bGCurve.Mode2D = BGCurve.Mode2DEnum.XY;
					BGCurvePoint[] array = new BGCurvePoint[fK3_PathPointData.points.Length];
					int j = 0;
					for (int num = fK3_PathPointData.points.Length; j < num; j++)
					{
						float[] array2 = fK3_PathPointData.points[j];
						if (num == 2)
						{
						...
						if (j > 0)
						{
							array3 = fK3_PathPointData.points[j - 1];
						}
						else
						{
						...
						if (j < num - 1)
						{
							array6 = fK3_PathPointData.points[j + 1];
						}
						else
						{
