...
namespace M__M.HaiWang.UI
{
	...
	{
		private Demo_UI_State m_curState = Demo_UI_State.StartupLoading;

		private LobbyContext m_context = new LobbyContext();

		private static LobbyUI s_instance;

		public event Action<LobbyContext> EventHandler_UI_Login_OnBtnLoginClick;

		public event Action<LobbyContext> EventHandler_UI_RoomSelection_OnRoomClick;

		public event Action<LobbyContext> EventHandler_UI_DeskSelection_OnSeatClick;

		public event Action<LobbyContext> EventHandler_UI_DeskSelection_OnBtnReturnClick;

		public static LobbyUI Get()
		{
			return s_instance;
		}
		...
		private void Display_UI_DeskSelection()
		{
			...
			{
				GUILayout.BeginHorizontal();
				DeskInfo deskInfo = m_context.desks[i];
				GUILayout.Label(deskInfo.name);
				for (int j = 0; j < 4; j++)
				{
					int num6 = j + 1;
					SeatInfo seat = deskInfo.GetSeat(num6);
					if (seat == null)
					{
					...
					{
						m_context.curDeskId = deskInfo.id;
		...
		private void Display_StateChangeTest()
		{
			...
			if (GUI.Button(new Rect(Screen.width - num, Screen.height - num3 - 20, num, num3), "切换UI"))
			{
				switch (m_curState)
				{
				case Demo_UI_State.StartupLoading:
					Change_UI_State(Demo_UI_State.Login);
					break;
				case Demo_UI_State.Login:
					Change_UI_State(Demo_UI_State.RoomSelection);
					break;
				case Demo_UI_State.RoomSelection:
					Change_UI_State(Demo_UI_State.DeskSelection);
					break;
				case Demo_UI_State.DeskSelection:
					Change_UI_State(Demo_UI_State.InGame);
					break;
				case Demo_UI_State.InGame:
					Change_UI_State(Demo_UI_State.StartupLoading);
					break;
				}
			}
		}
		...
		public void Change_UI_State(Demo_UI_State newState)
		{
			m_curState = newState;
		}
