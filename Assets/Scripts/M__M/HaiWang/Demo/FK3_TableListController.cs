...
{
	public class FK3_TableListController : FK3_SimpleSingletonBehaviour<FK3_TableListController>
	{
		private static FK3_TableListController instance;
		...
		private List<FK3_DeskInfo> deskinfo = new List<FK3_DeskInfo>();

		public List<GameObject> itemList = new List<GameObject>();

		public new static FK3_TableListController Get()
		{
			return instance;
		}
		...
		{
			instance = this;
			FK3_SimpleSingletonBehaviour<FK3_TableListController>.s_instance = this;
		}
		...
		private void Start()
		{
			...
			for (int i = 0; i < 15; i++)
			{
				...
				fK3_DeskInfo.name = "desk" + i;
				for (int j = 0; j < 4; j++)
				{
					fK3_DeskInfo.seats[j].isUsed = (j % 2 == 0);
				}
				deskinfo.Add(fK3_DeskInfo);
			}
		}
		...
		public void InitTableList(Action<int> act)
		{
			...
			{
				GenerateTableItem(FK3_GVars.lobby.desks);
			}
		...
		public void ShowTableList()
		{
			...
			{
				_scrollRect.verticalScrollbar = ((!_blank.activeSelf) ? null : ((FK3_GVars.lobby.desks.Count > 8) ? _scrollBar : null));
			}
			_scrollBar.gameObject.SetActive(value: false);
			if ((bool)_scrollBar)
			{
		...
		public void UpdateListUI(int index)
		{
			...
			{
				string name = FK3_GVars.lobby.desks[index].name;
				int seatUseCount = FK3_GVars.lobby.desks[index].GetSeatUseCount();
				...
				private void GenerateTableItem(List<FK3_DeskInfo> desk)
				{
					if (_gridRoot.childCount > 1)
					{
