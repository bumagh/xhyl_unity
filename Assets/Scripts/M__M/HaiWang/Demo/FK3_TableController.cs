...
{
	public class FK3_TableController : FK3_SimpleSingletonBehaviour<FK3_TableController>
	{
		private static FK3_TableController instance;
		...
		public new static FK3_TableController Get()
		{
			return instance;
		}
		...
		{
			instance = this;
			FK3_SimpleSingletonBehaviour<FK3_TableController>.s_instance = this;
			isOpening = false;
			isCanClick = true;
		...
		{
			chooseTableAct = act;
			UpdateTableList(FK3_GVars.lobby.desks);
		}
		...
		private void UpdateTableList(List<FK3_DeskInfo> desk)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				btnChilds[index].onClick.AddListener(delegate
				{
					ClickTableListItem(desk[index].seats, desk[index].id);
				});
			}
		}

		private void ClickTableListItem(FK3_SeatInfo[] netSeats, int deskNum)
		{
			FK3_MB_Singleton<FK3_GameUIController>.Get().EnterSeat(netSeats, deskNum);
		}
