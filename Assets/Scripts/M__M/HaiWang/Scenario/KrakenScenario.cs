...
{
	[Serializable]
	public class KrakenScenario : ScenarioBase
	{
		public Image bg_red;
		...
		private KrakenScenario()
			: base(ScenarioType.Boss_Kraken_深海八爪鱼)
		{
		}
		...
		{
			UnityEngine.Debug.Log(HW2_LogHelper.Brown("KrakenScenario ScenarioProgress"));
		...
		{
			Reset();
			UnityEngine.Debug.Log(HW2_LogHelper.Brown("KrakenScenario Enter"));
			HW2_Singleton<SoundMgr>.Get().PlayClip("深海八爪鱼剧情背景音乐", loop: true);
			HW2_Singleton<SoundMgr>.Get().SetVolume("深海八爪鱼剧情背景音乐", 0.7f);
		...
		{
			UnityEngine.Debug.Log(HW2_LogHelper.Magenta("KrakenScenario BossEnter"));
		...
		{
			UnityEngine.Debug.Log(HW2_LogHelper.Brown("KrakenScenario ScenarioRotate"));
			int num = dur / 10;
			startTime = dur;
