...
{
	public class KrakenBeheviour : FishBehaviour
	{
		protected Task _playLifeAniTask;

		protected Task _retractDieTask;
		...
		public void SetPositionAndRemainTime(int pos, float time)
		{
			remainTime = time;
			int num = pos + 1;
			Transform krakenBornPosList = fiSimpleSingletonBehaviour<StoryScenarioMgr>.Get().krakenBornPosList;
			...
			_blinkCircle.GetComponent<SortingLayerSetter>().OrderInLayer = num3 + 1;
		}
		...
		private void Awake()
		{
			...
			{
				_collisionReceiver = GetComponent<CollisionReceiver>();
			}
			if (_collisionReceiver == null)
			{
				_collisionReceiver = base.transform.GetComponentInChildren<CollisionReceiver>();
			}
			_collisionReceiver.actionOnTriggerEnter = delegate(Transform _transform, Collider _collider)
			{
				if (_collider.CompareTag("Bullet"))
				{
					...
					EffectMgr.Get().PlayBlinkCirle(_blinkCircle, int.Parse(name2.Substring(name2.Length - 1, 1)));
			...
			_collisionReceiver.actionOnMouseDown = delegate
			{
				...
				EffectMgr.Get().PlayBlinkCirle(_blinkCircle, int.Parse(name.Substring(name.Length - 1, 1)));
				OnMouseDown();
			};
		}
		...
		private void CleanKraken(KeyValueInfo keyValueInfo)
		{
			if (base.State == FishState.Live)
			{
				Die();
		...
		public override void Prepare()
		{
			...
			_playLifeAniTask = new Task(PlayLifeAniAfterBirth());
		...
		public override void Retract()
		{
			...
			HW2_MessageCenter.SendMessage("CheckAutoLock", null);
			_retractDieTask = new Task(RetractDie());
		}

		private IEnumerator RetractDie()
		{
			UnityEngine.Debug.LogError("RetractDie");
			try
			{
				animator.ResetTrigger("Birth");
				animator.ResetTrigger("Life");
