...
{
	public class FK3_GunController : MonoBehaviour
	{
		public delegate void EventHandler_RotateByInput(FK3_GunController gun, Vector3 begin, Vector3 end, float angle);

		public delegate void EventHandler_Shoot(FK3_GunController gun, Vector3 pos, bool faceDown, float angle, int gunValue, int seatId);
		...
		public FK3_FSM<FK3_GunController> gunFSM;
		...
		private void Start()
		{
			...
			_lockingGunGmObj = base.transform.Find("Gun/FK3_LockingGun").gameObject;
			_EMgunGmObj = base.transform.Find("Gun/FK3_EMGun").gameObject;
			...
			gunFSM = new FK3_FSM<FK3_GunController>(this);
			gunFSM.ChangeState<FK3_NormalGun>();
		}
		...
		{
			(gunFSM.CurrentState as FK3_GunState<FK3_GunController>)?.RotateByInput();
		}
		...
		public void Shoot()
		{
			(gunFSM.CurrentState as FK3_GunState<FK3_GunController>)?.Shoot();
		}
		...
		{
			if (gunFSM.IsInState<FK3_NormalGun>())
			{
				gunFSM.ChangeState<FK3_LockingGun>();
			}
			else if (gunFSM.IsInState<FK3_LockingGun>())
			{
				gunFSM.ChangeState<FK3_NormalGun>();
			}
		}
		...
		{
			if ((gunFSM.CurrentState as FK3_EMGun).phase == FK3_EMGun.EMGunPhase.Phase2)
			{
