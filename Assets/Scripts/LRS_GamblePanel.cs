...
public class LRS_GamblePanel : MonoBehaviour
{
	...
	public static LRS_GamblePanel ins;
	...
	{
		LRS_MB_Singleton<LRS_DiceGameController2>.GetInstance().Send_MultipleInfo(LRS_BetDiceType.Half);
		SetButton(canclick: false);
	}

	private void ClickBlack()
	{
		LRS_MB_Singleton<LRS_DiceGameController2>.GetInstance().Send_MultipleInfo(LRS_BetDiceType.Risk);
		SetButton(canclick: false);
	}
	...
	private Sprite Getsprite()
	{
		try
		{
			uint num = Getnum(LRS_MB_Singleton<LRS_DiceGameController2>.GetInstance().Gamelewinid);
	...
	{
		balancescore = LRS_GVars.winScorce;
		doublescore = LRS_GVars.winScorce * 2;
	...
	{
		LRS_MB_Singleton<LRS_DiceGameController2>.GetInstance().OnBtnReady_Click();
		LRS_MB_Singleton<LRS_DiceGameController2>.GetInstance().Hide();
		int credit = (!isGamble) ? (LRS_GVars.winScorce + LRS_GVars.credit) : AllScroce;
		LRS_MB_Singleton<LRS_MajorGameController>.GetInstance().SetCredit(credit);
