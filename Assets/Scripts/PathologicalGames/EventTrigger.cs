...
namespace PathologicalGames
{
	...
	public class EventTrigger : AreaTargetTracker
	{
		...
		protected void OnTriggered(GameObject other)
		{
			...
			case HIT_MODES.HitLayers:
				...
				if (!areaHit)
				{
					...
					if (component != null)
					{
						target = new Target(component, this);
					}
				}
				StartCoroutine(Fire());
				break;
		...
		protected bool OnNewTargetHandler(TargetTracker targetTracker, Target target)
		{
			...
			Target item = new Target(target.targetable, this);
		...
		public virtual IEnumerator Fire()
		{
			...
			foreach (Target item in detectedTargetsCache)
			{
			...
			if (base.gameObject.activeInHierarchy)
			{
				InstanceManager.Despawn(poolName, base.transform);
				yield return null;
			}
		}
		...
		protected void SpawnOnFirePrefab(bool passInfo)
		{
			if (!(eventTriggerPrefab == null))
			{
				...
				Transform transform = InstanceManager.Spawn(text, eventTriggerPrefab.transform, base.transform.position, base.transform.rotation);
