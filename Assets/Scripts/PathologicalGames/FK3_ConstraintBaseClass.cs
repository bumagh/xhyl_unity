...
{
	[AddComponentMenu("")]
	public class FK3_ConstraintBaseClass : FK3_ConstraintFrameworkBaseClass
	{
		public Transform _target;

		protected Transform _internalTarget;

		public FK3_UnityConstraints.NO_TARGET_OPTIONS _noTargetMode = FK3_UnityConstraints.NO_TARGET_OPTIONS.DoNothing;

		public FK3_UnityConstraints.MODE_OPTIONS _mode = FK3_UnityConstraints.MODE_OPTIONS.Constrain;
		...
		public Transform target
		{
			get
			{
				if (noTargetMode == FK3_UnityConstraints.NO_TARGET_OPTIONS.SetByScript)
				{
		...
		public FK3_UnityConstraints.NO_TARGET_OPTIONS noTargetMode
		{
			get
			{
		...
		public FK3_UnityConstraints.MODE_OPTIONS mode
		{
			get
			{
				return _mode;
		...
		protected sealed override void InitConstraint()
		{
			switch (mode)
			{
			case FK3_UnityConstraints.MODE_OPTIONS.Constrain:
				StartCoroutine("Constrain");
				break;
			case FK3_UnityConstraints.MODE_OPTIONS.Align:
				OnOnce();
				break;
			}
		}
		...
		{
			while (mode == FK3_UnityConstraints.MODE_OPTIONS.Constrain)
			{
				if (target == null)
				{
					switch (noTargetMode)
					{
					case FK3_UnityConstraints.NO_TARGET_OPTIONS.Error:
					{
					...
					case FK3_UnityConstraints.NO_TARGET_OPTIONS.DoNothing:
						yield return null;
						continue;
					case FK3_UnityConstraints.NO_TARGET_OPTIONS.ReturnToDefault:
						NoTargetDefault();
						yield return null;
						continue;
					}
