...
namespace PathologicalGames
{
	...
	public class FK3_PrefabPool
	{
		...
		public FK3_SpawnPool spawnPool;
		...
		public FK3_PrefabPool(Transform prefab)
		{
			this.prefab = prefab;
			prefabGO = prefab.gameObject;
		}

		public FK3_PrefabPool()
		{
		}
		...
		internal bool DespawnInstance(Transform xform, bool sendEventMessage)
		{
			if (logMessages)
			{
				UnityEngine.Debug.Log($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): Despawning '{xform.name}'");
			}
		...
		internal IEnumerator CullDespawned()
		{
			if (logMessages)
			{
				UnityEngine.Debug.Log($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): CULLING TRIGGERED! Waiting {cullDelay}sec to begin checking for despawns...");
			}
			...
			while (totalCount > cullAbove)
			{
				for (int i = 0; i < cullMaxPerPass; i++)
				{
					...
					if (_despawned.Count > 0)
					{
						...
						if (logMessages)
						{
							UnityEngine.Debug.Log($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): CULLING to {cullAbove} instances. Now at {totalCount}.");
						}
					}
					else if (logMessages)
					{
						UnityEngine.Debug.Log($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): CULLING waiting for despawn. Checking again in {cullDelay}sec");
						break;
					}
				}
				yield return new WaitForSeconds(cullDelay);
			...
			if (logMessages)
			{
				UnityEngine.Debug.Log($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): CULLING FINISHED! Stopping");
			}
			cullingActive = false;
			yield return null;
		}
		...
		internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
		{
			if (limitInstances && limitFIFO && _spawned.Count >= limitAmount)
			{
				...
				if (logMessages)
				{
					UnityEngine.Debug.Log($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): LIMIT REACHED! FIFO=True. Calling despawning for {transform}...");
				}
			...
			else
			{
				...
				if (logMessages)
				{
					UnityEngine.Debug.Log($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): respawning '{transform2.name}'.");
				}
		...
		public Transform SpawnNew(Vector3 pos, Quaternion rot)
		{
			if (limitInstances && totalCount >= limitAmount)
			{
				if (logMessages)
				{
					UnityEngine.Debug.Log($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): LIMIT REACHED! Not creating new instances! (Returning null)");
				}
				return null;
			}
			if (pos == Vector3.zero)
			...
			if (logMessages)
			{
				UnityEngine.Debug.Log($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): Spawned new instance '{transform.name}'.");
			}
			return transform;
		}
		...
		internal void PreloadInstances()
		{
			if (preloaded)
			{
				UnityEngine.Debug.Log($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): Already preloaded! You cannot preload twice. If you are running this through code, make sure it isn't also defined in the Inspector.");
				return;
			}
			preloaded = true;
			if (prefab == null)
			{
				UnityEngine.Debug.LogError($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): Prefab cannot be null.");
				return;
			}
			if (limitInstances && preloadAmount > limitAmount)
			{
				UnityEngine.Debug.LogWarning($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): You turned ON 'Limit Instances' and entered a 'Limit Amount' greater than the 'Preload Amount'! Setting preload amount to limit amount.");
				preloadAmount = limitAmount;
			}
			if (cullDespawned && preloadAmount > cullAbove)
			{
				UnityEngine.Debug.LogWarning($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): You turned ON Culling and entered a 'Cull Above' threshold greater than the 'Preload Amount'! This will cause the culling feature to trigger immediatly, which is wrong conceptually. Only use culling for extreme situations. See the docs.");
			}
			if (preloadTime)
			{
				if (preloadFrames > preloadAmount)
				{
					UnityEngine.Debug.LogWarning($"FK3_SpawnPool {spawnPool.poolName} ({prefab.name}): Preloading over-time is on but the frame duration is greater than the number of instances to preload. The minimum spawned per frame is 1, so the maximum time is the same as the number of instances. Changing the preloadFrames value...");
		...
		public bool Contains(Transform transform)
		{
			if (prefabGO == null)
			{
				UnityEngine.Debug.LogError($"FK3_SpawnPool {spawnPool.poolName}: FK3_PrefabPool.prefabGO is null");
			}
			return spawned.Contains(transform) || despawned.Contains(transform);
		}
