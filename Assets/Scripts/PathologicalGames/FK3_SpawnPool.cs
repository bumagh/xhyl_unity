...
{
	[AddComponentMenu("Path-o-logical/FK3_PoolManager/FK3_SpawnPool")]
	public sealed class FK3_SpawnPool : MonoBehaviour, IList<Transform>, IEnumerable, ICollection<Transform>, IEnumerable<Transform>
	{
		...
		public List<FK3_PrefabPool> _perPrefabPoolOptions = new List<FK3_PrefabPool>();

		public Dictionary<object, bool> prefabsFoldOutStates = new Dictionary<object, bool>();

		public float maxParticleDespawnTime = 300f;

		public FK3_PrefabsDict prefabs = new FK3_PrefabsDict();

		public Dictionary<object, bool> _editorListItemStates = new Dictionary<object, bool>();

		private List<FK3_PrefabPool> _prefabPools = new List<FK3_PrefabPool>();
		...
		public bool dontDestroyOnLoad
		{
			...
			set
			{
				...
				if (group != null)
				{
					...
					FK3_GVars.dontDestroyOnLoadList.Add(group.gameObject);
				}
			}
		}
		...
		public Dictionary<string, FK3_PrefabPool> prefabPools
		{
			get
			{
				Dictionary<string, FK3_PrefabPool> dictionary = new Dictionary<string, FK3_PrefabPool>();
		...
		private void Awake()
		{
			...
			if (logMessages)
			{
				UnityEngine.Debug.Log($"FK3_SpawnPool {poolName}: Initializing..");
			}
			for (int i = 0; i < _perPrefabPoolOptions.Count; i++)
			{
				if (_perPrefabPoolOptions[i].prefab == null)
				{
					UnityEngine.Debug.LogWarning($"Initialization Warning: Pool '{poolName}' contains a FK3_PrefabPool with no prefab reference. Skipping.");
					continue;
				}
				_perPrefabPoolOptions[i].inspectorInstanceConstructor();
				CreatePrefabPool(_perPrefabPoolOptions[i]);
			}
			FK3_PoolManager.Pools.Add(this);
		}
		...
		internal GameObject InstantiatePrefab(GameObject prefab, Vector3 pos, Quaternion rot)
		{
			...
			return FK3_InstanceHandler.InstantiatePrefab(prefab, pos, rot);
		}
		...
		internal void DestroyInstance(GameObject instance)
		{
			...
			{
				FK3_InstanceHandler.DestroyInstance(instance);
			}
		}
		...
		private void OnDestroy()
		{
			if (logMessages)
			{
				UnityEngine.Debug.Log($"FK3_SpawnPool {poolName}: Destroying...");
			}
			FK3_PoolManager.Pools.Remove(this);
			StopAllCoroutines();
			_spawned.Clear();
			foreach (FK3_PrefabPool prefabPool in _prefabPools)
			{
		...
		public void CreatePrefabPool(FK3_PrefabPool prefabPool)
		{
			...
			if (!prefabPool.preloaded)
			{
				if (logMessages)
				{
					UnityEngine.Debug.Log($"FK3_SpawnPool {poolName}: Preloading {prefabPool.preloadAmount} {prefabPool.prefab.name}");
				}
		...
		public void Add(Transform instance, string prefabName, bool despawn, bool parent)
		{
			for (int i = 0; i < _prefabPools.Count; i++)
			{
				...
				{
					UnityEngine.Debug.LogError("Unexpected Error: FK3_PrefabPool.prefabGO is null");
					return;
				}
				if (_prefabPools[i].prefabGO.name == prefabName)
				{
					...
					if (logMessages)
					{
						UnityEngine.Debug.Log($"FK3_SpawnPool {poolName}: Adding previously unpooled instance {instance.name}");
					}
					if (parent)
					{
			...
			UnityEngine.Debug.LogError($"FK3_SpawnPool {poolName}: FK3_PrefabPool {prefabName} not found.");
		}

		public void Add(Transform item)
		{
			string message = "Use FK3_SpawnPool.Spawn() to properly add items to the pool.";
		...
		public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot, Transform parent)
		{
			...
			FK3_PrefabPool fK3_PrefabPool = new FK3_PrefabPool(prefab);
			CreatePrefabPool(fK3_PrefabPool);
			transform = fK3_PrefabPool.SpawnInstance(pos, rot);
			if (parent != null)
			{
				transform.SetParent(parent);
			}
		...
		public void Despawn(Transform instance)
		{
			...
			for (int i = 0; i < _prefabPools.Count; i++)
			{
				...
				{
					UnityEngine.Debug.LogError($"FK3_SpawnPool {poolName}: {instance.name} has already been despawned. You cannot despawn something more than once!");
					return;
				}
			}
			if (!flag)
			{
				UnityEngine.Debug.LogError($"FK3_SpawnPool {poolName}: {instance.name} not found in FK3_SpawnPool");
			}
			else
			{
		...
		public FK3_PrefabPool GetPrefabPool(Transform prefab)
		{
			for (int i = 0; i < _prefabPools.Count; i++)
			{
				if (_prefabPools[i].prefabGO == null)
				{
					UnityEngine.Debug.LogError($"FK3_SpawnPool {poolName}: FK3_PrefabPool.prefabGO is null");
				}
				if (_prefabPools[i].prefabGO == prefab.gameObject)
				{
		...
		public FK3_PrefabPool GetPrefabPool(GameObject prefab)
		{
			for (int i = 0; i < _prefabPools.Count; i++)
			{
				if (_prefabPools[i].prefabGO == null)
		...
		private IEnumerator ListenForEmitDespawn(ParticleSystem emitter)
		{
			...
			while (emitter.IsAlive(withChildren: true) && emitterGO.activeInHierarchy)
			{
				...
				{
					UnityEngine.Debug.LogWarning($"FK3_SpawnPool {poolName}: Timed out while listening for all particles to die. Waited for {maxParticleDespawnTime}sec.");
				}
				yield return null;
			}
