...
public class ESP_LoadingViewController : ESP_MB_Singleton<ESP_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (ESP_MB_Singleton<ESP_LoadingViewController>._instance == null)
		{
			ESP_MB_Singleton<ESP_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (ESP_MB_Singleton<ESP_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public void Show()
	{
		...
		ESP_MySqlConnection.curView = "LoadingView";
	}
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!ESP_MB_Singleton<ESP_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
	...
	{
		percent += ESP_MySqlConnection.RandomTime;
		if (percent >= 1f)
		{
			percent = 1f;
		}
