...
public class USW_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {USW_LogHelper.Key(btnKey)} value {USW_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {USW_LogHelper.Key(btnKey)} value {USW_LogHelper.Key(GetValue(btnKey))}");
			}
