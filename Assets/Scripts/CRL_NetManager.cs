...
public class CRL_NetManager : CRL_MB_Singleton<CRL_NetManager>
{
	public delegate void NetMessageHandler(object[] args);

	private CRL_ClientSocket _pClient;

	private CRL_DataEncrypt _pCrypto;
	...
	private Queue<CRL_NetMessage> _netMsgQueue = new Queue<CRL_NetMessage>();

	private Dictionary<string, NetMessageHandler> _netMsgHandlerDic = new Dictionary<string, NetMessageHandler>();

	private void Awake()
	{
		if (CRL_MB_Singleton<CRL_NetManager>._instance == null)
		{
			CRL_MB_Singleton<CRL_NetManager>.SetInstance(this);
		}
	...
	private void _initClientSocket()
	{
		...
		_pClient = new CRL_ClientSocket(useTcpClient: true);
		_pCrypto = new CRL_DataEncrypt();
	...
	private void _handleException(Exception ex)
	{
		...
		isReady = false;
		isLogined = false;
		Queue<CRL_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
			_netMsgQueue.Enqueue(new CRL_NetMessage("netDown", new object[1]
			{
				ex
			}));
		}
	...
	private void _handleConnected()
	{
		try
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new CRL_NetMessage("netConnected", new object[0]));
			}
		}
		catch (Exception ex)
		{
	...
	{
		UnityEngine.Debug.Log("ZH2_GVars.versionCode: " + CRL_MySqlConnection.versionCode);
		Send("userService/checkVersion", new object[1]
		{
			CRL_MySqlConnection.versionCode
		});
	}
	...
	protected void _handleData(byte[] msgBuffer, int packetId, int endIndex)
	{
		...
		lock (netMsgQueue)
		{
			CRL_NetMessage cRL_NetMessage = new CRL_NetMessage(text, args);
			cRL_NetMessage.jsonString = @string;
			cRL_NetMessage.packetId = packetId;
			cRL_NetMessage.endIndex = endIndex;
			_netMsgQueue.Enqueue(cRL_NetMessage);
		}
	}

	private IEnumerator _checkTimeout(int id, int timeout, CRL_ClientSocket cs)
	{
		UnityEngine.Debug.Log(CRL_LogHelper.Aqua("_checkTimeout start id: {0}", id));
		...
		if (cs != null && cs.doneList.Count > id && !cs.doneList[id])
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new CRL_NetMessage("timeout", new object[1]
				{
					id
				}));
				isTimeout = true;
		...
		if (isTimeout)
		{
			UnityEngine.Debug.Log(CRL_LogHelper.Aqua("_checkTimeout start id: {0} timeout: {1}", id, isTimeout));
		}
	}
	...
	private IEnumerator _checkHeartTimeout(int timeout)
	{
		...
		if (isReady && !isHeartPong)
		{
			...
			lock (netMsgQueue)
			{
				...
				isReady = false;
				isLogined = false;
				_netMsgQueue.Enqueue(new CRL_NetMessage("heart timeout", new object[0]));
				isTimeout = true;
			}
		}
		if (isTimeout)
		{
			UnityEngine.Debug.Log(CRL_LogHelper.Aqua("_checkHeartTimeout is timeout"));
		}
	}

	public void _checkTimeoutStart(int id, int timeout, CRL_ClientSocket cs)
	{
	...
	public void Send(string method, object[] args)
	{
		...
		{
			UnityEngine.Debug.Log(CRL_LogHelper.Magenta("drop send>>method: {0}", method));
			return;
		}
		Hashtable hashtable = new Hashtable();
		hashtable.Add("method", method);
		hashtable.Add("args", args);
		hashtable.Add("version", CRL_MySqlConnection.Version);
		if (isReady)
		{
	...
	private bool GetNetMessage(out CRL_NetMessage netMsg)
	{
		Queue<CRL_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
	...
	private void DispatchNetMessage(CRL_NetMessage netMsg)
	{
		if (dropDispatch)
		{
			UnityEngine.Debug.Log(CRL_LogHelper.Magenta("drop send>>method: {0}", netMsg.method));
		}
