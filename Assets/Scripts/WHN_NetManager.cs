...
public class WHN_NetManager : WHN_MB_Singleton<WHN_NetManager>
{
	public delegate void NetMessageHandler(object[] args);

	private WHN_ClientSocket _pClient;

	private WHN_DataEncrypt _pCrypto;
	...
	private Queue<WHN_NetMessage> _netMsgQueue = new Queue<WHN_NetMessage>();

	private Dictionary<string, NetMessageHandler> _netMsgHandlerDic = new Dictionary<string, NetMessageHandler>();

	private void Awake()
	{
		if (WHN_MB_Singleton<WHN_NetManager>._instance == null)
		{
			WHN_MB_Singleton<WHN_NetManager>.SetInstance(this);
		}
	...
	private void _initClientSocket()
	{
		...
		_pClient = new WHN_ClientSocket(useTcpClient: true);
		_pCrypto = new WHN_DataEncrypt();
	...
	private void _handleException(Exception ex)
	{
		...
		isReady = false;
		isLogined = false;
		Queue<WHN_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
			_netMsgQueue.Enqueue(new WHN_NetMessage("netDown", new object[1]
			{
				ex
			}));
		}
	...
	private void _handleConnected()
	{
		try
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new WHN_NetMessage("netConnected", new object[0]));
			}
		}
		catch (Exception ex)
		{
	...
	{
		UnityEngine.Debug.Log("ZH2_GVars.versionCode: " + WHN_GVars.versionCode);
		Send("userService/checkVersion", new object[1]
		{
			WHN_GVars.versionCode
		});
	}
	...
	protected void _handleData(byte[] msgBuffer, int packetId, int endIndex)
	{
		...
		lock (netMsgQueue)
		{
			WHN_NetMessage wHN_NetMessage = new WHN_NetMessage(text, args);
			wHN_NetMessage.jsonString = @string;
			wHN_NetMessage.packetId = packetId;
			wHN_NetMessage.endIndex = endIndex;
			_netMsgQueue.Enqueue(wHN_NetMessage);
		}
	}

	private IEnumerator _checkTimeout(int id, int timeout, WHN_ClientSocket cs)
	{
		UnityEngine.Debug.Log(WHN_LogHelper.Aqua("_checkTimeout start id: {0}", id));
		...
		if (cs != null && cs.doneList.Count > id && !cs.doneList[id])
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new WHN_NetMessage("timeout", new object[1]
				{
					id
				}));
				isTimeout = true;
		...
		if (isTimeout)
		{
			UnityEngine.Debug.Log(WHN_LogHelper.Aqua("_checkTimeout start id: {0} timeout: {1}", id, isTimeout));
		}
	}
	...
	private IEnumerator _checkHeartTimeout(int timeout)
	{
		...
		if (isReady && !isHeartPong)
		{
			...
			lock (netMsgQueue)
			{
				...
				isReady = false;
				isLogined = false;
				_netMsgQueue.Enqueue(new WHN_NetMessage("heart timeout", new object[0]));
				isTimeout = true;
			}
		}
		if (isTimeout)
		{
			UnityEngine.Debug.Log(WHN_LogHelper.Aqua("_checkHeartTimeout is timeout"));
		}
	}

	public void _checkTimeoutStart(int id, int timeout, WHN_ClientSocket cs)
	{
	...
	public void Send(string method, object[] args)
	{
		...
		{
			UnityEngine.Debug.Log(WHN_LogHelper.Magenta("drop send>>method: {0}", method));
			return;
		}
		Hashtable hashtable = new Hashtable();
		hashtable.Add("method", method);
		hashtable.Add("args", args);
		hashtable.Add("version", WHN_GVars.Version);
		if (isReady)
		{
	...
	private bool GetNetMessage(out WHN_NetMessage netMsg)
	{
		Queue<WHN_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
	...
	private void DispatchNetMessage(WHN_NetMessage netMsg)
	{
		if (dropDispatch)
		{
			UnityEngine.Debug.Log(WHN_LogHelper.Magenta("drop send>>method: {0}", netMsg.method));
		}
