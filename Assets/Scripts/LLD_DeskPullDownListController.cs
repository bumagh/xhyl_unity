...
public class LLD_DeskPullDownListController : MonoBehaviour
{
	public static LLD_DeskPullDownListController Instance;
	...
	public void OnListButtonDown()
	{
		...
		imgBtns[LLD_MB_Singleton<LLD_LobbyViewController>.GetInstance().curDeskIndex].sprite = arrImgButton[1];
	...
	public void ChangeListHeightByDeskNum(int desknum, List<LLD_Desk> desklist)
	{
	...
	public void UpdateDeskInList(List<LLD_Desk> desks)
	{
	...
	public void SelectDeskByButton(int index)
	{
		if (bMoveFinished)
		{
			if (index > LLD_MB_Singleton<LLD_LobbyViewController>.GetInstance().curDeskIndex)
			{
				LLD_MB_Singleton<LLD_LobbyViewController>.GetInstance().ForceMoveDesksToRight(index);
			}
			else if (index < LLD_MB_Singleton<LLD_LobbyViewController>.GetInstance().curDeskIndex)
			{
				LLD_MB_Singleton<LLD_LobbyViewController>.GetInstance().ForceMoveDesksToLeft(index);
			}
