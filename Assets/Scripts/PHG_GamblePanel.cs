...
public class PHG_GamblePanel : MonoBehaviour
{
	...
	public static PHG_GamblePanel ins;
	...
	{
		PHG_MB_Singleton<PHG_DiceGameController2>.GetInstance().Send_MultipleInfo(PHG_BetDiceType.Half);
		SetButton(canclick: false);
	}

	private void ClickBlack()
	{
		PHG_MB_Singleton<PHG_DiceGameController2>.GetInstance().Send_MultipleInfo(PHG_BetDiceType.Risk);
		SetButton(canclick: false);
	}
	...
	private Sprite Getsprite()
	{
		try
		{
			uint num = Getnum(PHG_MB_Singleton<PHG_DiceGameController2>.GetInstance().Gamelewinid);
	...
	{
		balancescore = PHG_GVars.winScorce;
		doublescore = PHG_GVars.winScorce * 2;
	...
	{
		PHG_MB_Singleton<PHG_DiceGameController2>.GetInstance().OnBtnReady_Click();
		PHG_MB_Singleton<PHG_DiceGameController2>.GetInstance().Hide();
		int credit = (!isGamble) ? (PHG_GVars.winScorce + PHG_GVars.credit) : AllScroce;
		PHG_MB_Singleton<PHG_MajorGameController>.GetInstance().SetCredit(credit);
