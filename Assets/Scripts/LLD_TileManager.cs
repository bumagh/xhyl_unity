...
public class LLD_TileManager : MonoBehaviour
{
	...
	public IEnumerator StartChasing(int targetIndex, Action endAction)
	{
		...
		LLD_XWait xwait = new LLD_XWait(this).Begin();
		int count = 0;
		float timeCount = 0f;
	...
	public IEnumerator WaitSecondsAction(float seconds, Action<int> action, int param1, LLD_XWait xwait = null)
	{
		if (xwait == null)
		{
			yield return new WaitForSeconds(seconds);
