...
public class SHT_SymbolC : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		symlen = Cinstance<SHT_Gcore>.Instance.Symcont;
	...
	public void Init(int idx, int symid, SHT_Line line = null)
	{
		symidx = idx;
		lineroll = line;
	...
	{
		iconimg.sprite = Cinstance<SHT_Gcore>.Instance.SymSprlist[idx];
	}
	...
	public void StopRoll()
	{
		...
		if (isroll)
		{
			...
			if (Inlineidx > 2)
			{
				...
				int idx = Cinstance<SHT_Gcore>.Instance.Result[index];
				SetSymbolimg(idx, 0L);
			}
		}
		SHT_SoundManager.Instance.PlayStopAndio();
		speed = normalspeed;
	}
	...
	private void Setrollnum(List<int> randomlist, int rolllen)
	{
		...
		if (Inlineidx > 2)
		{
			...
			int value = Cinstance<SHT_Gcore>.Instance.Result[index2];
	...
	public void Playani(int winline = -1)
	{
		...
		try
		{
			Seq_ani = iconimg.SHT_PlayAni(string.Format(aniname, (SymbolId + 1).ToString("00")) + "{0:0}", string.Format(aniname, (SymbolId + 1).ToString("00")) + "{0:0}", 2, frames);
			Seq_ani.Restart();
		}
		catch (Exception arg)
		{
