...
public class CSF_LobbyViewController : CSF_MB_Singleton<CSF_LobbyViewController>
{
	...
	private CSF_HeadViewController _headViewController;

	private CSF_UserViewer _userViewer;
	...
	private List<CSF_Desk> _deskList = new List<CSF_Desk>();
	...
	private CSF_DeskWidget[] _deskWidgets = new CSF_DeskWidget[3];
	...
	private void Awake()
	{
		...
		if (CSF_MB_Singleton<CSF_LobbyViewController>._instance == null)
		{
			CSF_MB_Singleton<CSF_LobbyViewController>.SetInstance(this);
			PreInit();
		}
	}
	...
	private void InitFinGame()
	{
		...
		_headViewController = base.transform.Find("Title").GetComponent<CSF_HeadViewController>();
		_userViewer = base.transform.Find("PersonInfoDialog").GetComponent<CSF_UserViewer>();
	...
	private void Update()
	{
		if (UnityEngine.Input.GetKeyDown(KeyCode.Escape) && !CSF_MB_Singleton<CSF_LobbyViewController>.GetInstance()._IsAning && !CSF_LockManager.IsLocked("Esc"))
		{
			UnityEngine.Debug.Log("ESC Btn Down In Lobby View");
			CSF_MB_Singleton<CSF_GameManager>.GetInstance().Handle_BtnReturn();
		}
	}
	...
	private void _init()
	{
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().RegisterHandler("roomInfo", HandleNetMsg_RoomInfo);
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().RegisterHandler("enterDesk", HandleNetMsg_EnterDesk);
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().RegisterHandler("updateRoomInfo", HandleNetMsg_UpdateRoomInfo);
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().RegisterHandler("addExpeGoldAuto", HandleNetMsg_AddExpeGoldAuto);
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().RegisterHandler("expeGold", HandleNetMsg_ExpeGold);
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().RegisterHandler("playerInfo", HandleNetMsg_PlayerInfo);
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().RegisterHandler("updateDeskInfo", HandleNetMsg_UpdateDeskInfo);
		...
		CSF_Drag_DeskController._instance.left = OnBtnLeftDesk_Click;
		CSF_Drag_DeskController._instance.right = OnBtnRightDesk_Click;
	...
	public void Show()
	{
		...
		CSF_MB_Singleton<CSF_GameManager>.GetInstance().ChangeView(empty);
	...
	{
		if (!CSF_MySqlConnection.tryLockOnePoint && CSF_DeskPullDownListController.Instance.bMoveFinished)
		{
			_desksMoveToLeft();
		}
	}
	...
	{
		if (!CSF_MySqlConnection.tryLockOnePoint && _bCanClick)
		{
			_bCanClick = false;
			CSF_SoundManager.Instance.PlayClickAudio();
	...
	public void Send_EnterRoom(int roomType)
	{
		...
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().Send("userService/enterRoom", args);
		CSF_MB_Singleton<CSF_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "Send_EnterRoom");
	}
	...
	public void Send_EnterDesk(int deskId)
	{
		...
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().Send("userService/enterDesk", args);
		CSF_MB_Singleton<CSF_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "Send_EnterDesk");
	}
	...
	public void Send_LeaveRoom()
	{
		...
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().Send("userService/leaveRoom", args);
	}

	public void Send_GetUserAward()
	{
		object[] args = new object[0];
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().Send("userService/getUserAward", args);
	}

	public void Send_AddExpeGoldAuto()
	{
		object[] args = new object[0];
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().Send("userService/addExpeGoldAuto", args);
	}
	...
	public void Send_PlayerInfo(int userId)
	{
		...
		CSF_MB_Singleton<CSF_NetManager>.GetInstance().Send("userService/playerInfo", args);
	}

	public void HandleNetMsg_RoomInfo(object[] args)
	{
		UnityEngine.Debug.Log(CSF_LogHelper.NetHandle("HandleNetMsg_RoomInfo"));
		if (CSF_MySqlConnection.curView == "MajorGame" || CSF_MySqlConnection.curView == "DiceGame")
		{
			UnityEngine.Debug.Log(CSF_LogHelper.Orange("游戏中，不处理"));
			return;
		}
		_curState = "DeskView";
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
		CSF_MB_Singleton<CSF_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "Send_EnterRoom done");
		if (dictionary != null)
		{
			CSF_LockManager.UnLock("btn_room");
			UnityEngine.Debug.Log("isOverflow: " + dictionary["isOverflow"]);
			CSF_MB_Singleton<CSF_AlertDialog>.GetInstance().ShowDialog((CSF_MySqlConnection.language == "zh") ? "您的账户已爆机，请兑奖" : "Your account is blasting, please exchange");
			return;
		}
		object[] array = args[0] as object[];
		List<CSF_Desk> list = new List<CSF_Desk>();
		for (int i = 0; i < array.Length; i++)
		{
			Dictionary<string, object> data = array[i] as Dictionary<string, object>;
			list.Add(CSF_Desk.CreateWithDic(data));
		}
		_deskList = list;
	...
	{
		UnityEngine.Debug.Log(CSF_LogHelper.NetHandle("HandleNetMsg_EnterDesk"));
		CSF_LockManager.UnLock("EnterDesk");
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
		bool flag = (bool)dictionary["success"];
		CSF_MB_Singleton<CSF_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "Send_EnterDesk done");
		if (flag)
		{
			Hide();
			CSF_MySqlConnection.desk = _deskList[curDeskIndex];
			CSF_MB_Singleton<CSF_MajorGameController>.GetInstance().Show();
			CSF_MB_Singleton<CSF_MajorGameController>.GetInstance().InitGame();
			CSF_MB_Singleton<CSF_ScoreBank>.GetInstance().SetKeepScore(0);
			CSF_MB_Singleton<CSF_ScoreBank>.GetInstance().SetRate(CSF_MySqlConnection.desk.exchange);
			return;
		}
		switch ((int)dictionary["messageStatus"])
		{
		case 0:
			CSF_MB_Singleton<CSF_AlertDialog>.GetInstance().ShowDialog((CSF_MySqlConnection.language == "zh") ? "桌子不存在" : "Game table not exist");
			break;
		case 1:
			CSF_MB_Singleton<CSF_AlertDialog>.GetInstance().ShowDialog((CSF_MySqlConnection.language == "zh") ? "桌已满" : "Don't Have Free Table");
			break;
		case 2:
			CSF_MB_Singleton<CSF_AlertDialog>.GetInstance().ShowDialog((CSF_MySqlConnection.language == "zh") ? "您的账户已爆机，请兑奖" : "Your account is blasting, please excharge");
			break;
		case 3:
			CSF_MB_Singleton<CSF_AlertDialog>.GetInstance().ShowDialog((CSF_MySqlConnection.language == "zh") ? "体验币不足" : "Experience Coins insufficient");
			break;
		case 4:
			CSF_MB_Singleton<CSF_AlertDialog>.GetInstance().ShowDialog((CSF_MySqlConnection.language == "zh") ? "游戏币不足" : "Coins insufficient");
			break;
		default:
	...
	{
		UnityEngine.Debug.Log(CSF_LogHelper.NetHandle("HandleNetMsg_UpdateRoomInfo"));
		...
		if (count != list.Count)
		{
			...
			CSF_DeskPullDownListController.Instance.UpdateDeskInList(_deskList);
		}
		else
		{
			UpdateAllDeskWidgets();
	...
	public void HandleNetMsg_AddExpeGoldAuto(object[] args)
	{
		...
		CSF_MB_Singleton<CSF_AlertDialog>.GetInstance().ShowDialog("申请成功！请返回大厅重新游戏", showOkCancel: false, delegate
		{
	...
	{
		UnityEngine.Debug.Log(CSF_LogHelper.NetHandle("HandleNetMsg_ExpeGold"));
		if ((bool)args[0])
		{
			CSF_MySqlConnection.user.expeGold = 10000;
	...
	{
		UnityEngine.Debug.Log(CSF_LogHelper.NetHandle("HandleNetMsg_PlayerInfo"));
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
	...
	{
		UnityEngine.Debug.Log(CSF_LogHelper.NetHandle("HandleNetMsg_UpdateDeskInfo"));
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
	...
	private IEnumerator _showDeskViewAni()
	{
		...
		CSF_MySqlConnection.curView = "DeskSelectionView";
		CSF_LockManager.UnLock("btn_room");
		yield return null;
	}

	private CSF_DeskWidget _createDeskWidget()
	{
		...
		CSF_DeskWidget component = gameObject.GetComponent<CSF_DeskWidget>();
	...
	{
		CSF_DeskPullDownListController.Instance.bMoveFinished = false;
		CSF_SoundManager.Instance.PlayClickAudio();
		if (_deskList.Count != 0 && !_inputForbidden)
		{
			CSF_DeskPullDownListController.Instance.ShowListButtonHighBG(_bIsLeft: true, curDeskIndex);
			...
			sequence.onComplete = delegate
			{
				...
				CSF_DeskPullDownListController.Instance.bMoveFinished = true;
				CSF_DeskPointsController._Instance.ShowDeskPoint2(_deskList.Count, curDeskIndex);
			};
		}
	}
	...
	private void _desksMoveToRight()
	{
		...
		if (_deskList.Count != 0 && !_inputForbidden)
		{
			CSF_DeskPullDownListController.Instance.ShowListButtonHighBG(_bIsLeft: false, curDeskIndex);
	...
	{
		CSF_DeskWidget cSF_DeskWidget = _deskWidgets[a];
		_deskWidgets[a] = _deskWidgets[b];
		_deskWidgets[b] = cSF_DeskWidget;
	}
	...
	public void UpdateAllDeskWidgets()
	{
		...
		CSF_DeskPointsController._Instance.ShowDeskPoint2(_deskList.Count, curDeskIndex);
		CSF_DeskPullDownListController.Instance.ChangeListHeightByDeskNum(_deskList.Count, _deskList);
	}
	...
	private void _onBtnDesk_Click()
	{
		...
		{
			if (CSF_MySqlConnection.user.expeGold == 0 || CSF_MySqlConnection.user.expeGold < _deskList[curDeskIndex].minGold)
			{
				UnityEngine.Debug.Log($"expeGold: {CSF_MySqlConnection.user.expeGold}, _curDeskIndex: {curDeskIndex}, minGold: {_deskList[curDeskIndex].minGold}");
				CSF_MB_Singleton<CSF_AlertDialog>.GetInstance().ShowDialog("体验币不足，是否领取体验币？", showOkCancel: true, delegate
				{
			...
			{
				CSF_LockManager.Lock("EnterDesk");
				Send_EnterDesk(_deskList[curDeskIndex].id);
				CSF_SoundManager.Instance.StopLobbyBGM();
			}
		}
		else if (CSF_MySqlConnection.user.gameGold == 0 || CSF_MySqlConnection.user.gameGold < _deskList[curDeskIndex].minGold)
		{
			UnityEngine.Debug.LogError($"gameGold: {CSF_MySqlConnection.user.gameGold}, _curDeskIndex: {curDeskIndex}, minGold: {_deskList[curDeskIndex].minGold}");
			CSF_MB_Singleton<CSF_AlertDialog>.GetInstance().ShowDialog("游戏币不足！");
		}
		else
		{
			CSF_LockManager.Lock("EnterDesk");
	...
	{
		string info = (CSF_MySqlConnection.language == "zh") ? "选择房间" : "SelectRoom";
		if (_curState == "DeskView")
		{
			if (_curRoomType == 1)
			{
				info = ((CSF_MySqlConnection.language == "zh") ? "新手练习厅" : "Training");
			}
			else if (_curRoomType == 2)
			{
				info = ((CSF_MySqlConnection.language == "zh") ? "欢乐竞技厅" : "Arena");
			}
		}
	...
	{
		_IsAning = true;
		CSF_MB_Singleton<CSF_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "_hideRoomViewAni");
		Sequence sequence = DOTween.Sequence();
		...
		sequence.onComplete = delegate
		{
			CSF_MB_Singleton<CSF_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "_hideRoomViewAni done");
			_IsAning = false;
			_bCanClick = true;
		};
	}
	...
	public void ResetRoomView()
	{
		...
		CSF_MB_Singleton<CSF_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "ResetRoomView");
		_userViewer.gameObject.SetActive(value: false);
		Sequence sequence = DOTween.Sequence();
		...
		sequence.onComplete = delegate
		{
			...
			_bCanClick = false;
			_updateHeadInfo();
			CSF_MB_Singleton<CSF_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "ResetRoomView_Over");
		};
	}
	...
	public void HideDeskViewAni()
	{
		...
		sequence.onComplete = delegate
		{
			CSF_MB_Singleton<CSF_DeskSelectionViewController>.GetInstance().Hide();
		};
	}
}
