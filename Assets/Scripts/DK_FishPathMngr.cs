...
public class DK_FishPathMngr : MonoBehaviour
{
	public static DK_FishPathMngr G_FishPathMngr;

	private DK_PathManager[] allFishPath;

	private DK_PathManager[] smallFishPath;
	...
	public static DK_FishPathMngr GetSingleton()
	{
	...
	private void Awake()
	{
		...
		int num = 0;
		_AllPathLength = 88;
		allFishPath = new DK_PathManager[_AllPathLength];
		...
		for (int i = 0; i < _AllPathLength; i++)
		{
			...
			allFishPath[i] = new DK_PathManager();
		...
		smallFishPath = new DK_PathManager[_AllPathLength * 4];
	...
	public DK_PathManager GetFishPath(int type)
	{
		return allFishPath[type];
	}

	public DK_PathManager GetSmallFishPath(int type)
	{
		return smallFishPath[type];
	}

	private DK_PathManager ChangePath(DK_PathManager path, Vector3 vec)
	{
		int num = path.vecs.Length;
		DK_PathManager dK_PathManager = new DK_PathManager();
		dK_PathManager.vecs = new Vector3[num];
		for (int i = 0; i < num; i++)
		{
			dK_PathManager.vecs[i] = path.vecs[i] + vec;
		}
		return dK_PathManager;
	}

	public void GetAllPath(int type)
	{
		DK_PathManager dK_PathManager = smallFishPath[type];
		int num = dK_PathManager.vecs.Length;
		string text = string.Empty;
		for (int i = 0; i < num; i++)
		{
			string text2 = text;
			text = text2 + dK_PathManager.vecs[i][0].ToString() + "," + dK_PathManager.vecs[i][1].ToString() + "," + dK_PathManager.vecs[i][2].ToString() + " ";
		}
