...
public class LL_PrizeResult : MonoBehaviour
{
	...
	protected LL_HudManager mHudManager;
	...
	private void Start()
	{
		...
		mBet = GameObject.Find("BetPanel").GetComponent<LL_BetPanel>();
		...
		for (int i = 0; i < 8; i++)
		{
			if (i < strNickname.Length)
			{
				...
				if (strNickname[i].CompareTo(string.Empty) == 0)
				{
					...
					mUserItem[i].SetActiveRecursively(state: false);
				}
				else
				{
					mAllUserIcon[i].spriteName = "personIcon" + ((iIconId[i] <= 0) ? 1 : iIconId[i]);
					mUserItem[i].SetActiveRecursively(state: true);
				}
			}
			else
			{
	...
	public void ShowPrizeResult()
	{
		...
		LL_NetMngr.GetSingleton().MyCreateSocket.SendResultList(mUserSelf.TableId);
	...
	public int SetAnimalResult(int[] iAnimalID, int nLength = 1, int nJpPower = 0)
	{
		...
		{
			LL_ErrorManager.GetSingleton().AddError("Error:中奖结果错误length = " + nLength);
			return 1;
		}
		for (int i = 0; i < nLength; i++)
		{
			...
			{
				LL_ErrorManager.GetSingleton().AddError("Error:中奖动物错误:" + iAnimalID[i]);
				return 2;
			}
		}
		_resetResult();
	...
	public int SetLuckyJP(int iAnimalID, int iLuckyMachine, int iLuckyAnimalID, int nJPPower)
	{
		...
		{
			LL_ErrorManager.GetSingleton().AddError("Error:中奖动物错误" + iAnimalID);
			LL_ErrorManager.GetSingleton().AddError("Error:中奖动物错误" + iLuckyAnimalID);
			return 1;
		}
		if (iLuckyMachine > 8 || iLuckyMachine < 1)
		{
			LL_ErrorManager.GetSingleton().AddError("Error:中奖分机错误" + iLuckyMachine);
			return 2;
		}
		if (nJPPower > 5 || nJPPower < 2)
		{
			LL_ErrorManager.GetSingleton().AddError("Error:闪电倍率错误" + nJPPower);
			return 3;
		}
		_resetResult();
	...
	public int SetLuckyTimes(int iAnimalID, int iLuckyMachine, int[] iLuckyAnimalID, int nLength)
	{
		...
		{
			LL_ErrorManager.GetSingleton().AddError("Error:中奖动物错误" + iAnimalID);
			UnityEngine.Debug.Log("Error:中奖分机错误" + iLuckyMachine);
			return 2;
		}
		...
		for (int i = 0; i < nLength; i++)
		{
			...
			{
				LL_ErrorManager.GetSingleton().AddError("Error:中奖动物错误" + iLuckyAnimalID[i]);
				return 4;
			}
		}
		_resetResult();
