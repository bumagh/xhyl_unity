...
public class LRS_ReconnectHint : LRS_MB_Singleton<LRS_ReconnectHint>
{
	...
	private void Awake()
	{
		if (LRS_MB_Singleton<LRS_ReconnectHint>._instance == null)
		{
			LRS_MB_Singleton<LRS_ReconnectHint>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void Show()
	{
		if (!_goContainer.activeSelf)
		{
			...
			UnityEngine.Debug.Log(LRS_LogHelper.Magenta("ReconnectHint Show"));
	...
	public void Hide()
	{
		UnityEngine.Debug.Log(LRS_LogHelper.Magenta("ReconnectHint Hide"));
		LRS_LockManager.UnLock("Esc");
	...
	private IEnumerator _textInfoAni()
	{
		...
		while (true)
		{
			string prefix = (LRS_GVars.language == "zh") ? "网络断开，正在重连 " : "Reconnecting to the network ";
