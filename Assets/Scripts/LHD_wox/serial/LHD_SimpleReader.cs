...
{
	public class LHD_SimpleReader : ObjectReader
	{
		private Hashtable map;

		public LHD_SimpleReader()
		{
			map = new Hashtable();
		}
		...
		public object readPrimitiveArray(XmlReader xob, object id)
		{
			try
			{
				Type type = (Type)LHD_Serial.mapWOXToCSharp[xob.GetAttribute("elementType")];
		...
		public object readClassArray(Type[] a, string[] s)
		{
			...
			for (int i = 0; i < a.Length; i++)
			{
				...
				Type type = (Type)LHD_Serial.mapWOXToCSharp[s[i]];
				if (type == null)
				{
					type = (Type)LHD_Serial.mapArrayWOXToCSharp[s[i]];
					if (type == null)
					{
						try
						{
		...
		{
			Type type = (Type)LHD_Serial.mapWOXToCSharp[arrayTypeName];
			if (type != null)
			{
				return type;
			}
			type = (Type)LHD_Serial.mapArrayWOXToCSharp[arrayTypeName];
			if (type == null)
			{
		...
		public object readStringObject(XmlReader xob, object id)
		{
			try
			{
				Type type = (Type)LHD_Serial.mapWOXToCSharp[xob.GetAttribute("type")];
				if (type.Equals(typeof(Type)))
				{
					Type type2 = (Type)LHD_Serial.mapWOXToCSharp[xob.GetAttribute("value")];
					if (type2 == null)
					{
						type2 = (Type)LHD_Serial.mapArrayWOXToCSharp[xob.GetAttribute("value")];
						if (type2 == null)
						{
