...
public class PHG_GameRoot : PHG_MB_Singleton<PHG_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(PHG_MB_Singleton<PHG_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            PHG_MB_Singleton<PHG_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : PHG_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : PHG_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
