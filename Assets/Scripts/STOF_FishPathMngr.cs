...
public class STOF_FishPathMngr : MonoBehaviour
{
	public static STOF_FishPathMngr G_FishPathMngr;

	private STOF_PathManager[] allFishPath;

	private STOF_PathManager[] smallFishPath;
	...
	public static STOF_FishPathMngr GetSingleton()
	{
	...
	private void Awake()
	{
		...
		int num = 0;
		_AllPathLength = 88;
		allFishPath = new STOF_PathManager[_AllPathLength];
		...
		for (int i = 0; i < _AllPathLength; i++)
		{
			...
			allFishPath[i] = new STOF_PathManager();
		...
		smallFishPath = new STOF_PathManager[_AllPathLength * 4];
	...
	public STOF_PathManager GetFishPath(int type)
	{
		return allFishPath[type];
	}

	public STOF_PathManager GetSmallFishPath(int type)
	{
		return smallFishPath[type];
	}

	private STOF_PathManager ChangePath(STOF_PathManager path, Vector3 vec)
	{
		int num = path.vecs.Length;
		STOF_PathManager sTOF_PathManager = new STOF_PathManager();
		sTOF_PathManager.vecs = new Vector3[num];
		for (int i = 0; i < num; i++)
		{
			sTOF_PathManager.vecs[i] = path.vecs[i] + vec;
		}
		return sTOF_PathManager;
	}

	public void GetAllPath(int type)
	{
		STOF_PathManager sTOF_PathManager = smallFishPath[type];
		int num = sTOF_PathManager.vecs.Length;
		string text = string.Empty;
		for (int i = 0; i < num; i++)
		{
			string text2 = text;
			text = text2 + sTOF_PathManager.vecs[i][0].ToString() + "," + sTOF_PathManager.vecs[i][1].ToString() + "," + sTOF_PathManager.vecs[i][2].ToString() + " ";
		}
