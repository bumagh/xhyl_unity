...
public class BZJX_PlayerEffectCtrl : MonoBehaviour
{
	...
	public void ShowCoin(BZJX_FISH_TYPE fishType, Vector3 pos)
	{
		if (fishType < BZJX_FISH_TYPE.Fish_Turtle || (fishType < BZJX_FISH_TYPE.Fish_Same_Turtle && fishType >= BZJX_FISH_TYPE.Fish_Same_Shrimp))
		{
			_showSilver(BZJX_EffectMngr.GetSingleton().GetFlyCoinNum(fishType), pos);
			BZJX_MusicMngr.GetSingleton().PlayGameSound(BZJX_MusicMngr.GAME_SOUND.SOUND_SILVER_COIN);
		}
		else
		{
			_showGold(BZJX_EffectMngr.GetSingleton().GetFlyCoinNum(fishType), pos);
			BZJX_MusicMngr.GetSingleton().PlayGameSound(BZJX_MusicMngr.GAME_SOUND.SOUND_GOLD_COIN);
		}
	}

	public void ShowLiziCard(Vector3 pos)
	{
		Transform transform = BZJX_EffectMngr.GetSingleton().CreateEffectObj(_Lizi.transform);
		if (pos.x < -6f)
		{
		...
		transform.GetComponent<BZJX_FlyObj>().AnimatedFlyTo(_playerPos);
		_fLiziTime += 20f;
		BZJX_GameInfo.getInstance().GameScene.PlayEnergyGun(mPlayerIndex, isPlay: true);
	}
	...
	public void ShowFishScore(Vector3 pos, int nScore)
	{
		if (nScore > 0)
		{
			Transform transform = BZJX_EffectMngr.GetSingleton().CreateEffectObj(_Score.transform);
			...
			transform.GetComponent<BZJX_FlyObj>().AnimatedShow(_playerPos);
		}
	}
	...
	private void _showGold(int nCoinNum, Vector3 pos)
	{
		...
		{
			Transform transform = BZJX_EffectMngr.GetSingleton().CreateEffectObj(objCoinGold.transform);
			float d = -1f;
			...
			transform.GetComponent<BZJX_FlyObj>().FlyTo(_playerPos);
		}
	}
	...
	private void _showSilver(int nCoinNum, Vector3 pos)
	{
		for (int i = 0; i < nCoinNum; i++)
		{
			Transform transform = BZJX_EffectMngr.GetSingleton().CreateEffectObj(objCoinSilver.transform);
	...
	private void _updateBigPrizePlate()
	{
		...
		if (_prizePlateQue.Count >= 1 && _showTimePlate >= 1f)
		{
			...
			{
				_curPrizePlateObj.GetComponent<BZJX_PrizePlate>().DestroyObj();
			}
			int nNum = _prizePlateQue.Dequeue();
			_curPrizePlateObj = BZJX_EffectMngr.GetSingleton().CreateEffectObj(_prizePlate.transform).gameObject;
			_curPrizePlateObj.GetComponent<BZJX_PrizePlate>().ShowScore(nNum, _playerPos, mPlayerIndex);
			_showTimePlate = 0f;
			BZJX_MusicMngr.GetSingleton().PlayGameSound(BZJX_MusicMngr.GAME_SOUND.SOUND_SCORE_PANEL);
		}
	}
	...
	private void _updateLiziBarBette()
	{
		if (_fLiziTime > 0f)
		{
			...
			if (_fLiziTime <= 0f)
			{
				BZJX_GameInfo.getInstance().GameScene.PlayEnergyGun(mPlayerIndex, isPlay: false);
			}
		}
		else
		{
