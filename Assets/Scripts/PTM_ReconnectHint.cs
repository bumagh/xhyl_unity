...
public class PTM_ReconnectHint : PTM_MB_Singleton<PTM_ReconnectHint>
{
	...
	private void Awake()
	{
		if (PTM_MB_Singleton<PTM_ReconnectHint>._instance == null)
		{
			PTM_MB_Singleton<PTM_ReconnectHint>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void Show()
	{
		if (!_goContainer.activeSelf)
		{
			...
			UnityEngine.Debug.Log(PTM_LogHelper.Magenta("ReconnectHint Show"));
	...
	public void Hide()
	{
		UnityEngine.Debug.Log(PTM_LogHelper.Magenta("ReconnectHint Hide"));
		PTM_LockManager.UnLock("Esc");
	...
	private IEnumerator _textInfoAni()
	{
		...
		while (true)
		{
			string prefix = (PTM_GVars.language == "zh") ? "网络断开，正在重连 " : "Reconnecting to the network ";
