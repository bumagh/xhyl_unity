...
public class DPR_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {DPR_LogHelper.Key(btnKey)} value {DPR_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {DPR_LogHelper.Key(btnKey)} value {DPR_LogHelper.Key(GetValue(btnKey))}");
			}
