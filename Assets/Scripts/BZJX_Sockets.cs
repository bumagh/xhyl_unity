...
public class BZJX_Sockets : MonoBehaviour
{
	public bool isReconnect = true;

	private static BZJX_Sockets _MyCreateSocket;
	...
	private BZJX_DataEncrypt m_DataEncrypt;
	...
	public static BZJX_Sockets GetSingleton()
	{
	...
	public void CreateSocketGetPoint(BZJX_MessageControl MyMessageControl, BZJX_DataEncrypt MyDataEncrypt)
	{
		CreateReceiveThread();
	...
	public void SocketConnect()
	{
		...
		if (m_nSocketStartFlag)
		{
			if (!sendCheckVersion)
			{
				SendCheckVersion(BZJX_Constants.VERSION_CODE);
	...
	private bool _socketStart()
	{
		...
		IPEndPoint iPEndPoint = new IPEndPoint(IPAddress.Parse(BZJX_GameInfo.getInstance().IP), 10032);
	...
	private void _receiveNetMessage(byte[] msgBytes)
	{
		...
		else
		{
			try
			{
				hashtable = BZJX_ReadFileTool.JsonToClass<Hashtable>(@string);
			}
			catch
			{
				msgBytes = m_DataEncrypt.DecompressBytes(msgBytes);
				Console.WriteLine(Encoding.UTF8.GetString(msgBytes));
				hashtable = BZJX_ReadFileTool.JsonToClass<Hashtable>(Encoding.UTF8.GetString(msgBytes));
	...
	public bool _sendMsg(string strMethod, object[] args)
	{
		...
		else
		{
			...
			hashtable.Add("version", BZJX_Constants.Version);
			hashtable.Add("time", m_DataEncrypt.GetUnixTime());
			string text = BZJX_ReadFileTool.JsonByObject(hashtable);
	...
	public bool SendGunHitfish(int gunid, BZJX_HitFish[] hitfish1, bool bHaveKnife, BZJX_HitFish[] hitfish2)
	{
		string empty = string.Empty;
	...
	public bool SendPublicKey()
	{
		...
		string s = BZJX_ReadFileTool.JsonByObject(hashtable);
