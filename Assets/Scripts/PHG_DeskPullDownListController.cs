...
public class PHG_DeskPullDownListController : MonoBehaviour
{
	public static PHG_DeskPullDownListController Instance;
	...
	private void Awake()
	{
		...
		btnArrow.onClick.AddListener(OnListButtonDown);
	}
	...
	public void OnListButtonDown()
	{
		...
		imgBtns[PHG_MB_Singleton<PHG_LobbyViewController>.GetInstance().curDeskIndex].sprite = arrImgButton[1];
	...
	public void ChangeListHeightByDeskNum(int desknum, List<PHG_Desk> desklist)
	{
	...
	public void UpdateDeskInList(List<PHG_Desk> desks)
	{
		UnityEngine.Debug.Log("UpdateDeskInList desks.Count: " + desks.Count + ", buttons.count: " + buttons.Count);
	...
	public void SelectDeskByButton(int index)
	{
		if (bMoveFinished)
		{
			if (index > PHG_MB_Singleton<PHG_LobbyViewController>.GetInstance().curDeskIndex)
			{
				PHG_MB_Singleton<PHG_LobbyViewController>.GetInstance().ForceMoveDesksToRight(index);
			}
			else if (index < PHG_MB_Singleton<PHG_LobbyViewController>.GetInstance().curDeskIndex)
			{
				PHG_MB_Singleton<PHG_LobbyViewController>.GetInstance().ForceMoveDesksToLeft(index);
			}
