...
public class MSE_NetManager : MSE_MB_Singleton<MSE_NetManager>
{
	public delegate void NetMessageHandler(object[] args);

	private MSE_ClientSocket _pClient;

	private MSE_DataEncrypt _pCrypto;
	...
	private Queue<MSE_NetMessage> _netMsgQueue = new Queue<MSE_NetMessage>();

	private Dictionary<string, NetMessageHandler> _netMsgHandlerDic = new Dictionary<string, NetMessageHandler>();

	private void Awake()
	{
		if (MSE_MB_Singleton<MSE_NetManager>._instance == null)
		{
			MSE_MB_Singleton<MSE_NetManager>.SetInstance(this);
		}
	...
	private void _initClientSocket()
	{
		...
		_pClient = new MSE_ClientSocket(useTcpClient: true);
		_pCrypto = new MSE_DataEncrypt();
	...
	private void _handleException(Exception ex)
	{
		...
		isReady = false;
		isLogined = false;
		Queue<MSE_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
			_netMsgQueue.Enqueue(new MSE_NetMessage("netDown", new object[1]
			{
				ex
			}));
		}
	...
	private void _handleConnected()
	{
		try
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new MSE_NetMessage("netConnected", new object[0]));
			}
		}
		catch (Exception ex)
		{
	...
	{
		UnityEngine.Debug.Log("ZH2_GVars.versionCode: " + MSE_GVars.versionCode);
		Send("userService/checkVersion", new object[1]
		{
			MSE_GVars.versionCode
		});
	}
	...
	protected void _handleData(byte[] msgBuffer, int packetId, int endIndex)
	{
		...
		lock (netMsgQueue)
		{
			MSE_NetMessage mSE_NetMessage = new MSE_NetMessage(text, args);
			mSE_NetMessage.jsonString = @string;
			mSE_NetMessage.packetId = packetId;
			mSE_NetMessage.endIndex = endIndex;
			_netMsgQueue.Enqueue(mSE_NetMessage);
		}
	}

	private IEnumerator _checkTimeout(int id, int timeout, MSE_ClientSocket cs)
	{
		UnityEngine.Debug.Log(MSE_LogHelper.Aqua("_checkTimeout start id: {0}", id));
		...
		if (cs != null && cs.doneList.Count > id && !cs.doneList[id])
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new MSE_NetMessage("timeout", new object[1]
				{
					id
				}));
				isTimeout = true;
		...
		if (isTimeout)
		{
			UnityEngine.Debug.Log(MSE_LogHelper.Aqua("_checkTimeout start id: {0} timeout: {1}", id, isTimeout));
		}
	}
	...
	private IEnumerator _checkHeartTimeout(int timeout)
	{
		...
		if (isReady && !isHeartPong)
		{
			...
			lock (netMsgQueue)
			{
				...
				isReady = false;
				isLogined = false;
				_netMsgQueue.Enqueue(new MSE_NetMessage("heart timeout", new object[0]));
				isTimeout = true;
			}
		}
		if (isTimeout)
		{
			UnityEngine.Debug.Log(MSE_LogHelper.Aqua("_checkHeartTimeout is timeout"));
		}
	}

	public void _checkTimeoutStart(int id, int timeout, MSE_ClientSocket cs)
	{
	...
	public void Send(string method, object[] args)
	{
		...
		{
			UnityEngine.Debug.Log(MSE_LogHelper.Magenta("drop send>>method: {0}", method));
			return;
		}
		Hashtable hashtable = new Hashtable();
		hashtable.Add("method", method);
		hashtable.Add("args", args);
		hashtable.Add("version", MSE_GVars.Version);
		if (isReady)
		{
	...
	private bool GetNetMessage(out MSE_NetMessage netMsg)
	{
		Queue<MSE_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
	...
	private void DispatchNetMessage(MSE_NetMessage netMsg)
	{
		if (dropDispatch)
		{
			UnityEngine.Debug.Log(MSE_LogHelper.Magenta("drop send>>method: {0}", netMsg.method));
		}
