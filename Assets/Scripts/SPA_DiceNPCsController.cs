...
public class SPA_DiceNPCsController : MonoBehaviour
{
	...
	public IEnumerator Shake(float delay = 0f)
	{
		...
		SPA_SoundManager.Instance.PlayShakeDiceAudio();
		yield return new WaitForSeconds(1.5f);
		ShakeTableAndGold();
		yield return new WaitForSeconds(0.5f);
		StartCoroutine(SPA_SoundManager.Instance.PlayNPCsDefualtAudio());
	}

	public IEnumerator Open(SPA_DiceGameResultType result)
	{
		switch (result)
		{
		case SPA_DiceGameResultType.Lose:
			yield return new WaitForSeconds(1f);
			SPA_SoundManager.Instance.PlayDiceLooseAudio();
			break;
		case SPA_DiceGameResultType.Win:
			yield return new WaitForSeconds(1f);
			SPA_SoundManager.Instance.PlayDiceWinAudio();
			break;
		case SPA_DiceGameResultType.Overflow:
			yield return new WaitForSeconds(1f);
			SPA_SoundManager.Instance.PlayDiceWinAudio();
			yield return new WaitForSeconds(3.1f);
			SPA_SoundManager.Instance.PlayDiceOverFlowAudio();
			break;
		}
	}
