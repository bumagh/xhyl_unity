...
public class SHT_HUDController : SHT_MB_Singleton<SHT_HUDController>
{
	...
	private void Awake()
	{
		...
		if (SHT_MB_Singleton<SHT_HUDController>._instance == null)
		{
			SHT_MB_Singleton<SHT_HUDController>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void PreInit()
	{
		...
		{
			ruleController = base.transform.Find("Canvas/Mask/Rule").GetComponent<SHT_RulePicController>();
		}
	}

	public void Init()
	{
		SHT_MB_Singleton<SHT_OptionsController>.GetInstance().onItemRules = delegate
		{
			ruleController.Show();
		};
		ruleController.Hide();
	...
	{
		ping = new Ping(SHT_GVars.IPAddress);
		if (waitPing != null)
		{
	...
	public void OnBtnOptions_Click()
	{
		...
		{
			if (SHT_MB_Singleton<SHT_OptionsController>.GetInstance().isShow)
			{
