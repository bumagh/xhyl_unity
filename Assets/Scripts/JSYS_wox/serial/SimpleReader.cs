...
namespace JSYS_wox.serial
{
	public class SimpleReader : ObjectReader
	{
		...
		public object readPrimitiveArray(XmlReader xob, object id)
		{
			try
			{
				Type type = (Type)JSYS_Serial.mapWOXToCSharp[xob.GetAttribute("elementType")];
		...
		public object readClassArray(Type[] a, string[] s)
		{
			...
			for (int i = 0; i < a.Length; i++)
			{
				...
				Type type = (Type)JSYS_Serial.mapWOXToCSharp[s[i]];
				if (type == null)
				{
					type = (Type)JSYS_Serial.mapArrayWOXToCSharp[s[i]];
					if (type == null)
					{
						try
						{
		...
		{
			Type type = (Type)JSYS_Serial.mapWOXToCSharp[arrayTypeName];
			if (type != null)
			{
				return type;
			}
			type = (Type)JSYS_Serial.mapArrayWOXToCSharp[arrayTypeName];
			if (type == null)
			{
		...
		public object readStringObject(XmlReader xob, object id)
		{
			try
			{
				Type type = (Type)JSYS_Serial.mapWOXToCSharp[xob.GetAttribute("type")];
				if (type.Equals(typeof(Type)))
				{
					Type type2 = (Type)JSYS_Serial.mapWOXToCSharp[xob.GetAttribute("value")];
					if (type2 == null)
					{
						type2 = (Type)JSYS_Serial.mapArrayWOXToCSharp[xob.GetAttribute("value")];
						if (type2 == null)
						{
