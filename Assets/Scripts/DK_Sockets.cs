...
public class DK_Sockets : MonoBehaviour
{
	public bool isReconnect = true;

	private static DK_Sockets _MyCreateSocket;
	...
	private DK_DataEncrypt m_DataEncrypt;
	...
	public static DK_Sockets GetSingleton()
	{
	...
	public void CreateSocketGetPoint(DK_MessageControl MyMessageControl, DK_DataEncrypt MyDataEncrypt)
	{
		CreateReceiveThread();
	...
	public void SocketConnect()
	{
		...
		if (m_nSocketStartFlag)
		{
			if (!sendCheckVersion)
			{
				SendCheckVersion(DK_Constants.VERSION_CODE);
	...
	private bool _socketStart()
	{
		...
		IPEndPoint remoteEP = new IPEndPoint(IPAddress.Parse(DK_GameInfo.getInstance().IP), 10012);
		try
		{
	...
	private void _receiveNetMessage(byte[] msgBytes)
	{
		...
		else
		{
			try
			{
				hashtable = DK_ReadFileTool.JsonToClass<Hashtable>(@string);
			}
			catch (Exception)
			{
				msgBytes = m_DataEncrypt.DecompressBytes(msgBytes);
				Console.WriteLine(Encoding.UTF8.GetString(msgBytes));
				hashtable = DK_ReadFileTool.JsonToClass<Hashtable>(Encoding.UTF8.GetString(msgBytes));
	...
	public bool _sendMsg(string strMethod, object[] args)
	{
		...
		else
		{
			...
			hashtable.Add("version", DK_Constants.Version);
			hashtable.Add("time", m_DataEncrypt.GetUnixTime());
			string text = DK_ReadFileTool.JsonByObject(hashtable);
	...
	public bool SendCheckLogin(string userName, string passWord, int uiid, string versionCode = "")
	{
		...
		UnityEngine.Debug.Log("userService/checkLogin");
		return _sendMsg(strMethod, args);
	}
	...
	public bool SendGetGameGold()
	{
		string strMethod = "userService/getGameGold";
		object[] args = new object[0];
		return _sendMsg(strMethod, args);
	}
	...
	{
		string text = "fired," + gunid + "," + Math.Round(fRot, 2) + "," + gunValue + "," + bX2Gun;
		UnityEngine.Debug.LogWarning("开炮: " + text);
		return _sendMsg(text, null);
	}

	public bool SendGunHitfish(int gunid, DK_HitFish[] hitfish1, bool bHaveKnife, DK_HitFish[] hitfish2)
	{
		string empty = string.Empty;
		...
		UnityEngine.Debug.Log("发送gunHitFish: " + empty);
	...
	public bool SendPublicKey()
	{
		...
		string s = DK_ReadFileTool.JsonByObject(hashtable);
