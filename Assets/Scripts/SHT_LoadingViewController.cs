...
public class SHT_LoadingViewController : SHT_MB_Singleton<SHT_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (SHT_MB_Singleton<SHT_LoadingViewController>._instance == null)
		{
			SHT_MB_Singleton<SHT_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (SHT_MB_Singleton<SHT_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!SHT_MB_Singleton<SHT_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
