...
public class LKB_TileManager : MonoBehaviour
{
	...
	public IEnumerator StartChasing(int targetIndex, Action endAction)
	{
		...
		LKB_XWait xwait = new LKB_XWait(this).Begin();
		int count = 0;
		float timeCount = 0f;
	...
	public IEnumerator WaitSecondsAction(float seconds, Action<int> action, int param1, LKB_XWait xwait = null)
	{
		if (xwait == null)
		{
			yield return new WaitForSeconds(seconds);
