...
public class USW_LoadingViewController : USW_MB_Singleton<USW_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (USW_MB_Singleton<USW_LoadingViewController>._instance == null)
		{
			USW_MB_Singleton<USW_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (USW_MB_Singleton<USW_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!USW_MB_Singleton<USW_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
