...
public class ESP_SymbolC : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		symlen = Cinstance<ESP_Gcore>.Instance.Symcont;
	...
	public void Init(int idx, int symid, ESP_Line line = null)
	{
		symidx = idx;
		lineroll = line;
	...
	{
		iconimg.sprite = Cinstance<ESP_Gcore>.Instance.SymSprlist[idx];
	}
	...
	public void StopRoll()
	{
		...
		if (isroll)
		{
			...
			if (Inlineidx > 2)
			{
				...
				int idx = Cinstance<ESP_Gcore>.Instance.Result[index];
				SetSymbolimg(idx, 0L);
			}
		}
		ESP_SoundManager.Instance.PlayStopAndio();
		speed = normalspeed;
	}
	...
	private void Setrollnum(List<int> randomlist, int rolllen)
	{
		...
		if (Inlineidx > 2)
		{
			...
			int value = Cinstance<ESP_Gcore>.Instance.Result[index2];
	...
	public void Playani(int winline = -1)
	{
		...
		try
		{
			Seq_ani = iconimg.ESP_PlayAni(string.Format(aniname, (SymbolId + 1).ToString("00")) + "{0:0}_png", string.Format(aniname, (SymbolId + 1).ToString("00")) + "{0:0}_png", 1, frames);
			Seq_ani.Restart();
		}
		catch (Exception arg)
		{
