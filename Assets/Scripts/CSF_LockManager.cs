...
public class CSF_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {CSF_LogHelper.Key(btnKey)} value {CSF_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {CSF_LogHelper.Key(btnKey)} value {CSF_LogHelper.Key(GetValue(btnKey))}");
			}
