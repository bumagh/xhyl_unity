...
public sealed class OF_SpawnPool : MonoBehaviour, IList<Transform>, ICollection<Transform>, IEnumerable<Transform>, IEnumerable
{
	...
	public List<OF_PrefabPool> _perPrefabPoolOptions = new List<OF_PrefabPool>();
	...
	public OF_PrefabsDict prefabs = new OF_PrefabsDict();

	public Dictionary<object, bool> _editorListItemStates = new Dictionary<object, bool>();

	private List<OF_PrefabPool> _prefabPools = new List<OF_PrefabPool>();
	...
	public Dictionary<string, OF_PrefabPool> prefabPools
	{
		get
		{
			Dictionary<string, OF_PrefabPool> dictionary = new Dictionary<string, OF_PrefabPool>();
			foreach (OF_PrefabPool prefabPool in _prefabPools)
			{
	...
	private void Awake()
	{
		...
		foreach (OF_PrefabPool perPrefabPoolOption in _perPrefabPoolOptions)
		{
		...
		STOF_PoolManager.Pools.Add(this);
	}
	...
	private void OnDestroy()
	{
		...
		STOF_PoolManager.Pools.Remove(this);
		StopAllCoroutines();
		_spawned.Clear();
		foreach (OF_PrefabPool prefabPool in _prefabPools)
		{
	...
	public void CreatePrefabPool(OF_PrefabPool prefabPool)
	{
	...
	public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot)
	{
		...
		OF_PrefabPool oF_PrefabPool = new OF_PrefabPool(prefab);
		CreatePrefabPool(oF_PrefabPool);
		transform = oF_PrefabPool.SpawnInstance(pos, rot);
		transform.parent = group;
		_spawned.Add(transform);
		return transform;
	}
