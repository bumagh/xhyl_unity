...
public class PTM_TileManager : MonoBehaviour
{
	...
	public IEnumerator StartChasing(int targetIndex, Action endAction)
	{
		...
		PTM_XWait xwait = new PTM_XWait(this).Begin();
		int count = 0;
		float timeCount = 0f;
	...
	public IEnumerator WaitSecondsAction(float seconds, Action<int> action, int param1, PTM_XWait xwait = null)
	{
		if (xwait == null)
		{
			yield return new WaitForSeconds(seconds);
