...
public class PTM_NetManager : PTM_MB_Singleton<PTM_NetManager>
{
	public delegate void NetMessageHandler(object[] args);

	private PTM_ClientSocket _pClient;

	private PTM_DataEncrypt _pCrypto;
	...
	private Queue<PTM_NetMessage> _netMsgQueue = new Queue<PTM_NetMessage>();

	private Dictionary<string, NetMessageHandler> _netMsgHandlerDic = new Dictionary<string, NetMessageHandler>();

	private void Awake()
	{
		if (PTM_MB_Singleton<PTM_NetManager>._instance == null)
		{
			PTM_MB_Singleton<PTM_NetManager>.SetInstance(this);
		}
	...
	{
		UnityEngine.Debug.LogError("_initClientSocket");
		isHeartPong = true;
		isConnected = false;
		isReady = false;
		isLogined = false;
		...
		_pClient = new PTM_ClientSocket(useTcpClient: true);
		_pCrypto = new PTM_DataEncrypt();
	...
	private void _handleException(Exception ex)
	{
		...
		isReady = false;
		isLogined = false;
		Queue<PTM_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
			_netMsgQueue.Enqueue(new PTM_NetMessage("netDown", new object[1]
			{
				ex
			}));
		}
	...
	{
		connectCount++;
		UnityEngine.Debug.LogError("超时: " + connectCount);
		_initClientSocket();
	...
	private void _handleConnected()
	{
		try
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new PTM_NetMessage("netConnected", new object[0]));
			}
		}
		catch (Exception ex)
		{
	...
	{
		UnityEngine.Debug.Log("ZH2_GVars.versionCode: " + PTM_GVars.versionCode);
		Send("userService/checkVersion", new object[1]
		{
			PTM_GVars.versionCode
		});
	}
	...
	protected void _handleData(byte[] msgBuffer, int packetId, int endIndex)
	{
		...
		lock (netMsgQueue)
		{
			PTM_NetMessage pTM_NetMessage = new PTM_NetMessage(text, args);
			pTM_NetMessage.jsonString = @string;
			pTM_NetMessage.packetId = packetId;
			pTM_NetMessage.endIndex = endIndex;
			_netMsgQueue.Enqueue(pTM_NetMessage);
		}
	}

	private IEnumerator _checkTimeout(int id, int timeout, PTM_ClientSocket cs)
	{
		UnityEngine.Debug.Log(PTM_LogHelper.Aqua("_checkTimeout start id: {0}", id));
		...
		if (cs != null && cs.doneList.Count > id && !cs.doneList[id])
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new PTM_NetMessage("timeout", new object[1]
				{
					id
				}));
				isTimeout = true;
		...
		if (isTimeout)
		{
			UnityEngine.Debug.Log(PTM_LogHelper.Aqua("_checkTimeout start id: {0} timeout: {1}", id, isTimeout));
		}
	}
	...
	private IEnumerator _checkHeartTimeout(int timeout)
	{
		...
		if (isReady && !isHeartPong)
		{
			...
			lock (netMsgQueue)
			{
				...
				isReady = false;
				isLogined = false;
				_netMsgQueue.Enqueue(new PTM_NetMessage("heart timeout", new object[0]));
				isTimeout = true;
			}
		}
		if (isTimeout)
		{
			UnityEngine.Debug.Log(PTM_LogHelper.Aqua("_checkHeartTimeout is timeout"));
		}
	}

	public void _checkTimeoutStart(int id, int timeout, PTM_ClientSocket cs)
	{
	...
	public void Send(string method, object[] args)
	{
		...
		{
			UnityEngine.Debug.Log(PTM_LogHelper.Magenta("drop send>>method: {0}", method));
			return;
		}
		Hashtable hashtable = new Hashtable();
		hashtable.Add("method", method);
		hashtable.Add("args", args);
		hashtable.Add("version", PTM_GVars.Version);
		if (isReady)
		{
	...
	private bool GetNetMessage(out PTM_NetMessage netMsg)
	{
		Queue<PTM_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
	...
	private void DispatchNetMessage(PTM_NetMessage netMsg)
	{
		if (dropDispatch)
		{
			UnityEngine.Debug.Log(PTM_LogHelper.Magenta("drop send>>method: {0}", netMsg.method));
		}
