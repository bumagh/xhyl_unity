...
public class CSF_HUDController : CSF_MB_Singleton<CSF_HUDController>
{
	...
	private void Awake()
	{
		...
		if (CSF_MB_Singleton<CSF_HUDController>._instance == null)
		{
			CSF_MB_Singleton<CSF_HUDController>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void PreInit()
	{
		...
		{
			ruleController = base.transform.Find("Canvas/Mask/Rule").GetComponent<CSF_RulePicController>();
		}
	}

	public void Init()
	{
		CSF_MB_Singleton<CSF_OptionsController>.GetInstance().onItemRules = delegate
		{
			ruleController.Show();
		};
		ruleController.Hide();
	...
	public void OnBtnOptions_Click()
	{
		...
		{
			if (CSF_MB_Singleton<CSF_OptionsController>.GetInstance().isShow)
			{
