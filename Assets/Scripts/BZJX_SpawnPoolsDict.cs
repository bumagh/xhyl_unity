...
public class BZJX_SpawnPoolsDict : IDictionary<string, BZJX_SpawnPool>, ICollection<KeyValuePair<string, BZJX_SpawnPool>>, IEnumerable<KeyValuePair<string, BZJX_SpawnPool>>, IEnumerable
{
	private Dictionary<string, BZJX_SpawnPool> _pools = new Dictionary<string, BZJX_SpawnPool>();

	bool ICollection<KeyValuePair<string, BZJX_SpawnPool>>.IsReadOnly => true;

	public int Count => _pools.Count;

	public BZJX_SpawnPool this[string key]
	{
		get
		{
			try
	...
	public ICollection<BZJX_SpawnPool> Values
	{
		get
		{
			string message = "If you need this, please request it.";
	...
	void ICollection<KeyValuePair<string, BZJX_SpawnPool>>.CopyTo(KeyValuePair<string, BZJX_SpawnPool>[] array, int arrayIndex)
	{
	...
	public BZJX_SpawnPool Create(string poolName)
	{
		GameObject gameObject = new GameObject(poolName + "Pool");
		return gameObject.AddComponent<BZJX_SpawnPool>();
	}

	public BZJX_SpawnPool Create(string poolName, GameObject owner)
	{
		...
		try
		{
			...
			return owner.AddComponent<BZJX_SpawnPool>();
		}
		finally
		{
	...
	{
		if (!_pools.TryGetValue(poolName, out BZJX_SpawnPool value))
		{
	...
	{
		foreach (KeyValuePair<string, BZJX_SpawnPool> pool in _pools)
		{
			UnityEngine.Object.Destroy(pool.Value);
		}
	}

	internal void Add(BZJX_SpawnPool spawnPool)
	{
	...
	public void Add(string key, BZJX_SpawnPool value)
	{
	...
	internal bool Remove(BZJX_SpawnPool spawnPool)
	{
		if (!ContainsKey(spawnPool.poolName))
		{
	...
	public bool TryGetValue(string poolName, out BZJX_SpawnPool spawnPool)
	{
		return _pools.TryGetValue(poolName, out spawnPool);
	}

	public bool Contains(KeyValuePair<string, BZJX_SpawnPool> item)
	{
		string message = "Use PoolManager.Pools.Contains(string poolName) instead.";
		throw new NotImplementedException(message);
	}

	public void Add(KeyValuePair<string, BZJX_SpawnPool> item)
	{
		string message = "SpawnPools add themselves to PoolManager.Pools when created, so there is no need to Add() them explicitly. Create pools using PoolManager.Pools.Create() or add a SpawnPool component to a GameObject.";
		throw new NotImplementedException(message);
	}
	...
	private void CopyTo(KeyValuePair<string, BZJX_SpawnPool>[] array, int arrayIndex)
	{
		string message = "PoolManager.Pools cannot be copied";
		throw new NotImplementedException(message);
	}

	public bool Remove(KeyValuePair<string, BZJX_SpawnPool> item)
	{
		string message = "SpawnPools can only be destroyed, not removed and kept alive outside of PoolManager. There are only 2 legal ways to destroy a SpawnPool: Destroy the GameObject directly, if you have a reference, or use PoolManager.Destroy(string poolName).";
		throw new NotImplementedException(message);
	}

	public IEnumerator<KeyValuePair<string, BZJX_SpawnPool>> GetEnumerator()
	{
		return _pools.GetEnumerator();
	}
}
