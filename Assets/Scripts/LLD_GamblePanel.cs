...
public class LLD_GamblePanel : MonoBehaviour
{
	...
	public static LLD_GamblePanel ins;
	...
	{
		LLD_MB_Singleton<LLD_DiceGameController2>.GetInstance().Send_MultipleInfo(LLD_BetDiceType.Half);
		SetButton(canclick: false);
	}

	private void ClickBlack()
	{
		LLD_MB_Singleton<LLD_DiceGameController2>.GetInstance().Send_MultipleInfo(LLD_BetDiceType.Risk);
		SetButton(canclick: false);
	}
	...
	private Sprite Getsprite()
	{
		try
		{
			uint num = Getnum(LLD_MB_Singleton<LLD_DiceGameController2>.GetInstance().Gamelewinid);
	...
	{
		balancescore = LLD_GVars.winScorce;
		doublescore = LLD_GVars.winScorce * 2;
	...
	{
		LLD_MB_Singleton<LLD_DiceGameController2>.GetInstance().OnBtnReady_Click();
		LLD_MB_Singleton<LLD_DiceGameController2>.GetInstance().Hide();
		int credit = (!isGamble) ? (LLD_GVars.winScorce + LLD_GVars.credit) : AllScroce;
		LLD_MB_Singleton<LLD_MajorGameController>.GetInstance().SetCredit(credit);
