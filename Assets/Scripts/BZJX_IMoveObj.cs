...
public abstract class BZJX_IMoveObj : MonoBehaviour
{
	...
	private float _fLimitLeft = 0f - BZJX_SetCanvas.Width * 6.2f / 1280f;

	private float _fLimitRight = BZJX_SetCanvas.Width * 6.2f / 1280f;
	...
	private void Start()
	{
		_fLimitLeft = 0f - BZJX_SetCanvas.Width * 6.2f / 1280f;
		_fLimitRight = BZJX_SetCanvas.Width * 6.2f / 1280f;
	}
	...
	public void MoveLogic()
	{
		...
		{
			if (BZJX_GameInfo.getInstance().UserList[i].SeatIndex != _nPlayerID)
			{
				continue;
			}
			if ((bool)BZJX_GameInfo.getInstance().UserList[i].LockFish)
			{
				BZJX_ISwimObj swimObjByTag = BZJX_FishPoolMngr.GetSingleton().GetSwimObjByTag(BZJX_GameInfo.getInstance().UserList[i].LockFish);
				if (BZJX_GameInfo.getInstance().UserList[i].Lock && !swimObjByTag.bFishDead)
				{
