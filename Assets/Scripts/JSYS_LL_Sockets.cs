...
public class JSYS_LL_Sockets : MonoBehaviour
{
	public bool isReconnect = true;

	private static JSYS_LL_Sockets _MyCreateSocket;
	...
	public static JSYS_LL_Sockets GetSingleton()
	{
	...
	public void CreateSocketGetPoint(JSYS_LL_MessageControl MyMessageControl, JSYS_LL_DataEncrypt MyDataEncrypt)
	{
		CreateReceiveThread();
	...
	public void SocketConnect()
	{
		...
		if (m_nSocketStartFlag)
		{
			if (!sendCheckVersion)
			{
				SendCheckVersion(JSYS_LL_Constants.VERSION_CODE);
	...
	{
		UnityEngine.Debug.LogError("_socketStart");
		MySocket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
		IPEndPoint iPEndPoint = new IPEndPoint(IPAddress.Parse(JSYS_LL_GameInfo.getInstance().IP), 19926);
		...
		{
			UnityEngine.Debug.LogError("connect0: " + ex.Message);
			UnityEngine.Debug.LogError("connect1: " + ex);
			_netDownControl();
			flag = false;
		}
		UnityEngine.Debug.LogError(iPEndPoint + "  " + flag);
		return flag;
	}
	...
	private void _myReceiveControl()
	{
		...
		while (true)
		{
			...
			{
				UnityEngine.Debug.LogError("Receive:" + ex.Message);
				_netDownControl();
				return;
			}
			if (num <= 0)
	...
	public void socketListener()
	{
		...
		while (true)
		{
			...
			if (num - m_recriveHeartTime > 20000)
			{
				...
				UnityEngine.Debug.LogError("======socketListener======");
				_netDownControl();
	...
	private bool _sendMsg(string strMethod, object[] args)
	{
		...
		if (strMethod != "userService/heart" && Application.platform == RuntimePlatform.WindowsEditor)
		{
			try
			{
				UnityEngine.Debug.Log("已经发送的方法：" + strMethod + "  " + JsonMapper.ToJson(args));
			}
			catch
			{
			}
	...
	{
		UnityEngine.Debug.LogError("=====添加NetThread/NetDown=====");
	...
	public bool SendUserCoinIn(int nCoinInNumber = 0)
	{
	...
	{
		string strMethod = "GoldSharkDeskResultService/resultList";
		object[] args = new object[1]
		{
			nDeskId
		};
