...
public class LRS_LobbyViewController : LRS_MB_Singleton<LRS_LobbyViewController>
{
	...
	private LRS_HeadViewController _headViewController;

	private LRS_UserViewer _userViewer;
	...
	private List<LRS_Desk> _deskList = new List<LRS_Desk>();
	...
	private LRS_DeskWidget[] _deskWidgets = new LRS_DeskWidget[3];
	...
	private void Awake()
	{
		...
		if (LRS_MB_Singleton<LRS_LobbyViewController>._instance == null)
		{
			LRS_MB_Singleton<LRS_LobbyViewController>.SetInstance(this);
			PreInit();
		}
	}
	...
	private void InitFinGame()
	{
		...
		_headViewController = base.transform.Find("Title").GetComponent<LRS_HeadViewController>();
		_userViewer = base.transform.Find("PersonInfoDialog").GetComponent<LRS_UserViewer>();
	...
	private void Update()
	{
		if (UnityEngine.Input.GetKeyDown(KeyCode.Escape) && !LRS_MB_Singleton<LRS_LobbyViewController>.GetInstance()._IsAning && !LRS_LockManager.IsLocked("Esc"))
		{
			UnityEngine.Debug.Log("ESC Btn Down In Lobby View");
			LRS_MB_Singleton<LRS_GameManager>.GetInstance().Handle_BtnReturn();
		}
	}
	...
	private void _init()
	{
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().RegisterHandler("roomInfo", HandleNetMsg_RoomInfo);
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().RegisterHandler("enterDesk", HandleNetMsg_EnterDesk);
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().RegisterHandler("updateRoomInfo", HandleNetMsg_UpdateRoomInfo);
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().RegisterHandler("addExpeGoldAuto", HandleNetMsg_AddExpeGoldAuto);
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().RegisterHandler("expeGold", HandleNetMsg_ExpeGold);
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().RegisterHandler("playerInfo", HandleNetMsg_PlayerInfo);
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().RegisterHandler("updateDeskInfo", HandleNetMsg_UpdateDeskInfo);
		_goRoomView.SetActive(value: true);
		_goDeskView.SetActive(value: false);
		int num = (LRS_GVars.language == "en") ? 1 : 0;
		...
		LRS_Drag_DeskController._instance.left = OnBtnLeftDesk_Click;
		LRS_Drag_DeskController._instance.right = OnBtnRightDesk_Click;
	...
	public void Show()
	{
		...
		LRS_MB_Singleton<LRS_GameManager>.GetInstance().ChangeView(empty);
	...
	{
		if (!LRS_GVars.tryLockOnePoint && LRS_DeskPullDownListController.Instance.bMoveFinished)
		{
			_desksMoveToLeft();
		}
	}
	...
	{
		if (!LRS_GVars.tryLockOnePoint && _bCanClick)
		{
			_bCanClick = false;
			LRS_SoundManager.Instance.PlayClickAudio();
	...
	public void Send_EnterRoom(int roomType)
	{
		...
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().Send("userService/enterRoom", args);
		LRS_MB_Singleton<LRS_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "Send_EnterRoom");
	}
	...
	public void Send_EnterDesk(int deskId)
	{
		...
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().Send("userService/enterDesk", args);
		LRS_MB_Singleton<LRS_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "Send_EnterDesk");
	}
	...
	public void Send_LeaveRoom()
	{
		...
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().Send("userService/leaveRoom", args);
	}

	public void Send_GetUserAward()
	{
		object[] args = new object[0];
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().Send("userService/getUserAward", args);
	}

	public void Send_AddExpeGoldAuto()
	{
		object[] args = new object[0];
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().Send("userService/addExpeGoldAuto", args);
	}
	...
	public void Send_PlayerInfo(int userId)
	{
		...
		LRS_MB_Singleton<LRS_NetManager>.GetInstance().Send("userService/playerInfo", args);
	}

	public void HandleNetMsg_RoomInfo(object[] args)
	{
		UnityEngine.Debug.Log(LRS_LogHelper.NetHandle("HandleNetMsg_RoomInfo"));
		if (LRS_GVars.curView == "MajorGame" || LRS_GVars.curView == "DiceGame")
		{
			UnityEngine.Debug.Log(LRS_LogHelper.Orange("游戏中，不处理"));
			return;
		}
		_curState = "DeskView";
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
		LRS_MB_Singleton<LRS_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "Send_EnterRoom done");
		if (dictionary != null)
		{
			LRS_LockManager.UnLock("btn_room");
			UnityEngine.Debug.Log("isOverflow: " + dictionary["isOverflow"]);
			LRS_MB_Singleton<LRS_AlertDialog>.GetInstance().ShowDialog((LRS_GVars.language == "zh") ? "您的账户已爆机，请兑奖" : "Your account is blasting, please exchange");
			return;
		}
		object[] array = args[0] as object[];
		List<LRS_Desk> list = new List<LRS_Desk>();
		for (int i = 0; i < array.Length; i++)
		{
			Dictionary<string, object> data = array[i] as Dictionary<string, object>;
			list.Add(LRS_Desk.CreateWithDic(data));
		}
		_deskList = list;
	...
	{
		UnityEngine.Debug.Log(LRS_LogHelper.NetHandle("HandleNetMsg_EnterDesk"));
		LRS_LockManager.UnLock("EnterDesk");
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
		bool flag = (bool)dictionary["success"];
		LRS_MB_Singleton<LRS_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "Send_EnterDesk done");
		UnityEngine.Debug.LogError("flag: " + flag);
		if (flag)
		{
			Hide();
			UnityEngine.Debug.LogError("curDeskIndex: " + curDeskIndex);
			LRS_GVars.desk = _deskList[curDeskIndex];
			LRS_MB_Singleton<LRS_MajorGameController>.GetInstance().Show();
			LRS_MB_Singleton<LRS_MajorGameController>.GetInstance().InitGame();
			LRS_MB_Singleton<LRS_ScoreBank>.GetInstance().SetKeepScore(0);
			LRS_MB_Singleton<LRS_ScoreBank>.GetInstance().SetRate(LRS_GVars.desk.exchange);
			return;
		}
		switch ((int)dictionary["messageStatus"])
		{
		case 0:
			LRS_MB_Singleton<LRS_AlertDialog>.GetInstance().ShowDialog((LRS_GVars.language == "zh") ? "桌子不存在" : "Game table not exist");
			break;
		case 1:
			LRS_MB_Singleton<LRS_AlertDialog>.GetInstance().ShowDialog((LRS_GVars.language == "zh") ? "桌已满" : "Don't Have Free Table");
			break;
		case 2:
			LRS_MB_Singleton<LRS_AlertDialog>.GetInstance().ShowDialog((LRS_GVars.language == "zh") ? "您的账户已爆机，请兑奖" : "Your account is blasting, please excharge");
			break;
		case 3:
			LRS_MB_Singleton<LRS_AlertDialog>.GetInstance().ShowDialog((LRS_GVars.language == "zh") ? "体验币不足" : "Experience Coins insufficient");
			break;
		case 4:
			LRS_MB_Singleton<LRS_AlertDialog>.GetInstance().ShowDialog((LRS_GVars.language == "zh") ? "游戏币不足" : "Coins insufficient");
			break;
		default:
	...
	{
		UnityEngine.Debug.Log(LRS_LogHelper.NetHandle("HandleNetMsg_UpdateRoomInfo"));
		...
		if (count != list.Count)
		{
			...
			LRS_DeskPullDownListController.Instance.UpdateDeskInList(_deskList);
		}
		else
		{
			UpdateAllDeskWidgets();
	...
	public void HandleNetMsg_AddExpeGoldAuto(object[] args)
	{
		...
		LRS_MB_Singleton<LRS_AlertDialog>.GetInstance().ShowDialog("申请成功！请返回大厅重新游戏", showOkCancel: false, delegate
		{
	...
	{
		UnityEngine.Debug.Log(LRS_LogHelper.NetHandle("HandleNetMsg_ExpeGold"));
		if ((bool)args[0])
		{
			LRS_GVars.user.expeGold = 10000;
	...
	{
		UnityEngine.Debug.Log(LRS_LogHelper.NetHandle("HandleNetMsg_PlayerInfo"));
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
	...
	{
		UnityEngine.Debug.Log(LRS_LogHelper.NetHandle("HandleNetMsg_UpdateDeskInfo"));
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
	...
	private IEnumerator _showDeskViewAni()
	{
		...
		LRS_GVars.curView = "DeskSelectionView";
		LRS_LockManager.UnLock("btn_room");
		yield return null;
	}

	private LRS_DeskWidget _createDeskWidget()
	{
		...
		LRS_DeskWidget component = gameObject.GetComponent<LRS_DeskWidget>();
	...
	{
		LRS_DeskPullDownListController.Instance.bMoveFinished = false;
		LRS_SoundManager.Instance.PlayClickAudio();
		if (_deskList.Count != 0 && !_inputForbidden)
		{
			LRS_DeskPullDownListController.Instance.ShowListButtonHighBG(_bIsLeft: true, curDeskIndex);
			...
			sequence.onComplete = delegate
			{
				...
				LRS_DeskPullDownListController.Instance.bMoveFinished = true;
				LRS_DeskPointsController._Instance.ShowDeskPoint2(_deskList.Count, curDeskIndex);
			};
		}
	}
	...
	private void _desksMoveToRight()
	{
		...
		if (_deskList.Count != 0 && !_inputForbidden)
		{
			LRS_DeskPullDownListController.Instance.ShowListButtonHighBG(_bIsLeft: false, curDeskIndex);
	...
	{
		LRS_DeskWidget lRS_DeskWidget = _deskWidgets[a];
		_deskWidgets[a] = _deskWidgets[b];
		_deskWidgets[b] = lRS_DeskWidget;
	}
	...
	public void UpdateAllDeskWidgets()
	{
		...
		LRS_DeskPointsController._Instance.ShowDeskPoint2(_deskList.Count, curDeskIndex);
		LRS_DeskPullDownListController.Instance.ChangeListHeightByDeskNum(_deskList.Count, _deskList);
	}
	...
	private void _onBtnDesk_Click()
	{
		...
		{
			if (LRS_GVars.user.expeGold == 0 || LRS_GVars.user.expeGold < _deskList[curDeskIndex].minGold)
			{
				UnityEngine.Debug.Log($"expeGold: {LRS_GVars.user.expeGold}, _curDeskIndex: {curDeskIndex}, minGold: {_deskList[curDeskIndex].minGold}");
				LRS_MB_Singleton<LRS_AlertDialog>.GetInstance().ShowDialog("体验币不足，是否领取体验币？", showOkCancel: true, delegate
				{
			...
			{
				LRS_LockManager.Lock("EnterDesk");
				Send_EnterDesk(_deskList[curDeskIndex].id);
				LRS_SoundManager.Instance.StopLobbyBGM();
			}
		}
		else if (LRS_GVars.user.gameGold == 0 || LRS_GVars.user.gameGold < _deskList[curDeskIndex].minGold)
		{
			UnityEngine.Debug.LogError($"gameGold: {LRS_GVars.user.gameGold}, _curDeskIndex: {curDeskIndex}, minGold: {_deskList[curDeskIndex].minGold}");
			LRS_MB_Singleton<LRS_AlertDialog>.GetInstance().ShowDialog("游戏币不足！");
		}
		else
		{
			LRS_LockManager.Lock("EnterDesk");
	...
	{
		string info = (LRS_GVars.language == "zh") ? "选择房间" : "SelectRoom";
		if (_curState == "DeskView")
		{
			if (_curRoomType == 1)
			{
				info = ((LRS_GVars.language == "zh") ? "新手练习厅" : "Training");
			}
			else if (_curRoomType == 2)
			{
				info = ((LRS_GVars.language == "zh") ? "欢乐竞技厅" : "Arena");
			}
		}
	...
	{
		_IsAning = true;
		LRS_MB_Singleton<LRS_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "_hideRoomViewAni");
		Sequence sequence = DOTween.Sequence();
		...
		sequence.onComplete = delegate
		{
			LRS_MB_Singleton<LRS_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "_hideRoomViewAni done");
			_IsAning = false;
			_bCanClick = true;
		};
	}
	...
	public void ResetRoomView()
	{
		...
		LRS_MB_Singleton<LRS_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "ResetRoomView");
		_userViewer.gameObject.SetActive(value: false);
		Sequence sequence = DOTween.Sequence();
		...
		sequence.onComplete = delegate
		{
			...
			_bCanClick = false;
			_updateHeadInfo();
			LRS_MB_Singleton<LRS_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "ResetRoomView_Over");
		};
	}
	...
	public void HideDeskViewAni()
	{
		...
		sequence.onComplete = delegate
		{
			LRS_MB_Singleton<LRS_DeskSelectionViewController>.GetInstance().Hide();
		};
	}
}
