...
public class CSF_CellManager : MonoBehaviour
{
	public static CSF_CellManager cellManager;
	...
	private CSF_OverallComputer overall;

	private CSF_HitLineComputer[] winLines;
	...
	private List<CSF_WinLineResult> lineResults;
	...
	{
		List<int> showchange = CSF_MB_Singleton<CSF_MajorGameController>.GetInstance().Showchange1;
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				...
				{
					Cells[i, j].PlayState(CSF_CellState.normal, CSF_CellType.HongFa);
				}
			}
		}
	}
	...
	public void PreInit()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				Cells[i, j] = base.transform.Find($"Cell{i}_{j}").GetComponent<CSF_Cell>();
			}
		}
	}
	...
	public void UpdateCellGridData(int[,] grid, int[,] oldGrid)
	{
		this.grid = grid;
		this.oldGrid = oldGrid;
	}

	public void SetAllCellsState(CSF_CellState cSF_CellState = CSF_CellState.normal, bool isChange = false)
	{
		SetCells(isChange ? oldGrid : grid, cSF_CellState);
	}
	...
	public IEnumerator GridRollingControl(Action endAction)
	{
		...
		for (int col2 = 0; col2 < 5; col2++)
		{
			for (int row2 = 0; row2 < 3; row2++)
			{
				if (col2 == 4 && row2 == 2)
				{
					yield return StartCoroutine(Cells[row2, col2].RolllingEnd((CSF_CellType)grid[row2, col2]));
					break;
				}
				StartCoroutine(Cells[row2, col2].RolllingEnd((CSF_CellType)grid[row2, col2]));
				if (!forceStopRolling)
				{
	...
	public int CalHitResult2(CSF_MajorResult majorResult, bool isChange = false)
	{
		...
		int num2 = 0;
		num = 0;
		overall = new CSF_OverallComputer(isChange ? majorResult.oldGameContent : majorResult.gameContent);
		overall.Calculate();
		overallWin = overall.rate * lineNum * singleStake;
		UnityEngine.Debug.Log($"[overall] type: {overall.overallType}, rate: {overall.rate}");
		num += overallWin;
		winLines = new CSF_HitLineComputer[lineNum];
		winCount = 0;
		ResetHitGrid();
		lineResults = new List<CSF_WinLineResult>();
		...
		if (flag)
		{
			for (int i = 0; i < lineNum; i++)
			{
				...
				int[] betLine = GetBetLine(isChange ? majorResult.oldGameContent : majorResult.gameContent, i);
				CSF_HitLineComputer cSF_HitLineComputer = new CSF_HitLineComputer(i, betLine);
				CSF_WinLineResult[] array = cSF_HitLineComputer.Calculate2();
				cSF_WinLineResult.winType = CSF_WinLineType.Full;
				cSF_WinLineResult.lineIndex = i;
				cSF_WinLineResult.cellType = overall.cellType;
				cSF_WinLineResult.rate = overall.GetSingleLineRate();
				if (array.Length == 1)
				{
					cSF_WinLineResult.dragonNum = array[0].dragonNum;
					cSF_WinLineResult.maryTimes = array[0].maryTimes;
				}
				lineResults.Add(cSF_WinLineResult);
				SetHitGridLine(i, CSF_WinLineType.Full);
				num2 += cSF_WinLineResult.maryTimes;
				winCount++;
				list.Add(betLine);
				num += cSF_WinLineResult.rate * singleStake;
			}
		}
		else
		{
			...
			{
				int[] betLine2 = GetBetLine(isChange ? majorResult.oldGameContent : majorResult.gameContent, j);
				CSF_HitLineComputer cSF_HitLineComputer2 = new CSF_HitLineComputer(j, betLine2);
				list.Add(betLine2);
				CSF_WinLineResult[] array2 = cSF_HitLineComputer2.Calculate2();
				if (array2.Length == 2)
				{
	...
	private void PlayFullAwardAudio()
	{
		...
		CSF_SoundManager.Instance.PlayMajorGameAwardAudio(GetOverallType());
	}
	...
	public IEnumerator WinLinesAni(bool isReplay, float delay, int lineNum, int singleStake, CSF_LinesManager lineManager, CSF_BallManager ballManager, Action<int> changeScoreHandler, Action celebrateAction)
	{
		...
		if (overall.rate > 0)
		{
			...
			for (int j = 0; j < 5; j++)
			{
				for (int k = 0; k < 3; k++)
				{
					Cells[k, j].PlayState(CSF_CellState.flash);
		...
		if (winCount > 0 || overall.rate != 0)
		{
			...
			for (int l = 0; l < 5; l++)
			{
				for (int m = 0; m < 3; m++)
				{
					Cells[m, l].PlayState(CSF_CellState.gray);
				}
			}
		}
		int lineAudioIndex = 0;
		int maxRate = 0;
		CSF_CellType celltype = CSF_CellType.None;
		CSF_WinLineType winlinetype = CSF_WinLineType.None;
		for (int i = 0; i < lineResults.Count; i++)
		{
			if (lineResults[i].winType != 0)
			{
				...
				CSF_SoundManager.Instance.PlayDrawLineAudio(lineAudioIndex);
				lineAudioIndex++;
		...
		if (!isReplay && (winCount > 0 || overall.rate != 0))
		{
			...
			CSF_SoundManager.Instance.PlayMajorGameAwardAudio(celltype, CSF_HitLineComputer.CalculateHitNum(winlinetype));
			UnityEngine.Debug.LogError("中奖类型: " + celltype + " 中奖个数: " + CSF_HitLineComputer.CalculateHitNum(winlinetype));
			yield return StartCoroutine(WinCellsCelebrateAni(overall.rate != 0));
		}
		allFinishedAction();
	}

	private IEnumerator WaitHideBrightBall(CSF_BallManager ballManager, int index)
	{
		yield return new WaitForSeconds(3f);
		ballManager.HideBrightBall(index);
	}

	private void SetCells(int[,] grid, CSF_CellState cSF_CellState = CSF_CellState.normal)
	{
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				Cells[i, j].PlayState(cSF_CellState, (CSF_CellType)grid[i, j]);
			}
		}
	}
	...
	private int[,] SetGridSame(ref int[,] grid, CSF_CellType type = CSF_CellType.HuiFa)
	{
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
	...
	private void ShowSingleLineBegin(int index, CSF_WinLineType lineType)
	{
		...
		for (int i = num; i <= num2; i++)
		{
			...
			Cells[LineMap[index, i], i].PlayState(CSF_CellState.flash);
		}
	}
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				if (hitGrid[i, j] >= 3 || overall.rate > 0)
				{
					...
					Cells[i, j].PlayState(CSF_CellState.celebrate);
				}
				else
				{
					Cells[i, j].HideBorder();
					Cells[i, j].PlayState(CSF_CellState.gray);
				}
			}
		}
	}
	...
	private void ShowSingleLineEnd(int index)
	{
		for (int i = 0; i < 5; i++)
		{
			...
			Cells[LineMap[index, i], i].PlayState(CSF_CellState.gray);
		}
	}
	...
	private IEnumerator WaitForCelebrate(int num)
	{
		while (celebrateCount < num)
		{
			...
			yield return new WaitForSeconds(3f);
			coShowAllHitCell = StartCoroutine(CSF_Utils.DelayCall(0.5f, delegate
			{
	...
	public void OnOverallPlayed(string eventName)
	{
		if (overall.rate != 0)
		{
			...
			UnityEngine.Debug.Log(CSF_LogHelper.Aqua("eventName: {0}, _celebrateCount: {1}", eventName, celebrateCount));
		}
	}
	...
	private void SetHitGridLine(int lineIndex, CSF_WinLineType winType)
	{
		int num = 0;
		int num2 = 0;
		int a = 3;
