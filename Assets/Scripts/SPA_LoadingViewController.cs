...
public class SPA_LoadingViewController : SPA_MB_Singleton<SPA_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (SPA_MB_Singleton<SPA_LoadingViewController>._instance == null)
		{
			SPA_MB_Singleton<SPA_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (SPA_MB_Singleton<SPA_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!SPA_MB_Singleton<SPA_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
