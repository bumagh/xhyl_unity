...
public class BZJX_LocalData
{
	private XmlDocument mXmlDoc = new XmlDocument();

	private string mFileName = Application.persistentDataPath + "/Data.xml";

	private static BZJX_LocalData mSettedManager;

	public static BZJX_LocalData getInstance()
	{
		if (mSettedManager == null)
		{
			mSettedManager = new BZJX_LocalData();
		}
	...
	public void loadUserSetting()
	{
		...
		BZJX_GameInfo.getInstance().Setted = ReadUserSetting();
	}
	...
	{
		BZJX_SettedInfo setted = BZJX_GameInfo.getInstance().Setted;
	...
	private BZJX_SettedInfo ReadUserSetting()
	{
		BZJX_SettedInfo bZJX_SettedInfo = new BZJX_SettedInfo();
		bZJX_SettedInfo.bIsGameVolum = bool.Parse(_readData("/Data/UserSetting/GAMEVOLUM", "GameVolum"));
		bZJX_SettedInfo.bIsButtonVolum = bool.Parse(_readData("/Data/UserSetting/BUTTONVOLUM", "ButtonVolum"));
		bZJX_SettedInfo.bIsForbidPublicChat = bool.Parse(_readData("/Data/UserSetting/ISFORBIDPUBLICCHAT", "IsForbidPublicChat"));
		bZJX_SettedInfo.bIsForbidPrivateChat = bool.Parse(_readData("/Data/UserSetting/ISFORBIDPRIVATECHAT", "IsForbidPrivateChat"));
		bZJX_SettedInfo.bIsScreenNeverSleep = bool.Parse(_readData("/Data/UserSetting/ISSCREENNEVERSLEEP", "IsScreenNeverSleep"));
		return bZJX_SettedInfo;
	}
	...
	public void applySetting()
	{
		...
		BZJX_SoundManage.getInstance().IsButtonMusic = setted.bIsButtonVolum;
		if (BZJX_GameInfo.getInstance().currentState == BZJX_GameState.On_Game)
		{
			BZJX_MusicMngr.GetSingleton().SetMusicOnOff(setted.bIsGameVolum);
			BZJX_MusicMngr.GetSingleton().SetMusicFxOnOff(setted.bIsButtonVolum);
			BZJX_SoundManage.getInstance().setBgMusic(isPlay: false);
		}
		else
		{
			BZJX_SoundManage.getInstance().setBgMusic(setted.bIsGameVolum);
		}
	}
}
