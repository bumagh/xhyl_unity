...
public class PTM_CardShow
{
	...
	public PTM_CardShow(Transform tran, int idx, PTM_GamblePanel Gaml)
	{
		Tran = tran;
		index = idx;
		gaml = Gaml;
	...
	private Sprite Getsprite(int value)
	{
		try
		{
			...
			return PTM_GamblePanel.ins.GetSprite($"bjlpk{num + 1:0}");
		}
		catch (Exception message)
		{
			UnityEngine.Debug.LogError(message);
