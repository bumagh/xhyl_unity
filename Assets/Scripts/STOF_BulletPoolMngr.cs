...
public class STOF_BulletPoolMngr : MonoBehaviour
{
	public static STOF_BulletPoolMngr G_BulletPoolMngr;
	...
	public static STOF_BulletPoolMngr GetSingleton()
	{
	...
	{
		Transform transform = STOF_PoolManager.Pools["OFBulletPool"].Spawn(mBullet);
		STOF_Bullet component = transform.GetComponent<STOF_Bullet>();
		for (int i = 0; i < STOF_GameInfo.getInstance().UserList.Count; i++)
		{
			if (STOF_GameInfo.getInstance().UserList[i].SeatIndex == nPlayer)
			{
				STOF_Lock currentGun = STOF_GameInfo.getInstance().GameScene.GetCurrentGun(STOF_GameInfo.getInstance().UserList[i].SeatIndex);
				if (currentGun.lockProcess != STOF_ELOCK.MoveToOtherFish && STOF_GameInfo.getInstance().UserList[i].LockFish != null && STOF_GameInfo.getInstance().UserList[i].Lock)
				{
		...
		if (nPower <= 50 && nPower >= 1)
		{
			if (fLiziTime > 0f)
			{
				...
				component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_NORMAL_LIZI);
				STOF_MusicMngr.GetSingleton().PlayGameSound(STOF_MusicMngr.GAME_SOUND.SOUND_LIZI_SHOOT);
			}
			else
			{
				component.mIsLizi = false;
				STOF_MusicMngr.GetSingleton().PlayGameSound(STOF_MusicMngr.GAME_SOUND.SOUND_NORMAL_SHOOT);
				switch (nPlayer)
				{
				case 1:
					component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_NORMAL1);
					break;
				case 2:
					component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_NORMAL2);
					break;
				case 3:
					component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_NORMAL3);
					break;
				case 4:
					component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_NORMAL4);
					break;
				}
			}
		}
		else if (nPower < 1000 && nPower >= 51)
		{
			...
			{
				component.mIsLizi = true;
				component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_SUPER_LIZI);
				STOF_MusicMngr.GetSingleton().PlayGameSound(STOF_MusicMngr.GAME_SOUND.SOUND_LIZI_SHOOT);
			}
			else
			{
				...
				{
				case 1:
					component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_SUPER1);
					break;
				case 2:
					component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_SUPER2);
					break;
				case 3:
					component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_SUPER3);
					break;
				case 4:
					component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_SUPER4);
					break;
				}
			}
		}
		...
		{
			component.mIsLizi = true;
			component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_SUPER_LIZI4);
			STOF_MusicMngr.GetSingleton().PlayGameSound(STOF_MusicMngr.GAME_SOUND.SOUND_LIZI_SHOOT);
		}
		else
		{
			...
			{
			case 1:
				component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_SUPER4_1);
				break;
			case 2:
				component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_SUPER4_2);
				break;
			case 3:
				component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_SUPER4_3);
				break;
			case 4:
				component.SetBulletType(STOF_BULLET_TYPE.BULLET_TYP_SUPER4_4);
				break;
			}
		}
		...
		if (!isTellSvr && (nPlayer - 1) / 2 != (STOF_GameMngr.GetSingleton().mPlayerSeatID - 1) / 2)
		{
		...
		if (isTellSvr)
		{
			STOF_NetMngr.GetSingleton().MyCreateSocket.SendFired(component.mBulletID, _nPlayerIndex, fAgl, nPower, component.mIsLizi);
		}
		return transform;
	}
	...
	private void Update()
	{
		...
		if (STOF_GameParameter.G_bTest && Input.GetMouseButtonDown(0))
		{
	...
	public void DestroyBullet(GameObject bullet)
	{
		...
		STOF_PoolManager.Pools["OFBulletPool"].Despawn(bullet.transform);
	}
	...
	public void RemoveAllBullet()
	{
		...
		STOF_PoolManager.Pools["OFBulletPool"].DespawnAll();
	}
	...
	public void LockingBulletToNormal()
	{
		...
		{
			mCurrentPlayerBullet[i].GetComponent<STOF_Bullet>().IsLocking = false;
		}
	}
	...
	public void OtherLockingBulletToNormal(int player)
	{
		...
		{
			STOF_Bullet component = mOtherPlayerBullet[i].GetComponent<STOF_Bullet>();
