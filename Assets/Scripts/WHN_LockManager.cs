...
public class WHN_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {WHN_LogHelper.Key(btnKey)} value {WHN_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {WHN_LogHelper.Key(btnKey)} value {WHN_LogHelper.Key(GetValue(btnKey))}");
			}
