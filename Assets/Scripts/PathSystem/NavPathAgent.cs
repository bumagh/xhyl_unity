...
{
	[ExecuteInEditMode]
	public class NavPathAgent : BaseBehavior<FullSerializerSerializer>
	{
		[HideInInspector]
		...
		[SerializeField]
		protected Dictionary<int, PointEvent> _pointEventsListeners;

		public NavPathState state => _state;

		public event Action<NavPathAgent> OnStart;

		public event Action<NavPathAgent> OnLoop;

		public event Action<NavPathAgent> OnComplete;

		public NavPathAgent()
		{
			_state = NavPathState.Stopped;
		}
		...
		public void StartMove(Path path = null)
		{
			if (path != null)
			{
				this.path = path;
			...
			if (this.path == null)
			{
				UnityEngine.Debug.Log("NavPathAgent.StartMove failed, path is null!");
				return;
			}
			__pathPoints = this.path.GetPoints();
			if (__pathPoints == null)
			{
				UnityEngine.Debug.Log("NavPathAgent.StartMove failed, path has no point!");
				return;
			}
		...
		{
			if (_state != NavPathState.Moving)
			{
				UnityEngine.Debug.LogWarning("Current state is not State.Moving, resume failed!");
			}
			else
			{
				_state = NavPathState.Paused;
			}
		}

		public void ResumeMove()
		{
			if (_state != NavPathState.Paused)
			{
				UnityEngine.Debug.LogWarning("Current state is not State.Paused, resume failed!");
			}
			else
			{
				_state = NavPathState.Moving;
			}
		}
		...
		public void Reverse()
		{
			if (state == NavPathState.Stopped)
			{
				UnityEngine.Debug.Log("Reverse failed, current state is State.Stopped, cannot do reversing!");
			}
		}

		public void AddEventListener(int index, UnityAction<int, NavPathAgent> handler)
		{
			if (_pointEventsListeners == null)
			{
				_pointEventsListeners = new Dictionary<int, PointEvent>();
			}
			if (!_pointEventsListeners.ContainsKey(index))
			{
				_pointEventsListeners.Add(index, new PointEvent());
			}
			_pointEventsListeners[index].AddListener(handler);
		}

		public void RemoveEventListener(int index, UnityAction<int, NavPathAgent> handler)
		{
			if (_pointEventsListeners == null)
			{
				_pointEventsListeners = new Dictionary<int, PointEvent>();
