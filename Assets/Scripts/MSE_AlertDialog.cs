...
public class MSE_AlertDialog : MSE_MB_Singleton<MSE_AlertDialog>
{
	...
	private void Awake()
	{
		...
		InitFinGame();
		if (MSE_MB_Singleton<MSE_AlertDialog>._instance == null)
		{
			MSE_MB_Singleton<MSE_AlertDialog>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void ShowDialog(string content, bool showOkCancel = false, Action callback = null)
	{
		...
		MSE_MB_Singleton<MSE_GameRoot>.GetInstance().StartCoroutine(MSE_Utils.DelayCall(delay, delegate
		{
	...
	{
		isTouchForbidden = !MSE_MB_Singleton<MSE_GameManager>.GetInstance().GetTouchEnable();
		if (isTouchForbidden)
		{
			MSE_MB_Singleton<MSE_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "alert", includeEsc: false);
		}
		...
		if (MSE_MB_Singleton<MSE_OptionsController>.GetInstance() != null)
		{
			MSE_MB_Singleton<MSE_OptionsController>.GetInstance().HideAllPanel();
		}
	}
	...
	public void Hide()
	{
		...
		if (isTouchForbidden)
		{
			MSE_MB_Singleton<MSE_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "alert done, reforbidden", includeEsc: false);
		}
	}
	...
	public void OnBtnCancel_Click()
	{
		if (!MSE_GVars.tryLockOnePoint)
		{
			if (MSE_SoundManager.Instance != null)
			{
