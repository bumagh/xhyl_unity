...
public class SPA_ReconnectHint : SPA_MB_Singleton<SPA_ReconnectHint>
{
	...
	private void Awake()
	{
		if (SPA_MB_Singleton<SPA_ReconnectHint>._instance == null)
		{
			SPA_MB_Singleton<SPA_ReconnectHint>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void Show()
	{
		if (!_goContainer.activeSelf)
		{
			...
			UnityEngine.Debug.Log(SPA_LogHelper.Magenta("ReconnectHint Show"));
	...
	public void Hide()
	{
		UnityEngine.Debug.Log(SPA_LogHelper.Magenta("ReconnectHint Hide"));
		SPA_LockManager.UnLock("Esc");
	...
	private IEnumerator _textInfoAni()
	{
		...
		while (true)
		{
			string prefix = (SPA_GVars.language == "zh") ? "网络断开，正在重连 " : "Reconnecting to the network ";
