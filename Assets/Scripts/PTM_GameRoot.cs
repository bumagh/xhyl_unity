...
public class PTM_GameRoot : PTM_MB_Singleton<PTM_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(PTM_MB_Singleton<PTM_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            PTM_MB_Singleton<PTM_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : PTM_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : PTM_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
