...
public class GOReplacer : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		stopwatch.Stop();
		UnityEngine.Debug.Log(HW2_LogHelper.Orange($"加载耗时 in Awake：{stopwatch.Elapsed.TotalSeconds}"));
	}

	private void Start()
	{
		_stopwatch.Stop();
		UnityEngine.Debug.Log(HW2_LogHelper.Orange($"加载耗时 Awake to Start：{_stopwatch.Elapsed.TotalSeconds}"));
