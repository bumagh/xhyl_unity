...
public class SHT_Gcore : Cinstance<SHT_Gcore>
{
	...
	public void Init()
	{
		try
		{
			...
			{
				string text = string.Format("Ele{0}_02", i.ToString("00"));
			...
			{
				List<int> item = SHT_LinePosIdx.Getwinlinepos(j, 5);
				AllLinePos.Add(item);
			}
	...
	{
		IsRoll = false;
		SHT_MB_Singleton<SHT_MajorGameController>.GetInstance().AllStop();
	}
	...
	public void QuickStop()
	{
		if (IsRoll)
		{
			SHT_MB_Singleton<SHT_MajorGameController>.GetInstance().StopRoll();
			IsRoll = false;
		}
	}
	...
	{
		IsRoll = true;
		SHT_MB_Singleton<SHT_MajorGameController>.GetInstance().StartRollSym();
	}
