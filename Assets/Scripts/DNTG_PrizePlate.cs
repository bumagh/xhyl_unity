...
public class DNTG_PrizePlate : MonoBehaviour
{
	...
	public void ShowScore(int nNum, Vector3 playerPos, int playerID)
	{
		...
		imgPlate.color = new Color(1f, 1f, 1f, 0f);
		imgPlate.DOFade(1f, 0.25f).OnComplete(delegate
		{
			SetMaterial();
		});
		...
		base.transform.DOMove(playerPos + vector * 1.9f, 0.35f);
	...
	{
		DNTG_EffectMngr.GetSingleton().DestroyEffectObj(base.gameObject);
	}
