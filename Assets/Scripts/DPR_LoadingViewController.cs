...
public class DPR_LoadingViewController : DPR_MB_Singleton<DPR_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (DPR_MB_Singleton<DPR_LoadingViewController>._instance == null)
		{
			DPR_MB_Singleton<DPR_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (DPR_MB_Singleton<DPR_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public void Show()
	{
		...
		DPR_MySqlConnection.curView = "LoadingView";
	}
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!DPR_MB_Singleton<DPR_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
	...
	{
		percent += DPR_MySqlConnection.RandomTime;
		if (percent >= 1f)
		{
			percent = 1f;
		}
