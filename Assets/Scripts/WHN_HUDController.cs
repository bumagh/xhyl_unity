...
public class WHN_HUDController : WHN_MB_Singleton<WHN_HUDController>
{
	...
	private void Awake()
	{
		...
		if (WHN_MB_Singleton<WHN_HUDController>._instance == null)
		{
			WHN_MB_Singleton<WHN_HUDController>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void PreInit()
	{
		...
		{
			ruleController = base.transform.Find("Canvas/Mask/Rule").GetComponent<WHN_RulePicController>();
		}
	}

	public void Init()
	{
		WHN_MB_Singleton<WHN_OptionsController>.GetInstance().onItemRules = delegate
		{
			ruleController.Show();
		};
		ruleController.Hide();
	...
	{
		ping = new Ping(WHN_GVars.IPAddress);
		if (waitPing != null)
		{
	...
	public void OnBtnOptions_Click()
	{
		...
		{
			if (WHN_MB_Singleton<WHN_OptionsController>.GetInstance().isShow)
			{
