...
public class BCBM_All_DaSanYuan : MonoBehaviour
{
	...
	private void Start()
	{
		...
		for (int i = 0; i < mAnimalArr.Length; i++)
		{
			...
			mAnimalArr[i].GetComponent<BCBM_AnimalScript>().SetAnimation(BCBM_RoleAnimationScript.AnimationType.AT_STOP);
		}
		_isFirstRun = true;
	}
	...
	public void ShowOneAnimal(int index)
	{
		...
		mAnimalArr[index].GetComponent<BCBM_AnimalScript>().SetAnimalActionState(Animal_Action_State.Animal_Play_Win);
	}
