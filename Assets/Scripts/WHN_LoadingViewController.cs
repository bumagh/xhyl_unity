...
public class WHN_LoadingViewController : WHN_MB_Singleton<WHN_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (WHN_MB_Singleton<WHN_LoadingViewController>._instance == null)
		{
			WHN_MB_Singleton<WHN_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (WHN_MB_Singleton<WHN_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!WHN_MB_Singleton<WHN_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
