...
public class PHG_LoadingViewController : PHG_MB_Singleton<PHG_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (PHG_MB_Singleton<PHG_LoadingViewController>._instance == null)
		{
			PHG_MB_Singleton<PHG_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (PHG_MB_Singleton<PHG_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!PHG_MB_Singleton<PHG_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
