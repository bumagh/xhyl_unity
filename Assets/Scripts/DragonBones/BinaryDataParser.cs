...
namespace DragonBones
{
	public class BinaryDataParser : ObjectDataParser
	{
		...
		protected override void _ParseArray(Dictionary<string, object> rawData)
		{
			...
			short[] intArrayBuffer = Array.Empty<short>();
			float[] floatArrayBuffer = Array.Empty<float>();
			short[] frameIntArrayBuffer = Array.Empty<short>();
			float[] frameFloatArrayBuffer = Array.Empty<float>();
			short[] frameArrayBuffer = Array.Empty<short>();
			ushort[] timelineArrayBuffer = Array.Empty<ushort>();
