...
public class BCBM_PrizeResult : MonoBehaviour
{
	...
	protected BCBM_PersonInfo mUserSelf;
	...
	protected BCBM_HudManager mHudManager;

	protected BCBM_BetPanel mBet;
	...
	private void Start()
	{
		...
		mUserSelf = BCBM_GameInfo.getInstance().UserInfo;
		...
		mHudManager = GameObject.Find("HudPanel").GetComponent<BCBM_HudManager>();
		mBet = GameObject.Find("BetPanel").GetComponent<BCBM_BetPanel>();
	...
	public void ShowPrizeResult()
	{
		...
		BCBM_NetMngr.GetSingleton().MyCreateSocket.SendResultList(mUserSelf.TableId);
	...
	public int SetAnimalResult(int[] iAnimalID, int nLength = 1, int nJpPower = 0)
	{
		...
		{
			BCBM_ErrorManager.GetSingleton().AddError("Error:中奖结果错误length = " + nLength);
			return 1;
		}
		for (int i = 0; i < nLength; i++)
		{
			...
			{
				BCBM_ErrorManager.GetSingleton().AddError("Error:中奖动物错误:" + iAnimalID[i]);
				return 2;
			}
		}
		_resetResult();
	...
	public int SetLuckyJP(int iAnimalID, int iLuckyMachine, int iLuckyAnimalID, int nJPPower)
	{
		...
		{
			BCBM_ErrorManager.GetSingleton().AddError("Error:中奖动物错误" + iAnimalID);
			BCBM_ErrorManager.GetSingleton().AddError("Error:中奖动物错误" + iLuckyAnimalID);
			return 1;
		}
		if (iLuckyMachine > 8 || iLuckyMachine < 1)
		{
			BCBM_ErrorManager.GetSingleton().AddError("Error:中奖分机错误" + iLuckyMachine);
			return 2;
		}
		if (nJPPower > 5 || nJPPower < 2)
		{
			BCBM_ErrorManager.GetSingleton().AddError("Error:闪电倍率错误" + nJPPower);
			return 3;
		}
		_resetResult();
	...
	public int SetLuckyTimes(int iAnimalID, int iLuckyMachine, int[] iLuckyAnimalID, int nLength)
	{
		...
		for (int i = 0; i < nLength; i++)
		{
			...
			{
				BCBM_ErrorManager.GetSingleton().AddError("Error:中奖动物错误" + iLuckyAnimalID[i]);
				return 4;
			}
		}
		_resetResult();
