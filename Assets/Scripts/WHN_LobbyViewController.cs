...
public class WHN_LobbyViewController : WHN_MB_Singleton<WHN_LobbyViewController>
{
	...
	private WHN_HeadViewController _headViewController;

	private WHN_UserViewer _userViewer;
	...
	private List<WHN_Desk> _deskList = new List<WHN_Desk>();
	...
	private WHN_DeskWidget[] _deskWidgets = new WHN_DeskWidget[3];
	...
	private void Awake()
	{
		...
		if (WHN_MB_Singleton<WHN_LobbyViewController>._instance == null)
		{
			WHN_MB_Singleton<WHN_LobbyViewController>.SetInstance(this);
			PreInit();
		}
	}
	...
	private void InitFinGame()
	{
		...
		_headViewController = base.transform.Find("Title").GetComponent<WHN_HeadViewController>();
		_userViewer = base.transform.Find("PersonInfoDialog").GetComponent<WHN_UserViewer>();
	...
	private void Update()
	{
		if (UnityEngine.Input.GetKeyDown(KeyCode.Escape) && !WHN_MB_Singleton<WHN_LobbyViewController>.GetInstance()._IsAning && !WHN_LockManager.IsLocked("Esc"))
		{
			UnityEngine.Debug.Log("ESC Btn Down In Lobby View");
			WHN_MB_Singleton<WHN_GameManager>.GetInstance().Handle_BtnReturn();
		}
	}
	...
	private void _init()
	{
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().RegisterHandler("roomInfo", HandleNetMsg_RoomInfo);
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().RegisterHandler("enterDesk", HandleNetMsg_EnterDesk);
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().RegisterHandler("updateRoomInfo", HandleNetMsg_UpdateRoomInfo);
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().RegisterHandler("addExpeGoldAuto", HandleNetMsg_AddExpeGoldAuto);
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().RegisterHandler("expeGold", HandleNetMsg_ExpeGold);
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().RegisterHandler("playerInfo", HandleNetMsg_PlayerInfo);
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().RegisterHandler("updateDeskInfo", HandleNetMsg_UpdateDeskInfo);
		_goRoomView.SetActive(value: true);
		_goDeskView.SetActive(value: false);
		int num = (WHN_GVars.language == "en") ? 1 : 0;
		...
		WHN_Drag_DeskController._instance.left = OnBtnLeftDesk_Click;
		WHN_Drag_DeskController._instance.right = OnBtnRightDesk_Click;
	...
	public void Show()
	{
		...
		WHN_MB_Singleton<WHN_GameManager>.GetInstance().ChangeView(empty);
	...
	{
		if (!WHN_GVars.tryLockOnePoint && WHN_DeskPullDownListController.Instance.bMoveFinished)
		{
			_desksMoveToLeft();
		}
	}
	...
	{
		if (!WHN_GVars.tryLockOnePoint && _bCanClick)
		{
			_bCanClick = false;
			WHN_SoundManager.Instance.PlayClickAudio();
	...
	public void Send_EnterRoom(int roomType)
	{
		...
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().Send("userService/enterRoom", args);
		WHN_MB_Singleton<WHN_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "Send_EnterRoom");
	}
	...
	public void Send_EnterDesk(int deskId)
	{
		...
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().Send("userService/enterDesk", args);
		WHN_MB_Singleton<WHN_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "Send_EnterDesk");
	}
	...
	public void Send_LeaveRoom()
	{
		...
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().Send("userService/leaveRoom", args);
	}

	public void Send_GetUserAward()
	{
		object[] args = new object[0];
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().Send("userService/getUserAward", args);
	}

	public void Send_AddExpeGoldAuto()
	{
		object[] args = new object[0];
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().Send("userService/addExpeGoldAuto", args);
	}
	...
	public void Send_PlayerInfo(int userId)
	{
		...
		WHN_MB_Singleton<WHN_NetManager>.GetInstance().Send("userService/playerInfo", args);
	}

	public void HandleNetMsg_RoomInfo(object[] args)
	{
		UnityEngine.Debug.Log(WHN_LogHelper.NetHandle("HandleNetMsg_RoomInfo"));
		if (WHN_GVars.curView == "MajorGame" || WHN_GVars.curView == "DiceGame")
		{
			UnityEngine.Debug.Log(WHN_LogHelper.Orange("游戏中，不处理"));
			return;
		}
		_curState = "DeskView";
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
		WHN_MB_Singleton<WHN_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "Send_EnterRoom done");
		if (dictionary != null)
		{
			WHN_LockManager.UnLock("btn_room");
			UnityEngine.Debug.Log("isOverflow: " + dictionary["isOverflow"]);
			WHN_MB_Singleton<WHN_AlertDialog>.GetInstance().ShowDialog((WHN_GVars.language == "zh") ? "您的账户已爆机，请兑奖" : "Your account is blasting, please exchange");
			return;
		}
		object[] array = args[0] as object[];
		List<WHN_Desk> list = new List<WHN_Desk>();
		for (int i = 0; i < array.Length; i++)
		{
			Dictionary<string, object> data = array[i] as Dictionary<string, object>;
			list.Add(WHN_Desk.CreateWithDic(data));
		}
		_deskList = list;
	...
	{
		UnityEngine.Debug.Log(WHN_LogHelper.NetHandle("HandleNetMsg_EnterDesk"));
		WHN_LockManager.UnLock("EnterDesk");
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
		bool flag = (bool)dictionary["success"];
		WHN_MB_Singleton<WHN_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "Send_EnterDesk done");
		UnityEngine.Debug.LogError("flag: " + flag);
		if (flag)
		{
			Hide();
			UnityEngine.Debug.LogError("curDeskIndex: " + curDeskIndex);
			WHN_GVars.desk = _deskList[curDeskIndex];
			WHN_MB_Singleton<WHN_MajorGameController>.GetInstance().Show();
			WHN_MB_Singleton<WHN_MajorGameController>.GetInstance().InitGame();
			WHN_MB_Singleton<WHN_ScoreBank>.GetInstance().SetKeepScore(0);
			WHN_MB_Singleton<WHN_ScoreBank>.GetInstance().SetRate(WHN_GVars.desk.exchange);
			return;
		}
		switch ((int)dictionary["messageStatus"])
		{
		case 0:
			WHN_MB_Singleton<WHN_AlertDialog>.GetInstance().ShowDialog((WHN_GVars.language == "zh") ? "桌子不存在" : "Game table not exist");
			break;
		case 1:
			WHN_MB_Singleton<WHN_AlertDialog>.GetInstance().ShowDialog((WHN_GVars.language == "zh") ? "桌已满" : "Don't Have Free Table");
			break;
		case 2:
			WHN_MB_Singleton<WHN_AlertDialog>.GetInstance().ShowDialog((WHN_GVars.language == "zh") ? "您的账户已爆机，请兑奖" : "Your account is blasting, please excharge");
			break;
		case 3:
			WHN_MB_Singleton<WHN_AlertDialog>.GetInstance().ShowDialog((WHN_GVars.language == "zh") ? "体验币不足" : "Experience Coins insufficient");
			break;
		case 4:
			WHN_MB_Singleton<WHN_AlertDialog>.GetInstance().ShowDialog((WHN_GVars.language == "zh") ? "游戏币不足" : "Coins insufficient");
			break;
		default:
	...
	{
		UnityEngine.Debug.Log(WHN_LogHelper.NetHandle("HandleNetMsg_UpdateRoomInfo"));
		...
		if (count != list.Count)
		{
			...
			WHN_DeskPullDownListController.Instance.UpdateDeskInList(_deskList);
		}
		else
		{
			UpdateAllDeskWidgets();
	...
	public void HandleNetMsg_AddExpeGoldAuto(object[] args)
	{
		...
		WHN_MB_Singleton<WHN_AlertDialog>.GetInstance().ShowDialog("申请成功！请返回大厅重新游戏", showOkCancel: false, delegate
		{
	...
	{
		UnityEngine.Debug.Log(WHN_LogHelper.NetHandle("HandleNetMsg_ExpeGold"));
		if ((bool)args[0])
		{
			WHN_GVars.user.expeGold = 10000;
	...
	{
		UnityEngine.Debug.Log(WHN_LogHelper.NetHandle("HandleNetMsg_PlayerInfo"));
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
	...
	{
		UnityEngine.Debug.Log(WHN_LogHelper.NetHandle("HandleNetMsg_UpdateDeskInfo"));
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
	...
	private IEnumerator _showDeskViewAni()
	{
		...
		WHN_GVars.curView = "DeskSelectionView";
		WHN_LockManager.UnLock("btn_room");
		yield return null;
	}

	private WHN_DeskWidget _createDeskWidget()
	{
		...
		WHN_DeskWidget component = gameObject.GetComponent<WHN_DeskWidget>();
	...
	{
		WHN_DeskPullDownListController.Instance.bMoveFinished = false;
		WHN_SoundManager.Instance.PlayClickAudio();
		if (_deskList.Count != 0 && !_inputForbidden)
		{
			WHN_DeskPullDownListController.Instance.ShowListButtonHighBG(_bIsLeft: true, curDeskIndex);
			...
			sequence.onComplete = delegate
			{
				...
				WHN_DeskPullDownListController.Instance.bMoveFinished = true;
				WHN_DeskPointsController._Instance.ShowDeskPoint2(_deskList.Count, curDeskIndex);
			};
		}
	}
	...
	private void _desksMoveToRight()
	{
		...
		if (_deskList.Count != 0 && !_inputForbidden)
		{
			WHN_DeskPullDownListController.Instance.ShowListButtonHighBG(_bIsLeft: false, curDeskIndex);
	...
	{
		WHN_DeskWidget wHN_DeskWidget = _deskWidgets[a];
		_deskWidgets[a] = _deskWidgets[b];
		_deskWidgets[b] = wHN_DeskWidget;
	}
	...
	public void UpdateAllDeskWidgets()
	{
		...
		WHN_DeskPointsController._Instance.ShowDeskPoint2(_deskList.Count, curDeskIndex);
		WHN_DeskPullDownListController.Instance.ChangeListHeightByDeskNum(_deskList.Count, _deskList);
	}
	...
	private void _onBtnDesk_Click()
	{
		...
		{
			if (WHN_GVars.user.expeGold == 0 || WHN_GVars.user.expeGold < _deskList[curDeskIndex].minGold)
			{
				UnityEngine.Debug.Log($"expeGold: {WHN_GVars.user.expeGold}, _curDeskIndex: {curDeskIndex}, minGold: {_deskList[curDeskIndex].minGold}");
				WHN_MB_Singleton<WHN_AlertDialog>.GetInstance().ShowDialog("体验币不足，是否领取体验币？", showOkCancel: true, delegate
				{
			...
			{
				WHN_LockManager.Lock("EnterDesk");
				Send_EnterDesk(_deskList[curDeskIndex].id);
				WHN_SoundManager.Instance.StopLobbyBGM();
			}
		}
		else if (WHN_GVars.user.gameGold == 0 || WHN_GVars.user.gameGold < _deskList[curDeskIndex].minGold)
		{
			UnityEngine.Debug.LogError($"gameGold: {WHN_GVars.user.gameGold}, _curDeskIndex: {curDeskIndex}, minGold: {_deskList[curDeskIndex].minGold}");
			WHN_MB_Singleton<WHN_AlertDialog>.GetInstance().ShowDialog("游戏币不足！");
		}
		else
		{
			WHN_LockManager.Lock("EnterDesk");
	...
	{
		string info = (WHN_GVars.language == "zh") ? "选择房间" : "SelectRoom";
		if (_curState == "DeskView")
		{
			if (_curRoomType == 1)
			{
				info = ((WHN_GVars.language == "zh") ? "新手练习厅" : "Training");
			}
			else if (_curRoomType == 2)
			{
				info = ((WHN_GVars.language == "zh") ? "欢乐竞技厅" : "Arena");
			}
		}
	...
	{
		_IsAning = true;
		WHN_MB_Singleton<WHN_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "_hideRoomViewAni");
		Sequence sequence = DOTween.Sequence();
		...
		sequence.onComplete = delegate
		{
			WHN_MB_Singleton<WHN_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "_hideRoomViewAni done");
			_IsAning = false;
			_bCanClick = true;
		};
	}
	...
	public void ResetRoomView()
	{
		...
		WHN_MB_Singleton<WHN_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "ResetRoomView");
		_userViewer.gameObject.SetActive(value: false);
		Sequence sequence = DOTween.Sequence();
		...
		sequence.onComplete = delegate
		{
			...
			_bCanClick = false;
			_updateHeadInfo();
			WHN_MB_Singleton<WHN_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "ResetRoomView_Over");
		};
	}
	...
	public void HideDeskViewAni()
	{
		...
		sequence.onComplete = delegate
		{
			WHN_MB_Singleton<WHN_DeskSelectionViewController>.GetInstance().Hide();
		};
	}
}
