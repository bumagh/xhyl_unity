...
public class PHG_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {PHG_LogHelper.Key(btnKey)} value {PHG_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {PHG_LogHelper.Key(btnKey)} value {PHG_LogHelper.Key(GetValue(btnKey))}");
			}
