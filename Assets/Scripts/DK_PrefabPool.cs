...
public class DK_PrefabPool
{
	...
	public DK_PrefabPool(Transform prefab)
	{
		this.prefab = prefab;
		prefabGO = prefab.gameObject;
	}

	public DK_PrefabPool()
	{
	}
	...
	internal bool DespawnInstance(Transform xform)
	{
		...
		DK_PoolManagerUtils.SetActive(xform.gameObject, state: false);
		if (xform.gameObject.CompareTag("BigFishEffect"))
		{
			xform.gameObject.SetActive(value: true);
		}
	...
	internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
	{
		...
		else
		{
			...
			DK_PoolManagerUtils.SetActive(transform2.gameObject, state: true);
		}
		if (transform2 != null)
		{
	...
	internal void AddUnpooled(Transform inst, bool despawn)
	{
		...
		if (despawn)
		{
			DK_PoolManagerUtils.SetActive(inst.gameObject, state: false);
			_despawned.Add(inst);
		}
		else
		{
