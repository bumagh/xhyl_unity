...
public class CRL_GameRoot : CRL_MB_Singleton<CRL_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(CRL_MB_Singleton<CRL_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            CRL_MB_Singleton<CRL_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : CRL_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : CRL_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
