...
public class STQM_BulletPoolMngr : MonoBehaviour
{
	public static STQM_BulletPoolMngr G_BulletPoolMngr;
	...
	public STQM_GameScene _gameScene;
	...
	public static STQM_BulletPoolMngr GetSingleton()
	{
	...
	private void Awake()
	{
		...
		_gameScene = GetComponent<STQM_GameScene>();
	}
	...
	{
		Transform transform = STQM_PoolManager.Pools["BulletPool"].Spawn(mBullet);
		STQM_Bullet component = transform.GetComponent<STQM_Bullet>();
		for (int i = 0; i < STQM_GameInfo.getInstance().UserList.Count; i++)
		{
			if (STQM_GameInfo.getInstance().UserList[i].SeatIndex == nPlayer)
			{
				STQM_Lock currentGun = STQM_GameInfo.getInstance().GameScene.GetCurrentGun(STQM_GameInfo.getInstance().UserList[i].SeatIndex);
				if (currentGun.lockProcess != STQM_ELOCK.MoveToOtherFish && STQM_GameInfo.getInstance().UserList[i].LockFish != null && STQM_GameInfo.getInstance().UserList[i].Lock)
				{
		...
		if (nPower <= 50 && nPower >= 1)
		{
			if (fLiziTime > 0f)
			{
				...
				component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_NORMAL_LIZI);
				STQM_MusicMngr.GetSingleton().PlayGameSound(STQM_MusicMngr.GAME_SOUND.SOUND_LIZI_SHOOT);
			}
			else
			{
				component.mIsLizi = false;
				STQM_MusicMngr.GetSingleton().PlayGameSound(STQM_MusicMngr.GAME_SOUND.SOUND_NORMAL_SHOOT);
				switch (nPlayer)
				{
				case 1:
					component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_NORMAL1);
					break;
				case 2:
					component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_NORMAL2);
					break;
				case 3:
					component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_NORMAL3);
					break;
				case 4:
					component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_NORMAL4);
					break;
				}
			}
		}
		else if (nPower < 1000 && nPower >= 51)
		{
			...
			{
				component.mIsLizi = true;
				component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_SUPER_LIZI);
				STQM_MusicMngr.GetSingleton().PlayGameSound(STQM_MusicMngr.GAME_SOUND.SOUND_LIZI_SHOOT);
			}
			else
			{
				...
				{
				case 1:
					component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_SUPER1);
					break;
				case 2:
					component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_SUPER2);
					break;
				case 3:
					component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_SUPER3);
					break;
				case 4:
					component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_SUPER4);
					break;
				}
			}
		}
		...
		{
			component.mIsLizi = true;
			component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_SUPER_LIZI4);
			STQM_MusicMngr.GetSingleton().PlayGameSound(STQM_MusicMngr.GAME_SOUND.SOUND_LIZI_SHOOT);
		}
		else
		{
			...
			{
			case 1:
				component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_SUPER4_1);
				break;
			case 2:
				component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_SUPER4_2);
				break;
			case 3:
				component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_SUPER4_3);
				break;
			case 4:
				component.SetBulletType(STQM_BULLET_TYPE.BULLET_TYP_SUPER4_4);
				break;
			}
		}
		...
		if (isTellSvr)
		{
			STQM_NetMngr.GetSingleton().MyCreateSocket.SendFired(component.mBulletID, _nPlayerIndex, fAgl, nPower, component.mIsLizi);
		}
		return transform;
	}
	...
	private void Update()
	{
		...
		if (STQM_GameParameter.G_bTest && Input.GetMouseButtonDown(0))
		{
	...
	public void DestroyBullet(GameObject bullet)
	{
		...
		STQM_PoolManager.Pools["BulletPool"].Despawn(bullet.transform);
	}
	...
	public void Clear()
	{
		...
		STQM_PoolManager.Pools["BulletPool"].DespawnAll();
		mCurrentPlayerBullet.Clear();
		mOtherPlayerBullet.Clear();
		G_BulletPoolMngr = null;
	}
	...
	public void LockingBulletToNormal()
	{
		...
		{
			mCurrentPlayerBullet[i].GetComponent<STQM_Bullet>().IsLocking = false;
		}
	}
	...
	public void OtherLockingBulletToNormal(int player)
	{
		...
		{
			STQM_Bullet component = mOtherPlayerBullet[i].GetComponent<STQM_Bullet>();
