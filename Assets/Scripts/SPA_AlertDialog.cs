...
public class SPA_AlertDialog : SPA_MB_Singleton<SPA_AlertDialog>
{
	...
	private void Awake()
	{
		...
		InitFinGame();
		if (SPA_MB_Singleton<SPA_AlertDialog>._instance == null)
		{
			SPA_MB_Singleton<SPA_AlertDialog>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void ShowDialog(string content, bool showOkCancel = false, Action callback = null)
	{
		...
		SPA_MB_Singleton<SPA_GameRoot>.GetInstance().StartCoroutine(SPA_Utils.DelayCall(delay, delegate
		{
	...
	{
		isTouchForbidden = !SPA_MB_Singleton<SPA_GameManager>.GetInstance().GetTouchEnable();
		if (isTouchForbidden)
		{
			SPA_MB_Singleton<SPA_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "alert", includeEsc: false);
		}
		...
		if (SPA_MB_Singleton<SPA_OptionsController>.GetInstance() != null)
		{
	...
	public void Hide()
	{
		...
		if (isTouchForbidden)
		{
			SPA_MB_Singleton<SPA_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "alert done, reforbidden", includeEsc: false);
		}
	}
	...
	public void OnBtnCancel_Click()
	{
		if (!SPA_GVars.tryLockOnePoint)
		{
			if (SPA_SoundManager.Instance != null)
			{
