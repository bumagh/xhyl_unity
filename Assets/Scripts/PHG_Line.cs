...
public class PHG_Line : MonoBehaviour
{
	private List<PHG_SymbolC> iconlist;
	...
	{
		result = new List<int>();
		iconlist = new List<PHG_SymbolC>();
		Rollsymnum = new List<int>();
		RandomIco = Cinstance<PHG_Gcore>.Instance.Symcont;
	...
	private void Initline()
	{
		...
		try
		{
			while (enumerator.MoveNext())
			{
				...
				component.Init(symid: (num > 2) ? UnityEngine.Random.Range(0, 6) : Cinstance<PHG_Gcore>.Instance.Normallist[index], idx: num++, line: this);
		...
		if (!stopline)
		{
			try
			{
				...
				if (LineIdx == 4)
				{
					...
					t.SetDelay(1f).AppendCallback(Cinstance<PHG_Gcore>.Instance.AllStop);
				}
			}
			catch (Exception arg)
			{
