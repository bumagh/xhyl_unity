...
public class SHT_NetManager : SHT_MB_Singleton<SHT_NetManager>
{
	public delegate void NetMessageHandler(object[] args);

	private SHT_ClientSocket _pClient;

	private SHT_DataEncrypt _pCrypto;
	...
	private Queue<SHT_NetMessage> _netMsgQueue = new Queue<SHT_NetMessage>();

	private Dictionary<string, NetMessageHandler> _netMsgHandlerDic = new Dictionary<string, NetMessageHandler>();

	private void Awake()
	{
		if (SHT_MB_Singleton<SHT_NetManager>._instance == null)
		{
			SHT_MB_Singleton<SHT_NetManager>.SetInstance(this);
		}
	...
	private void _initClientSocket()
	{
		...
		_pClient = new SHT_ClientSocket(useTcpClient: true);
		_pCrypto = new SHT_DataEncrypt();
	...
	private void _handleException(Exception ex)
	{
		...
		isReady = false;
		isLogined = false;
		Queue<SHT_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
			_netMsgQueue.Enqueue(new SHT_NetMessage("netDown", new object[1]
			{
				ex
			}));
		}
	...
	private void _handleConnected()
	{
		try
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new SHT_NetMessage("netConnected", new object[0]));
			}
		}
		catch (Exception ex)
		{
	...
	{
		UnityEngine.Debug.Log("ZH2_GVars.versionCode: " + SHT_GVars.versionCode);
		Send("userService/checkVersion", new object[1]
		{
			SHT_GVars.versionCode
		});
	}
	...
	protected void _handleData(byte[] msgBuffer, int packetId, int endIndex)
	{
		...
		lock (netMsgQueue)
		{
			SHT_NetMessage sHT_NetMessage = new SHT_NetMessage(text, args);
			sHT_NetMessage.jsonString = @string;
			sHT_NetMessage.packetId = packetId;
			sHT_NetMessage.endIndex = endIndex;
			_netMsgQueue.Enqueue(sHT_NetMessage);
		}
	}

	private IEnumerator _checkTimeout(int id, int timeout, SHT_ClientSocket cs)
	{
		UnityEngine.Debug.Log(SHT_LogHelper.Aqua("_checkTimeout start id: {0}", id));
		...
		if (cs != null && cs.doneList.Count > id && !cs.doneList[id])
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new SHT_NetMessage("timeout", new object[1]
				{
					id
				}));
				isTimeout = true;
		...
		if (isTimeout)
		{
			UnityEngine.Debug.Log(SHT_LogHelper.Aqua("_checkTimeout start id: {0} timeout: {1}", id, isTimeout));
		}
	}
	...
	private IEnumerator _checkHeartTimeout(int timeout)
	{
		...
		if (isReady && !isHeartPong)
		{
			...
			lock (netMsgQueue)
			{
				...
				isReady = false;
				isLogined = false;
				_netMsgQueue.Enqueue(new SHT_NetMessage("heart timeout", new object[0]));
				isTimeout = true;
			}
		}
		if (isTimeout)
		{
			UnityEngine.Debug.Log(SHT_LogHelper.Aqua("_checkHeartTimeout is timeout"));
		}
	}

	public void _checkTimeoutStart(int id, int timeout, SHT_ClientSocket cs)
	{
	...
	public void Send(string method, object[] args)
	{
		...
		{
			UnityEngine.Debug.Log(SHT_LogHelper.Magenta("drop send>>method: {0}", method));
			return;
		}
		Hashtable hashtable = new Hashtable();
		hashtable.Add("method", method);
		hashtable.Add("args", args);
		hashtable.Add("version", SHT_GVars.Version);
		if (isReady)
		{
	...
	private bool GetNetMessage(out SHT_NetMessage netMsg)
	{
		Queue<SHT_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
	...
	private void DispatchNetMessage(SHT_NetMessage netMsg)
	{
		if (dropDispatch)
		{
			UnityEngine.Debug.Log(SHT_LogHelper.Magenta("drop send>>method: {0}", netMsg.method));
		}
