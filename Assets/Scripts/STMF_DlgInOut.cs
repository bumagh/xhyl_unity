...
public class STMF_DlgInOut : MonoBehaviour
{
	[SerializeField]
	...
	private STMF_ErrorTipAnim sptErrorTipAnim;
	...
	private void GetAndInitCompenent()
	{
		...
		sptErrorTipAnim = imgError.GetComponent<STMF_ErrorTipAnim>();
	...
	private void ClickBtnOut()
	{
		...
		{
			STMF_NetMngr.GetSingleton().MyCreateSocket.SendCoinOut(gameInfo.Table.DeltaCoin * gameInfo.Table.ScorePerCoin);
			STMF_SoundManage.getInstance().playButtonMusic(STMF_ButtonMusicType.coinUp);
			return;
		}
		int scoreCount = gameInfo.User.ScoreCount;
		if (scoreCount > 0)
		{
			STMF_NetMngr.GetSingleton().MyCreateSocket.SendCoinOut(scoreCount);
			STMF_SoundManage.getInstance().playButtonMusic(STMF_ButtonMusicType.coinUp);
		}
		else
		{
