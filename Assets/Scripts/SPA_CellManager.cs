...
public class SPA_CellManager : MonoBehaviour
{
	public static SPA_CellManager cellManager;
	...
	private SPA_OverallComputer overall;

	private SPA_HitLineComputer[] winLines;
	...
	private List<SPA_WinLineResult> lineResults;
	...
	{
		List<int> showchange = SPA_MB_Singleton<SPA_MajorGameController>.GetInstance().Showchange1;
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				...
				{
					Cells[i, j].PlayState(SPA_CellState.normal, SPA_CellType.JiuHu);
				}
			}
		}
	}
	...
	public void PreInit()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				Cells[i, j] = base.transform.Find($"Cell{i}_{j}").GetComponent<SPA_Cell>();
			}
		}
	}
	...
	public IEnumerator GridRollingControl(Action endAction)
	{
		...
		for (int col2 = 0; col2 < 5; col2++)
		{
			for (int row2 = 0; row2 < 3; row2++)
			{
				if (col2 == 4 && row2 == 2)
				{
					yield return StartCoroutine(Cells[row2, col2].RolllingEnd((SPA_CellType)grid[row2, col2]));
					break;
				}
				StartCoroutine(Cells[row2, col2].RolllingEnd((SPA_CellType)grid[row2, col2]));
				if (!forceStopRolling)
				{
	...
	public int CalHitResult2(SPA_MajorResult majorResult, bool isChange = false)
	{
		...
		int num2 = 0;
		num = 0;
		overall = new SPA_OverallComputer(majorResult.gameContent);
		overall.Calculate();
		overallWin = overall.rate * lineNum * singleStake;
		UnityEngine.Debug.Log($"[overall] type: {overall.overallType}, rate: {overall.rate}");
		num += overallWin;
		winLines = new SPA_HitLineComputer[lineNum];
		winCount = 0;
		ResetHitGrid();
		lineResults = new List<SPA_WinLineResult>();
		List<int[]> list = new List<int[]>();
		for (int i = 0; i < lineNum; i++)
		{
			int[] betLine = GetBetLine(majorResult.gameContent, i);
			SPA_HitLineComputer sPA_HitLineComputer = new SPA_HitLineComputer(i, betLine);
			list.Add(betLine);
			SPA_WinLineResult[] array = sPA_HitLineComputer.Calculate2();
			if (array.Length == 2)
			{
	...
	private void PlayFullAwardAudio()
	{
		...
		SPA_SoundManager.Instance.PlayMajorGameAwardAudio(GetOverallType());
	}
	...
	public IEnumerator WinLinesAni(bool isReplay, float delay, int lineNum, int singleStake, SPA_LinesManager lineManager, SPA_BallManager ballManager, Action<int> changeScoreHandler, Action celebrateAction)
	{
		...
		if (overall.rate > 0)
		{
			...
			for (int j = 0; j < 5; j++)
			{
				for (int k = 0; k < 3; k++)
				{
					Cells[k, j].PlayState(SPA_CellState.flash);
		...
		if (winCount > 0 || overall.rate != 0)
		{
			...
			for (int l = 0; l < 5; l++)
			{
				for (int m = 0; m < 3; m++)
				{
					Cells[m, l].PlayState(SPA_CellState.gray);
				}
			}
		}
		if (winCount > 0)
		{
			SPA_SoundManager.Instance.PlayMajorGameAwardAudio(winCount);
		}
		int lineAudioIndex = 0;
		int maxRate = 0;
		SPA_CellType celltype = SPA_CellType.None;
		SPA_WinLineType winlinetype = SPA_WinLineType.None;
		for (int i = 0; i < lineResults.Count; i++)
		{
			if (lineResults[i].winType != 0)
			{
				...
				SPA_SoundManager.Instance.PlayDrawLineAudio(lineAudioIndex);
				lineAudioIndex++;
		...
		if (!isReplay && (winCount > 0 || overall.rate != 0))
		{
			...
			UnityEngine.Debug.LogError("中奖类型: " + celltype + " 中奖个数: " + SPA_HitLineComputer.CalculateHitNum(winlinetype));
			yield return StartCoroutine(WinCellsCelebrateAni(overall.rate != 0));
		}
		allFinishedAction();
	}

	private IEnumerator WaitHideBrightBall(SPA_BallManager ballManager, int index)
	{
		yield return new WaitForSeconds(3f);
		ballManager.HideBrightBall(index);
	}

	private void SetCells(int[,] grid, SPA_CellState cSF_CellState = SPA_CellState.normal)
	{
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				Cells[i, j].PlayState(cSF_CellState, (SPA_CellType)grid[i, j]);
			}
		}
	}
	...
	{
		int num = Enum.GetNames(SPA_CellType.None.GetType()).Length;
	...
	private int[,] SetGridSame(ref int[,] grid, SPA_CellType type = SPA_CellType.NuChe)
	{
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
	...
	private void ShowSingleLineBegin(int index, SPA_WinLineType lineType)
	{
		int num = 0;
		int num2 = 4;
		string state = "green";
		...
		for (int i = num; i <= num2; i++)
		{
			...
			Cells[LineMap[index, i], i].PlayState(SPA_CellState.flash);
		}
	}
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 5; j++)
			{
				if (hitGrid[i, j] >= 2 || overall.rate > 0)
				{
					...
					Cells[i, j].PlayState(SPA_CellState.celebrate);
				}
				else
				{
					Cells[i, j].HideBorder();
					Cells[i, j].PlayState(SPA_CellState.gray);
				}
			}
		}
	}
	...
	private void ShowSingleLineEnd(int index)
	{
		for (int i = 0; i < 5; i++)
		{
			...
			Cells[LineMap[index, i], i].PlayState(SPA_CellState.gray);
		}
	}
	...
	private IEnumerator WaitForCelebrate(int num)
	{
		while (celebrateCount < num)
		{
			...
			yield return new WaitForSeconds(3f);
			coShowAllHitCell = StartCoroutine(SPA_Utils.DelayCall(0.5f, delegate
			{
	...
	public void OnOverallPlayed(string eventName)
	{
		...
		{
			celebrateCount++;
			coShowAllHitCell = StartCoroutine(SPA_Utils.DelayCall(2f, delegate
			{
				if (celebrateCount < num)
				{
					ShowAllHitCell();
			...
			UnityEngine.Debug.Log(SPA_LogHelper.Aqua("eventName: {0}, _celebrateCount: {1}", eventName, celebrateCount));
		}
	}
	...
	private void SetHitGridLine(int lineIndex, SPA_WinLineType winType)
	{
		int num = 0;
		int num2 = 2;
		int num3 = 2;
