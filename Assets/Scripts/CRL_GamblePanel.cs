...
public class CRL_GamblePanel : MonoBehaviour
{
	...
	public static CRL_GamblePanel ins;
	...
	{
		CRL_MB_Singleton<CRL_DiceGameController2>.GetInstance().Send_MultipleInfo(CRL_BetDiceType.Half);
		SetButton(canclick: false);
	}

	private void ClickBlack()
	{
		CRL_MB_Singleton<CRL_DiceGameController2>.GetInstance().Send_MultipleInfo(CRL_BetDiceType.Risk);
		SetButton(canclick: false);
	}
	...
	private Sprite Getsprite()
	{
		try
		{
			uint num = Getnum(CRL_MB_Singleton<CRL_DiceGameController2>.GetInstance().Gamelewinid);
	...
	{
		balancescore = CRL_MySqlConnection.winScorce;
		doublescore = CRL_MySqlConnection.winScorce * 2;
	...
	{
		CRL_MB_Singleton<CRL_DiceGameController2>.GetInstance().OnBtnReady_Click();
		CRL_MB_Singleton<CRL_DiceGameController2>.GetInstance().Hide();
		int credit = (!isGamble) ? (CRL_MySqlConnection.winScorce + CRL_MySqlConnection.credit) : AllScroce;
		CRL_MB_Singleton<CRL_MajorGameController>.GetInstance().SetCredit(credit);
