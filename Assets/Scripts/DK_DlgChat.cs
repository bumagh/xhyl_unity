...
public class DK_DlgChat : MonoBehaviour
{
	[SerializeField]
	...
	[HideInInspector]
	public DK_ChatCtrl sptChatCtrl;
	...
	private void ClickBtnCommonUse(int index)
	{
		...
		DK_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, ifChart.text);
	...
	private void ClickBtnSend()
	{
		if (ifChart.text != string.Empty)
		{
			...
			DK_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, chatMessage);
			base.gameObject.SetActive(value: false);
		}
		else
		{
