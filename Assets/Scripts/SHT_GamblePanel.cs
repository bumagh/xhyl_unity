...
public class SHT_GamblePanel : MonoBehaviour
{
	...
	public static SHT_GamblePanel ins;
	...
	{
		SHT_MB_Singleton<SHT_DiceGameController2>.GetInstance().Send_MultipleInfo(SHT_BetDiceType.Half);
		SetButton(canclick: false);
	}

	private void ClickBlack()
	{
		SHT_MB_Singleton<SHT_DiceGameController2>.GetInstance().Send_MultipleInfo(SHT_BetDiceType.Risk);
		SetButton(canclick: false);
	}
	...
	private Sprite Getsprite()
	{
		try
		{
			uint num = Getnum(SHT_MB_Singleton<SHT_DiceGameController2>.GetInstance().Gamelewinid);
	...
	{
		balancescore = SHT_GVars.winScorce;
		doublescore = SHT_GVars.winScorce * 2;
	...
	{
		SHT_MB_Singleton<SHT_DiceGameController2>.GetInstance().OnBtnReady_Click();
		SHT_MB_Singleton<SHT_DiceGameController2>.GetInstance().Hide();
		int credit = (!isGamble) ? (SHT_GVars.winScorce + SHT_GVars.credit) : AllScroce;
		SHT_MB_Singleton<SHT_MajorGameController>.GetInstance().SetCredit(credit);
