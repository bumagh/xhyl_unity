...
public class BCBM_LuckySpin : MonoBehaviour
{
	...
	private void _TaiHaoSpin_Update()
	{
		...
		{
			_fTaiHaoSpeed += BCBM_GameTool.GetDeltaTime() * 0.2f;
		}
		else
		{
			_fTaiHaoSpeed = mfTaiHao_MaxSpeed;
		}
		_Cur_TaiHao_MaterialOffset += BCBM_GameTool.GetDeltaTime() * _fTaiHaoSpeed;
	...
	private void _LuckyTypSpin_Update()
	{
		...
		{
			_fLuckyPrizeSpeed += BCBM_GameTool.GetDeltaTime();
		}
		else
		{
			_fLuckyPrizeSpeed = mfLuckyPrize_MaxSpeed;
		}
		_Cur_LuckyType_MaterialOffset -= BCBM_GameTool.GetDeltaTime() * _fLuckyPrizeSpeed;
	...
	private void _TaiHao_StopUpdate()
	{
		...
		else
		{
			...
			{
				BCBM_MusicMngr.GetSingleton().PlayGameMusic(BCBM_MusicMngr.MUSIC_GAME_MUSIC.GAME_LUCKYPRIZE_BEGIN);
			}
		}
	}
