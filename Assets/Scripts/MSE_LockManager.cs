...
public class MSE_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {MSE_LogHelper.Key(btnKey)} value {MSE_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {MSE_LogHelper.Key(btnKey)} value {MSE_LogHelper.Key(GetValue(btnKey))}");
			}
