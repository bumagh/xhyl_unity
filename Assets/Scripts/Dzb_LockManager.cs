...
public class Dzb_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {Dzb_LogHelper.Key(btnKey)} value {Dzb_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {Dzb_LogHelper.Key(btnKey)} value {Dzb_LogHelper.Key(GetValue(btnKey))}");
			}
