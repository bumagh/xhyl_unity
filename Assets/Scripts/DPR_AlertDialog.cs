...
public class DPR_AlertDialog : DPR_MB_Singleton<DPR_AlertDialog>
{
	...
	private void Awake()
	{
		...
		InitFinGame();
		if (DPR_MB_Singleton<DPR_AlertDialog>._instance == null)
		{
			DPR_MB_Singleton<DPR_AlertDialog>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void ShowDialog(string content, bool showOkCancel = false, Action callback = null)
	{
		...
		DPR_MB_Singleton<DPR_GameRoot>.GetInstance().StartCoroutine(DPR_Utils.DelayCall(delay, delegate
		{
	...
	{
		isTouchForbidden = !DPR_MB_Singleton<DPR_GameManager>.GetInstance().GetTouchEnable();
		if (isTouchForbidden)
		{
			DPR_MB_Singleton<DPR_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "alert", includeEsc: false);
		}
		...
		if (DPR_MB_Singleton<DPR_OptionsController>.GetInstance() != null)
		{
	...
	public void Hide()
	{
		...
		if (isTouchForbidden)
		{
			DPR_MB_Singleton<DPR_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "alert done, reforbidden", includeEsc: false);
		}
	}
	...
	public void OnBtnCancel_Click()
	{
		if (!DPR_MySqlConnection.tryLockOnePoint)
		{
			if (DPR_SoundManager.Instance != null)
			{
