...
public class PHG_AlertDialog : PHG_MB_Singleton<PHG_AlertDialog>
{
	...
	private void Awake()
	{
		...
		InitFinGame();
		if (PHG_MB_Singleton<PHG_AlertDialog>._instance == null)
		{
			PHG_MB_Singleton<PHG_AlertDialog>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void ShowDialog(string content, bool showOkCancel = false, Action callback = null)
	{
		...
		PHG_MB_Singleton<PHG_GameRoot>.GetInstance().StartCoroutine(PHG_Utils.DelayCall(delay, delegate
		{
	...
	{
		isTouchForbidden = !PHG_MB_Singleton<PHG_GameManager>.GetInstance().GetTouchEnable();
		if (isTouchForbidden)
		{
			PHG_MB_Singleton<PHG_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "alert", includeEsc: false);
		}
		...
		if (PHG_MB_Singleton<PHG_OptionsController>.GetInstance() != null)
		{
	...
	public void Hide()
	{
		...
		if (isTouchForbidden)
		{
			PHG_MB_Singleton<PHG_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "alert done, reforbidden", includeEsc: false);
		}
	}
	...
	public void OnBtnCancel_Click()
	{
		if (!PHG_GVars.tryLockOnePoint)
		{
			if (PHG_SoundManager.Instance != null)
			{
