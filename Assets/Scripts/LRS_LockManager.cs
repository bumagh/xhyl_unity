...
public class LRS_LockManager
{
	...
	public static void Lock(string btnKey, int times = 1)
	{
		...
		UnityEngine.Debug.Log($"Lock {LRS_LogHelper.Key(btnKey)} value {LRS_LogHelper.Key(GetValue(btnKey))}");
	}
	...
	public static bool IsLocked(string btnKey)
	{
		if (_lockDic.ContainsKey(btnKey))
		{
			...
			{
				UnityEngine.Debug.Log($"IsLocked {LRS_LogHelper.Key(btnKey)} value {LRS_LogHelper.Key(GetValue(btnKey))}");
			}
