...
public class STWM_ClientSocket
{
	...
	public Action<int, int, STWM_ClientSocket> checkTimeoutHandler;
	...
	public STWM_ClientSocket(bool useTcpClient)
	{
	...
	public void EndReceiveHandle(IAsyncResult ar)
	{
		...
		lock (_lock)
		{
			try
			{
				...
				{
					exceptionHandler(new Exception("链接失败"));
					return;
				}
				int num = _pNetwrokStream.EndRead(ar);
				if (num == 0)
				{
					exceptionHandler(new Exception("数据为0"));
					return;
				}
				int num2 = 0;
