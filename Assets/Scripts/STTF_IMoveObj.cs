...
public abstract class STTF_IMoveObj : MonoBehaviour
{
	...
	private float _fLimitLeft = 0f - STTF_SetCanvas.Width * 6.2f / 1280f;

	private float _fLimitRight = STTF_SetCanvas.Width * 6.2f / 1280f;
	...
	private void Start()
	{
		_fLimitLeft = 0f - STTF_SetCanvas.Width * 6.2f / 1280f;
		_fLimitRight = STTF_SetCanvas.Width * 6.2f / 1280f;
	}
	...
	public void MoveLogic()
	{
		...
		{
			if (STTF_GameInfo.getInstance().UserList[i].SeatIndex != _nPlayerID)
			{
				continue;
			}
			if ((bool)STTF_GameInfo.getInstance().UserList[i].LockFish)
			{
				STTF_ISwimObj swimObjByTag = STTF_FishPoolMngr.GetSingleton().GetSwimObjByTag(STTF_GameInfo.getInstance().UserList[i].LockFish);
				if (STTF_GameInfo.getInstance().UserList[i].Lock && !swimObjByTag.bFishDead)
				{
	...
	public void SetRotation(Vector3 dir)
	{
		...
		quaternion.SetFromToRotation((mPlayerID != 1 && mPlayerID != 2) ? Vector3.down : Vector3.up, dir);
