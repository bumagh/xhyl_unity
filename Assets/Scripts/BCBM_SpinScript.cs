...
public class BCBM_SpinScript : MonoBehaviour
{
	...
	public void GoToStop(float timeLen, float speed = 100f)
	{
		...
		if (m_bClockwise)
		{
			float num2 = ((float)m_No * 360f / (float)BCBM_Parameter.G_nAnimalNumber - y + 360f) % 360f;
			num = (int)(timeLen * speed) / 360;
			m_AglDistance = num2 + num * 360f;
		}
		else
		{
			float num3 = (y - (float)m_No * 360f / (float)BCBM_Parameter.G_nAnimalNumber + 360f) % 360f;
