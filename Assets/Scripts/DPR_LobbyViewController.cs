...
public class DPR_LobbyViewController : DPR_MB_Singleton<DPR_LobbyViewController>
{
	...
	private DPR_HeadViewController _headViewController;

	private DPR_UserViewer _userViewer;
	...
	private List<DPR_Desk> _deskList = new List<DPR_Desk>();
	...
	private DPR_DeskWidget[] _deskWidgets = new DPR_DeskWidget[3];
	...
	private void Awake()
	{
		...
		if (DPR_MB_Singleton<DPR_LobbyViewController>._instance == null)
		{
			DPR_MB_Singleton<DPR_LobbyViewController>.SetInstance(this);
			PreInit();
		}
	}
	...
	private void InitFinGame()
	{
		...
		_headViewController = base.transform.Find("Title").GetComponent<DPR_HeadViewController>();
		_userViewer = base.transform.Find("PersonInfoDialog").GetComponent<DPR_UserViewer>();
	...
	private void Update()
	{
		if (UnityEngine.Input.GetKeyDown(KeyCode.Escape) && !DPR_MB_Singleton<DPR_LobbyViewController>.GetInstance()._IsAning && !DPR_LockManager.IsLocked("Esc"))
		{
			UnityEngine.Debug.Log("ESC Btn Down In Lobby View");
			DPR_MB_Singleton<DPR_GameManager>.GetInstance().Handle_BtnReturn();
		}
	}
	...
	private void _init()
	{
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().RegisterHandler("roomInfo", HandleNetMsg_RoomInfo);
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().RegisterHandler("enterDesk", HandleNetMsg_EnterDesk);
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().RegisterHandler("updateRoomInfo", HandleNetMsg_UpdateRoomInfo);
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().RegisterHandler("addExpeGoldAuto", HandleNetMsg_AddExpeGoldAuto);
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().RegisterHandler("expeGold", HandleNetMsg_ExpeGold);
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().RegisterHandler("playerInfo", HandleNetMsg_PlayerInfo);
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().RegisterHandler("updateDeskInfo", HandleNetMsg_UpdateDeskInfo);
		_goRoomView.SetActive(value: true);
		_goDeskView.SetActive(value: false);
		int num = (DPR_MySqlConnection.language == "en") ? 1 : 0;
		...
		DPR_Drag_DeskController._instance.left = OnBtnLeftDesk_Click;
		DPR_Drag_DeskController._instance.right = OnBtnRightDesk_Click;
	...
	public void Show()
	{
		...
		DPR_MB_Singleton<DPR_GameManager>.GetInstance().ChangeView(empty);
	...
	{
		if (!DPR_MySqlConnection.tryLockOnePoint && DPR_DeskPullDownListController.Instance.bMoveFinished)
		{
			_desksMoveToLeft();
		}
	}
	...
	{
		if (!DPR_MySqlConnection.tryLockOnePoint && _bCanClick)
		{
			_bCanClick = false;
			DPR_SoundManager.Instance.PlayClickAudio();
	...
	public void Send_EnterRoom(int roomType)
	{
		...
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().Send("userService/enterRoom", args);
		DPR_MB_Singleton<DPR_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "Send_EnterRoom");
	}
	...
	public void Send_EnterDesk(int deskId)
	{
		...
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().Send("userService/enterDesk", args);
		DPR_MB_Singleton<DPR_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "Send_EnterDesk");
	}
	...
	public void Send_LeaveRoom()
	{
		...
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().Send("userService/leaveRoom", args);
	}

	public void Send_GetUserAward()
	{
		object[] args = new object[0];
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().Send("userService/getUserAward", args);
	}

	public void Send_AddExpeGoldAuto()
	{
		object[] args = new object[0];
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().Send("userService/addExpeGoldAuto", args);
	}
	...
	public void Send_PlayerInfo(int userId)
	{
		...
		DPR_MB_Singleton<DPR_NetManager>.GetInstance().Send("userService/playerInfo", args);
	}

	public void HandleNetMsg_RoomInfo(object[] args)
	{
		UnityEngine.Debug.Log(DPR_LogHelper.NetHandle("HandleNetMsg_RoomInfo"));
		if (DPR_MySqlConnection.curView == "MajorGame" || DPR_MySqlConnection.curView == "DiceGame")
		{
			UnityEngine.Debug.Log(DPR_LogHelper.Orange("游戏中，不处理"));
			return;
		}
		_curState = "DeskView";
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
		DPR_MB_Singleton<DPR_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "Send_EnterRoom done");
		if (dictionary != null)
		{
			DPR_LockManager.UnLock("btn_room");
			UnityEngine.Debug.Log("isOverflow: " + dictionary["isOverflow"]);
			DPR_MB_Singleton<DPR_AlertDialog>.GetInstance().ShowDialog((DPR_MySqlConnection.language == "zh") ? "您的账户已爆机，请兑奖" : "Your account is blasting, please exchange");
			return;
		}
		object[] array = args[0] as object[];
		List<DPR_Desk> list = new List<DPR_Desk>();
		for (int i = 0; i < array.Length; i++)
		{
			Dictionary<string, object> data = array[i] as Dictionary<string, object>;
			list.Add(DPR_Desk.CreateWithDic(data));
		}
		_deskList = list;
	...
	{
		UnityEngine.Debug.Log(DPR_LogHelper.NetHandle("HandleNetMsg_EnterDesk"));
		DPR_LockManager.UnLock("EnterDesk");
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
		bool flag = (bool)dictionary["success"];
		DPR_MB_Singleton<DPR_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "Send_EnterDesk done");
		if (flag)
		{
			Hide();
			DPR_MySqlConnection.desk = _deskList[curDeskIndex];
			DPR_MB_Singleton<DPR_MajorGameController>.GetInstance().Show();
			DPR_MB_Singleton<DPR_MajorGameController>.GetInstance().InitGame();
			DPR_MB_Singleton<DPR_ScoreBank>.GetInstance().SetKeepScore(0);
			DPR_MB_Singleton<DPR_ScoreBank>.GetInstance().SetRate(DPR_MySqlConnection.desk.exchange);
			return;
		}
		switch ((int)dictionary["messageStatus"])
		{
		case 0:
			DPR_MB_Singleton<DPR_AlertDialog>.GetInstance().ShowDialog((DPR_MySqlConnection.language == "zh") ? "桌子不存在" : "Game table not exist");
			break;
		case 1:
			DPR_MB_Singleton<DPR_AlertDialog>.GetInstance().ShowDialog((DPR_MySqlConnection.language == "zh") ? "桌已满" : "Don't Have Free Table");
			break;
		case 2:
			DPR_MB_Singleton<DPR_AlertDialog>.GetInstance().ShowDialog((DPR_MySqlConnection.language == "zh") ? "您的账户已爆机，请兑奖" : "Your account is blasting, please excharge");
			break;
		case 3:
			DPR_MB_Singleton<DPR_AlertDialog>.GetInstance().ShowDialog((DPR_MySqlConnection.language == "zh") ? "体验币不足" : "Experience Coins insufficient");
			break;
		case 4:
			DPR_MB_Singleton<DPR_AlertDialog>.GetInstance().ShowDialog((DPR_MySqlConnection.language == "zh") ? "游戏币不足" : "Coins insufficient");
			break;
		default:
	...
	{
		UnityEngine.Debug.Log(DPR_LogHelper.NetHandle("HandleNetMsg_UpdateRoomInfo"));
		...
		if (count != list.Count)
		{
			...
			DPR_DeskPullDownListController.Instance.UpdateDeskInList(_deskList);
		}
		else
		{
			UpdateAllDeskWidgets();
	...
	public void HandleNetMsg_AddExpeGoldAuto(object[] args)
	{
		...
		DPR_MB_Singleton<DPR_AlertDialog>.GetInstance().ShowDialog("申请成功！请返回大厅重新游戏", showOkCancel: false, delegate
		{
	...
	{
		UnityEngine.Debug.Log(DPR_LogHelper.NetHandle("HandleNetMsg_ExpeGold"));
		if ((bool)args[0])
		{
			DPR_MySqlConnection.user.expeGold = 10000;
	...
	{
		UnityEngine.Debug.Log(DPR_LogHelper.NetHandle("HandleNetMsg_PlayerInfo"));
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
	...
	{
		UnityEngine.Debug.Log(DPR_LogHelper.NetHandle("HandleNetMsg_UpdateDeskInfo"));
		Dictionary<string, object> dictionary = args[0] as Dictionary<string, object>;
	...
	private IEnumerator _showDeskViewAni()
	{
		...
		DPR_MySqlConnection.curView = "DeskSelectionView";
		DPR_LockManager.UnLock("btn_room");
		yield return null;
	}

	private DPR_DeskWidget _createDeskWidget()
	{
		...
		DPR_DeskWidget component = gameObject.GetComponent<DPR_DeskWidget>();
	...
	{
		DPR_DeskPullDownListController.Instance.bMoveFinished = false;
		DPR_SoundManager.Instance.PlayClickAudio();
		if (_deskList.Count != 0 && !_inputForbidden)
		{
			DPR_DeskPullDownListController.Instance.ShowListButtonHighBG(_bIsLeft: true, curDeskIndex);
			...
			sequence.onComplete = delegate
			{
				...
				DPR_DeskPullDownListController.Instance.bMoveFinished = true;
				DPR_DeskPointsController._Instance.ShowDeskPoint2(_deskList.Count, curDeskIndex);
			};
		}
	}
	...
	private void _desksMoveToRight()
	{
		...
		if (_deskList.Count != 0 && !_inputForbidden)
		{
			DPR_DeskPullDownListController.Instance.ShowListButtonHighBG(_bIsLeft: false, curDeskIndex);
	...
	public void ForceMoveDesksToLeft(int newIndex)
	{
		...
		if (!_inputForbidden)
		{
			...
			StartCoroutine(DPR_Utils.DelayCall(0.5f, delegate
			{
			}));
		}
	}
	...
	{
		DPR_DeskWidget dPR_DeskWidget = _deskWidgets[a];
		_deskWidgets[a] = _deskWidgets[b];
		_deskWidgets[b] = dPR_DeskWidget;
	}
	...
	public void UpdateAllDeskWidgets()
	{
		...
		DPR_DeskPointsController._Instance.ShowDeskPoint2(_deskList.Count, curDeskIndex);
		DPR_DeskPullDownListController.Instance.ChangeListHeightByDeskNum(_deskList.Count, _deskList);
	}
	...
	private void _onBtnDesk_Click()
	{
		...
		{
			if (DPR_MySqlConnection.user.expeGold == 0 || DPR_MySqlConnection.user.expeGold < _deskList[curDeskIndex].minGold)
			{
				UnityEngine.Debug.Log($"expeGold: {DPR_MySqlConnection.user.expeGold}, _curDeskIndex: {curDeskIndex}, minGold: {_deskList[curDeskIndex].minGold}");
				DPR_MB_Singleton<DPR_AlertDialog>.GetInstance().ShowDialog("体验币不足，是否领取体验币？", showOkCancel: true, delegate
				{
			...
			{
				DPR_LockManager.Lock("EnterDesk");
				Send_EnterDesk(_deskList[curDeskIndex].id);
				DPR_SoundManager.Instance.StopLobbyBGM();
			}
		}
		else if (DPR_MySqlConnection.user.gameGold == 0 || DPR_MySqlConnection.user.gameGold < _deskList[curDeskIndex].minGold)
		{
			UnityEngine.Debug.LogError($"gameGold: {DPR_MySqlConnection.user.gameGold}, _curDeskIndex: {curDeskIndex}, minGold: {_deskList[curDeskIndex].minGold}");
			DPR_MB_Singleton<DPR_AlertDialog>.GetInstance().ShowDialog("游戏币不足！");
		}
		else
		{
			DPR_LockManager.Lock("EnterDesk");
	...
	{
		string info = (DPR_MySqlConnection.language == "zh") ? "选择房间" : "SelectRoom";
		if (_curState == "DeskView")
		{
			if (_curRoomType == 1)
			{
				info = ((DPR_MySqlConnection.language == "zh") ? "新手练习厅" : "Training");
			}
			else if (_curRoomType == 2)
			{
				info = ((DPR_MySqlConnection.language == "zh") ? "欢乐竞技厅" : "Arena");
			}
		}
	...
	{
		_IsAning = true;
		DPR_MB_Singleton<DPR_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "_hideRoomViewAni");
		Sequence sequence = DOTween.Sequence();
		...
		sequence.onComplete = delegate
		{
			DPR_MB_Singleton<DPR_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "_hideRoomViewAni done");
			_IsAning = false;
			_bCanClick = true;
		};
	}
	...
	public void ResetRoomView()
	{
		...
		DPR_MB_Singleton<DPR_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "ResetRoomView");
		_userViewer.gameObject.SetActive(value: false);
		Sequence sequence = DOTween.Sequence();
		...
		sequence.onComplete = delegate
		{
			...
			_bCanClick = false;
			_updateHeadInfo();
			DPR_MB_Singleton<DPR_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "ResetRoomView_Over");
		};
	}
	...
	public void HideDeskViewAni()
	{
		...
		sequence.onComplete = delegate
		{
			DPR_MB_Singleton<DPR_DeskSelectionViewController>.GetInstance().Hide();
		};
	}
}
