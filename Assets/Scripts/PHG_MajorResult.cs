...
public class PHG_MajorResult
{
	...
	public PHG_MajorResult Init(int lineNum, int singleStake, int totalStake, Dictionary<string, object> dic)
	{
		this.lineNum = lineNum;
		this.singleStake = singleStake;
		this.totalStake = totalStake;
		totalWin = (int)dic["totalWin"];
		Cinstance<PHG_Gcore>.Instance.Result = new List<int>();
		Cinstance<PHG_Gcore>.Instance.WinlineList = new List<int>();
		Cinstance<PHG_Gcore>.Instance.Winlinenumlist = new List<int>();
		Cinstance<PHG_Gcore>.Instance.WinScoreList = new List<int>();
		...
		for (int k = 0; k < length; k++)
		{
			...
			{
				int num = array2[k, l];
				Cinstance<PHG_Gcore>.Instance.Result.Add(num - 1);
			}
		}
		...
		for (int m = 0; m < array5.Length; m++)
		{
			if (array5[m] != 0)
			{
				Cinstance<PHG_Gcore>.Instance.WinlineList.Add(array4[m]);
				Cinstance<PHG_Gcore>.Instance.Winlinenumlist.Add(array3[m]);
				Cinstance<PHG_Gcore>.Instance.WinScoreList.Add(array5[m]);
			}
		}
