...
public class MSE_Line : MonoBehaviour
{
	private List<MSE_SymbolC> iconlist;

	private List<int> result;

	private int RandomIco = 8;
	...
	private void Awake()
	{
		...
		{
			base.transform.DOLocalMoveY(posy - 40f, 0.2f);
		});
		seq_line.AppendInterval(0.3f);
		seq_line.AppendCallback(delegate
		{
	...
	private void Initline()
	{
		...
		try
		{
			while (enumerator.MoveNext())
			{
				...
				component.Init(symid: (num > 2) ? UnityEngine.Random.Range(0, 6) : Cinstance<MSE_Gcore>.Instance.Normallist[index], idx: num++, line: this);
	...
	public void Playani()
	{
		if (!stopline)
		{
			...
			if (LineIdx == 4)
			{
				...
				t.SetDelay(1f).AppendCallback(Cinstance<MSE_Gcore>.Instance.AllStop);
			}
		}
	}
