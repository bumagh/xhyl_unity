...
public class USW_HUDController : USW_MB_Singleton<USW_HUDController>
{
	...
	private void Awake()
	{
		...
		if (USW_MB_Singleton<USW_HUDController>._instance == null)
		{
			USW_MB_Singleton<USW_HUDController>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void PreInit()
	{
		...
		{
			ruleController = base.transform.Find("Canvas/Mask/Rule").GetComponent<USW_RulePicController>();
		}
	}

	public void Init()
	{
		USW_MB_Singleton<USW_OptionsController>.GetInstance().onItemRules = delegate
		{
			ruleController.Show();
		};
		ruleController.Hide();
	...
	{
		ping = new Ping(USW_GVars.IPAddress);
		if (waitPing != null)
		{
	...
	public void OnBtnOptions_Click()
	{
		...
		{
			if (USW_MB_Singleton<USW_OptionsController>.GetInstance().isShow)
			{
