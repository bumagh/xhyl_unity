...
public class STQM_Sockets : MonoBehaviour
{
	public bool isReconnect = true;

	private static STQM_Sockets _MyCreateSocket;
	...
	private STQM_DataEncrypt m_DataEncrypt;
	...
	public static STQM_Sockets GetSingleton()
	{
	...
	public void CreateSocketGetPoint(STQM_MessageControl MyMessageControl, STQM_DataEncrypt MyDataEncrypt)
	{
		CreateReceiveThread();
	...
	public void SocketConnect()
	{
		...
		if (m_nSocketStartFlag)
		{
			if (!sendCheckVersion)
			{
				SendCheckVersion(STQM_Constants.VERSION_CODE);
	...
	private bool _socketStart()
	{
		...
		IPEndPoint remoteEP = new IPEndPoint(IPAddress.Parse(STQM_GameInfo.getInstance().IP), 10013);
		try
		{
	...
	private void _receiveNetMessage(byte[] msgBytes)
	{
		...
		else
		{
			try
			{
				hashtable = STQM_ReadFileTool.JsonToClass<Hashtable>(@string);
			}
			catch (Exception)
			{
				msgBytes = m_DataEncrypt.DecompressBytes(msgBytes);
				Console.WriteLine(Encoding.UTF8.GetString(msgBytes));
				hashtable = STQM_ReadFileTool.JsonToClass<Hashtable>(Encoding.UTF8.GetString(msgBytes));
	...
	public bool _sendMsg(string strMethod, object[] args)
	{
		...
		if (args == null)
		{
			UnityEngine.Debug.Log("---" + strMethod);
		...
		else
		{
			...
			hashtable.Add("version", STQM_Constants.Version);
			hashtable.Add("time", m_DataEncrypt.GetUnixTime());
			string text = STQM_ReadFileTool.JsonByObject(hashtable);
	...
	public bool SendFired(int gunid, int seatid, float fRot, int gunValue, bool bX2Gun)
	{
		...
		UnityEngine.Debug.Log("userService/fired");
		return _sendMsg(strMethod, null);
	}

	public bool SendGunHitfish(int gunid, STQM_HitFish[] hitfish1, bool bHaveKnife, STQM_HitFish[] hitfish2)
	{
		string empty = string.Empty;
		...
		UnityEngine.Debug.Log("userService/gunHitFish");
	...
	public bool SendPublicKey()
	{
		...
		string s = STQM_ReadFileTool.JsonByObject(hashtable);
