...
public class WHN_Gcore : Cinstance<WHN_Gcore>
{
	...
	private int symcont = 10;

	public int lineConut = 15;
	...
	public void Init()
	{
		try
		{
			...
			for (int i = 1; i <= symcont; i++)
			{
				...
				{
				case 2:
					empty = string.Format("dxEle{0}_02_png", i.ToString("00"));
					break;
				case 10:
					empty = $"Wild_02_png";
					break;
				default:
					empty = string.Format("Ele{0}_02_png", i.ToString("00"));
					break;
				}
				Sprite sprite = WHN_MB_Singleton<WHN_GameManager>.GetInstance().Getanisprite(empty);
				SymSprlist.Add(sprite);
				if (sprite == null)
				{
					UnityEngine.Debug.LogError("图片: " + empty + " 不存在,请检查名字");
				}
			}
			...
			{
				List<int> item = WHN_LinePosIdx.Getwinlinepos(j, 5);
				AllLinePos.Add(item);
			}
	...
	{
		IsRoll = false;
		WHN_MB_Singleton<WHN_MajorGameController>.GetInstance().AllStop();
	}
	...
	public void QuickStop()
	{
		if (IsRoll)
		{
			WHN_MB_Singleton<WHN_MajorGameController>.GetInstance().StopRoll();
			IsRoll = false;
		}
	}
	...
	{
		IsRoll = true;
		WHN_MB_Singleton<WHN_MajorGameController>.GetInstance().StartRollSym();
	}
