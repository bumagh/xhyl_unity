...
public abstract class STQM_IMoveObj : MonoBehaviour
{
	...
	private float _fLimitLeft = 0f - STQM_SetCanvas.Width * 6.2f / 1280f;

	private float _fLimitRight = STQM_SetCanvas.Width * 6.2f / 1280f;
	...
	private void Start()
	{
		_fLimitLeft = 0f - STQM_SetCanvas.Width * 6.2f / 1280f;
		_fLimitRight = STQM_SetCanvas.Width * 6.2f / 1280f;
	}
	...
	public void MoveLogic()
	{
		...
		{
			if (STQM_GameInfo.getInstance().UserList[i].SeatIndex != _nPlayerID)
			{
				continue;
			}
			if ((bool)STQM_GameInfo.getInstance().UserList[i].LockFish)
			{
				STQM_ISwimObj swimObjByTag = STQM_FishPoolMngr.GetSingleton().GetSwimObjByTag(STQM_GameInfo.getInstance().UserList[i].LockFish);
				if (STQM_GameInfo.getInstance().UserList[i].Lock && !swimObjByTag.bFishDead)
				{
