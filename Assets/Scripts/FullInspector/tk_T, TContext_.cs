	...
	public class tk<T, TContext>
	{
		...
		public class Button : tkControl<T, TContext>
		{
			...
			public Button(string methodName)
			{
				...
				{
					UnityEngine.Debug.LogError("Unable to find method " + methodName + " on " + typeof(T).CSharpName());
		...
		public class DefaultInspector : tkControl<T, TContext>
		{
			...
			public DefaultInspector()
			{
				...
				{
					UnityEngine.Debug.LogError("Unable to find tkControlPropertyEditor");
				}
				if (type_IObjectPropertyEditor == null)
				{
					UnityEngine.Debug.LogError("Unable to find IObjectPropertyEditor");
				}
			}

			protected override T DoEdit(Rect rect, T obj, TContext context, fiGraphMetadata metadata)
			{
				return (T)fiLateBindings.PropertyEditor.EditSkipUntilNot(new Type[2]
				{
					type_fitkControlPropertyEditor,
					type_IObjectPropertyEditor
			...
			protected override float DoGetHeight(T obj, TContext context, fiGraphMetadata metadata)
			{
				return fiLateBindings.PropertyEditor.GetElementHeightSkipUntilNot(new Type[2]
				{
					type_fitkControlPropertyEditor,
					type_IObjectPropertyEditor
		...
		public class Foldout : tkControl<T, TContext>
		{
			...
			public Foldout(fiGUIContent label, FontStyle fontStyle, tkControl<T, TContext> control)
				: this(label, fontStyle, defaultToExpanded: true, control)
			{
			}
		...
		public class HorizontalGroup : tkControl<T, TContext>, IEnumerable
		{
			...
			protected override IEnumerable<tkIControl> NonMemberChildControls
			{
				get
				{
					foreach (SectionItem item2 in _items)
					{
		...
		public class PropertyEditor : tkControl<T, TContext>
		{
			...
			protected override T DoEdit(Rect rect, T obj, TContext context, fiGraphMetadata metadata)
			{
				...
				fiGraphMetadataChild fiGraphMetadataChild = default(fiGraphMetadataChild);
				fiGraphMetadataChild.Metadata = GetInstanceMetadata(metadata);
				fiGraphMetadataChild metadata2 = fiGraphMetadataChild;
				fiGUIContent currentValue = _label.GetCurrentValue(obj, context);
