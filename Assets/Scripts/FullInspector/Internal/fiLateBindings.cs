...
namespace FullInspector.Internal
{
	public static class fiLateBindings
	{
		...
		public static class EditorUtility
		{
			...
			{
				if (VerifyBinding("EditorUtility.SetDirty", _Bindings._EditorUtility_SetDirty) && unityObject != null)
				{
		...
		private static bool VerifyBinding(string name, object obj)
		{
			if (obj == null)
			{
				...
				{
					UnityEngine.Debug.LogWarning("There is no binding for " + name + " even though we are in an editor");
				}
				return false;
			}
			return true;
