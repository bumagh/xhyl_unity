...
namespace FullInspector
{
	public class Facade<T>
	{
		...
		public void PopulateInstance(ref T instance)
		{
			...
			{
				UnityEngine.Debug.LogWarning("PopulateInstance: Actual Facade type is different (instance.GetType() = " + instance.GetType().CSharpName() + ", InstanceType = " + InstanceType.CSharpName() + ")");
			}
			...
			foreach (KeyValuePair<string, string> facadeMember in FacadeMembers)
			{
				...
				if (propertyByName != null)
				{
					...
					{
						UnityEngine.Debug.LogError("Skipping property " + key + " in facade due to deserialization exception.\n" + ex);
					}
				}
			}
		}
