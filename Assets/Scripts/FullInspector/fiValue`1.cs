...
namespace FullInspector
{
	public abstract class fiValue<T> : fiValueProxyEditor, fiIValueProxyAPI, ISerializationCallbackReceiver
	{
		...
		private void Serialize()
		{
			...
			{
				UnityEngine.Debug.LogError("Exception caught when serializing " + this + " (with type " + GetType() + ")\n" + ex);
			}
		...
		private void Deserialize()
		{
			...
			if (!string.IsNullOrEmpty(SerializedState))
			{
				...
				{
					UnityEngine.Debug.LogError("Exception caught when deserializing " + this + " (with type " + GetType() + ");\n" + ex);
				}
			}
		}
	}
