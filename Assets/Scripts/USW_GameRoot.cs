...
public class USW_GameRoot : USW_MB_Singleton<USW_GameRoot>
{
    ...
    public void Awake()
    {
        ...
        if (!(USW_MB_Singleton<USW_GameRoot>._instance != null))
        {
            StartCoroutine(InitSingletons());
            USW_MB_Singleton<USW_GameRoot>.SetInstance(this);
        }
    }
    ...
    public static T GetSingleton<T>() where T : USW_MB_Singleton<T>
    {
    ...
    public static void PureAddReleaseAction<T>(T t) where T : USW_MB_Singleton<T>
    {
        _singletonReleaseList.Add(delegate
        {
            t.Release();
