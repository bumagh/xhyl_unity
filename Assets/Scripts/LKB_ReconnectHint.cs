...
public class LKB_ReconnectHint : LKB_MB_Singleton<LKB_ReconnectHint>
{
	...
	private void Awake()
	{
		if (LKB_MB_Singleton<LKB_ReconnectHint>._instance == null)
		{
			LKB_MB_Singleton<LKB_ReconnectHint>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void Show()
	{
		if (!_goContainer.activeSelf)
		{
			...
			UnityEngine.Debug.Log(LKB_LogHelper.Magenta("ReconnectHint Show"));
	...
	public void Hide()
	{
		UnityEngine.Debug.Log(LKB_LogHelper.Magenta("ReconnectHint Hide"));
		LKB_LockManager.UnLock("Esc");
	...
	private IEnumerator _textInfoAni()
	{
		...
		while (true)
		{
			string prefix = (LKB_GVars.language == "zh") ? "网络断开，正在重连 " : "Reconnecting to the network ";
