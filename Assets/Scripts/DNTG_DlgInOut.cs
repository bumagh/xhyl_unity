...
public class DNTG_DlgInOut : MonoBehaviour
{
	[SerializeField]
	...
	private void ClickBtnOut()
	{
		...
		{
			DNTG_NetMngr.GetSingleton().MyCreateSocket.SendCoinOut(gameInfo.Table.DeltaCoin * gameInfo.Table.ScorePerCoin);
			DNTG_SoundManage.getInstance().playButtonMusic(DNTG_ButtonMusicType.coinUp);
			return;
		}
		int scoreCount = gameInfo.User.ScoreCount;
		if (scoreCount > 0)
		{
			DNTG_NetMngr.GetSingleton().MyCreateSocket.SendCoinOut(scoreCount);
			DNTG_SoundManage.getInstance().playButtonMusic(DNTG_ButtonMusicType.coinUp);
		}
		else
		{
	...
	private void ClickBtnIn()
	{
		...
		if (num * gameInfo.Table.ScorePerCoin + gameInfo.User.ScoreCount <= 900000)
		{
			if (num > 0)
			{
				DNTG_NetMngr.GetSingleton().MyCreateSocket.SendCoinIn(num);
				DNTG_SoundManage.getInstance().playButtonMusic(DNTG_ButtonMusicType.coinUp);
			}
			else
			{
