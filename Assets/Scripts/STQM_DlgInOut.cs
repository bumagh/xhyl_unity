...
public class STQM_DlgInOut : MonoBehaviour
{
	[SerializeField]
	...
	private void ClickBtnOut()
	{
		...
		{
			STQM_NetMngr.GetSingleton().MyCreateSocket.SendCoinOut(gameInfo.Table.DeltaCoin * gameInfo.Table.ScorePerCoin);
			STQM_SoundManage.getInstance().playButtonMusic(STQM_ButtonMusicType.coinUp);
			return;
		}
		int scoreCount = gameInfo.User.ScoreCount;
		if (scoreCount > 0)
		{
			STQM_NetMngr.GetSingleton().MyCreateSocket.SendCoinOut(scoreCount);
			STQM_SoundManage.getInstance().playButtonMusic(STQM_ButtonMusicType.coinUp);
		}
		else
		{
	...
	private void ClickBtnIn()
	{
		...
		if (num * gameInfo.Table.ScorePerCoin + gameInfo.User.ScoreCount <= 900000)
		{
			if (num > 0)
			{
				STQM_NetMngr.GetSingleton().MyCreateSocket.SendCoinIn(num);
				STQM_SoundManage.getInstance().playButtonMusic(STQM_ButtonMusicType.coinUp);
			}
			else
			{
