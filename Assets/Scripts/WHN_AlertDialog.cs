...
public class WHN_AlertDialog : WHN_MB_Singleton<WHN_AlertDialog>
{
	...
	private void Awake()
	{
		...
		InitFinGame();
		if (WHN_MB_Singleton<WHN_AlertDialog>._instance == null)
		{
			WHN_MB_Singleton<WHN_AlertDialog>.SetInstance(this);
			PreInit();
		}
	}
	...
	public void ShowDialog(string content, bool showOkCancel = false, Action callback = null)
	{
		...
		WHN_MB_Singleton<WHN_GameRoot>.GetInstance().StartCoroutine(WHN_Utils.DelayCall(delay, delegate
		{
	...
	{
		isTouchForbidden = !WHN_MB_Singleton<WHN_GameManager>.GetInstance().GetTouchEnable();
		if (isTouchForbidden)
		{
			WHN_MB_Singleton<WHN_GameManager>.GetInstance().SetTouchEnable(isEnable: true, "alert", includeEsc: false);
		}
		...
		if (WHN_MB_Singleton<WHN_OptionsController>.GetInstance() != null)
		{
	...
	public void Hide()
	{
		...
		if (isTouchForbidden)
		{
			WHN_MB_Singleton<WHN_GameManager>.GetInstance().SetTouchEnable(isEnable: false, "alert done, reforbidden", includeEsc: false);
		}
	}
	...
	public void OnBtnCancel_Click()
	{
		if (!WHN_GVars.tryLockOnePoint)
		{
			if (WHN_SoundManager.Instance != null)
			{
