...
public class STMF_DlgChat : MonoBehaviour
{
	[SerializeField]
	...
	[HideInInspector]
	public STMF_ChatCtrl sptChatCtrl;
	...
	private void ClickBtnCommonUse(int index)
	{
		...
		STMF_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, ifChart.text);
	...
	private void ClickBtnSend()
	{
		if (ifChart.text != string.Empty)
		{
			...
			STMF_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, chatMessage);
			base.gameObject.SetActive(value: false);
		}
		else
		{
	...
	public void ShowChat()
	{
		...
		if (gameInfo.IsGameShuUp || gameInfo.IsUserShutUp)
		{
			...
			btnSend.GetComponent<STMF_ButtonImgAni>().enabled = false;
			btnSend.image.sprite = spiBtnSend[language * 2 + 1];
			SetBtnCommonUse(b: false);
		}
		else
		{
			btnSend.GetComponent<STMF_ButtonImgAni>().enabled = true;
