...
public class CSF_NetManager : CSF_MB_Singleton<CSF_NetManager>
{
	public delegate void NetMessageHandler(object[] args);

	private CSF_ClientSocket _pClient;

	private CSF_DataEncrypt _pCrypto;
	...
	private Queue<CSF_NetMessage> _netMsgQueue = new Queue<CSF_NetMessage>();

	private Dictionary<string, NetMessageHandler> _netMsgHandlerDic = new Dictionary<string, NetMessageHandler>();

	private void Awake()
	{
		if (CSF_MB_Singleton<CSF_NetManager>._instance == null)
		{
			CSF_MB_Singleton<CSF_NetManager>.SetInstance(this);
		}
	...
	private void _initClientSocket()
	{
		...
		_pClient = new CSF_ClientSocket(useTcpClient: true);
		_pCrypto = new CSF_DataEncrypt();
	...
	private void _handleException(Exception ex)
	{
		...
		isReady = false;
		isLogined = false;
		Queue<CSF_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
			_netMsgQueue.Enqueue(new CSF_NetMessage("netDown", new object[1]
			{
				ex
			}));
		}
	...
	private void _handleConnected()
	{
		try
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new CSF_NetMessage("netConnected", new object[0]));
			}
		}
		catch (Exception ex)
		{
	...
	{
		UnityEngine.Debug.Log("ZH2_GVars.versionCode: " + CSF_MySqlConnection.versionCode);
		Send("userService/checkVersion", new object[1]
		{
			CSF_MySqlConnection.versionCode
		});
	}
	...
	protected void _handleData(byte[] msgBuffer, int packetId, int endIndex)
	{
		...
		lock (netMsgQueue)
		{
			CSF_NetMessage cSF_NetMessage = new CSF_NetMessage(text, args);
			cSF_NetMessage.jsonString = @string;
			cSF_NetMessage.packetId = packetId;
			cSF_NetMessage.endIndex = endIndex;
			_netMsgQueue.Enqueue(cSF_NetMessage);
		}
	}

	private IEnumerator _checkTimeout(int id, int timeout, CSF_ClientSocket cs)
	{
		UnityEngine.Debug.Log(CSF_LogHelper.Aqua("_checkTimeout start id: {0}", id));
		...
		if (cs != null && cs.doneList.Count > id && !cs.doneList[id])
		{
			...
			lock (netMsgQueue)
			{
				_netMsgQueue.Enqueue(new CSF_NetMessage("timeout", new object[1]
				{
					id
				}));
				isTimeout = true;
		...
		if (isTimeout)
		{
			UnityEngine.Debug.Log(CSF_LogHelper.Aqua("_checkTimeout start id: {0} timeout: {1}", id, isTimeout));
		}
	}
	...
	private IEnumerator _checkHeartTimeout(int timeout)
	{
		...
		if (isReady && !isHeartPong)
		{
			...
			lock (netMsgQueue)
			{
				...
				isReady = false;
				isLogined = false;
				_netMsgQueue.Enqueue(new CSF_NetMessage("heart timeout", new object[0]));
				isTimeout = true;
			}
		}
		if (isTimeout)
		{
			UnityEngine.Debug.Log(CSF_LogHelper.Aqua("_checkHeartTimeout is timeout"));
		}
	}

	public void _checkTimeoutStart(int id, int timeout, CSF_ClientSocket cs)
	{
	...
	public void Send(string method, object[] args)
	{
		...
		{
			UnityEngine.Debug.Log(CSF_LogHelper.Magenta("drop send>>method: {0}", method));
			return;
		}
		Hashtable hashtable = new Hashtable();
		hashtable.Add("method", method);
		hashtable.Add("args", args);
		hashtable.Add("version", CSF_MySqlConnection.Version);
		if (isReady)
		{
	...
	private bool GetNetMessage(out CSF_NetMessage netMsg)
	{
		Queue<CSF_NetMessage> netMsgQueue = _netMsgQueue;
		lock (netMsgQueue)
		{
	...
	private void DispatchNetMessage(CSF_NetMessage netMsg)
	{
		if (dropDispatch)
		{
			UnityEngine.Debug.Log(CSF_LogHelper.Magenta("drop send>>method: {0}", netMsg.method));
		}
