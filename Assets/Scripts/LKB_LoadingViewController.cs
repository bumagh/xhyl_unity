...
public class LKB_LoadingViewController : LKB_MB_Singleton<LKB_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (LKB_MB_Singleton<LKB_LoadingViewController>._instance == null)
		{
			LKB_MB_Singleton<LKB_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (LKB_MB_Singleton<LKB_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public void Show()
	{
		UnityEngine.Debug.LogError("显示登陆界面");
		if (_goUIContainer == null)
		{
			_goUIContainer = base.gameObject;
		}
		LKB_Utils.TrySetActive(_goUIContainer, active: true);
		LKB_GVars.curView = "LoadingView";
	}
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!LKB_MB_Singleton<LKB_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
