...
public class BCBM_BetPanel : MonoBehaviour
{
	protected BCBM_HudManager mHudMgr = new BCBM_HudManager();
	...
	protected BCBM_TableInfo mTableInfo;

	protected BCBM_PersonInfo mUser;

	protected int mCurrentTableID;

	protected int mCurrentChip = 1;

	protected BCBM_UICheckbox[] mChipOption = new BCBM_UICheckbox[3];
	...
	private void Awake()
	{
		...
		for (int i = 0; i < 15; i++)
		{
			...
			if (i < 3)
			{
				...
				mChipOption[i] = mChipBtn.Find("Chip" + i).GetComponent<BCBM_UICheckbox>();
				mChipText[i] = mChipBtn.Find("Chip" + i).Find("Label").GetComponent<UILabel>();
			}
		}
		mUser = BCBM_GameInfo.getInstance().UserInfo;
		HideBetPanel();
		ClearAllBet();
	...
	private void Update()
	{
		UnityEngine.Debug.LogError(base.gameObject.name);
		if (mIsFrameLight)
		{
	...
	public void ShowBetPanel(bool bIsMove = true)
	{
		...
		mBetPanelBtnSprite.spriteName = ((BCBM_GameInfo.getInstance().Language == 0) ? "shouQi" : "shouQi_e");
	...
	public void ModifyChip(BCBM_TableInfo tableInfo)
	{
	...
	public void AutoBet()
	{
		...
		if (_isAllBetSuccess())
		{
			if (!BCBM_MyTest.TEST)
			{
				UnityEngine.Debug.LogError("下注2");
				for (int i = 0; i < 15; i++)
				{
					BCBM_NetMngr.GetSingleton().MyCreateSocket.SendUserBet(null, i, mLastOnceBet[i], mCurrentTableID);
				}
				mIsAllzero = false;
			}
		}
		...
		{
			mIsAutoBet = false;
			mAutoBtnLabel.text = ((BCBM_GameInfo.getInstance().Language == 0) ? "自动" : "Auto");
		}
	}
	...
	public void _onClickBetItem(GameObject sender)
	{
		...
		if (mLastOnceBet[num] > 0)
		{
			...
			if (BCBM_LuckyLion_SoundManager.GetSingleton() != null)
			{
				BCBM_LuckyLion_SoundManager.GetSingleton().playButtonSound(BCBM_LuckyLion_SoundManager.EUIBtnSoundType.BetSuccess);
			}
			...
			if (!BCBM_MyTest.TEST)
			{
				UnityEngine.Debug.LogError("下注3");
				BCBM_NetMngr.GetSingleton().MyCreateSocket.SendUserBet(null, num, mLastOnceBet[num], mCurrentTableID);
			}
		}
		else if (BCBM_LuckyLion_SoundManager.GetSingleton() != null)
		{
			BCBM_LuckyLion_SoundManager.GetSingleton().playButtonSound(BCBM_LuckyLion_SoundManager.EUIBtnSoundType.BetFail);
		}
	}
	...
	public void _onClickCancelBtn()
	{
		if (mHudMgr.TimeCD > 0)
		{
			...
			if (!BCBM_MyTest.TEST)
			{
				BCBM_NetMngr.GetSingleton().MyCreateSocket.SendCancelBet(mCurrentTableID);
			}
			_setFuncBtnEnabled(bIsEnabled: false, 0);
			_setFuncBtnEnabled(bIsEnabled: false, 2);
			int j;
	...
	public void _onClickXuyaBtn()
	{
		...
		if (_isAllBetSuccess())
		{
			if (!BCBM_MyTest.TEST)
			{
				UnityEngine.Debug.LogError("下注4");
				for (int i = 0; i < 15; i++)
				{
					BCBM_NetMngr.GetSingleton().MyCreateSocket.SendUserBet(null, i, mLastOnceBet[i], mCurrentTableID);
				}
	...
	public void _onClickAutoBtn()
	{
		...
		{
			mIsAutoBet = true;
			mAutoBtnLabel.text = ((BCBM_GameInfo.getInstance().Language == 0) ? "取消自动" : "Manual");
			_setFuncBtnEnabled(bIsEnabled: false, 0);
			_setFuncBtnEnabled(bIsEnabled: false, 1);
			return;
		}
