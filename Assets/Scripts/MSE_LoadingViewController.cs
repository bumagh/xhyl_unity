...
public class MSE_LoadingViewController : MSE_MB_Singleton<MSE_LoadingViewController>
{
	...
	private void Awake()
	{
		...
		if (MSE_MB_Singleton<MSE_LoadingViewController>._instance == null)
		{
			MSE_MB_Singleton<MSE_LoadingViewController>.SetInstance(this);
			PreInit();
		}
		if (MSE_MB_Singleton<MSE_LoadingViewController>._instance != this)
		{
			Hide();
	...
	public IEnumerator LoadingAni()
	{
		...
		while (!MSE_MB_Singleton<MSE_NetManager>.GetInstance().isLogined)
		{
			yield return null;
		}
