...
public class STTF_FishPathMngr : MonoBehaviour
{
	public static STTF_FishPathMngr G_FishPathMngr;

	private STTF_PathManager[] allFishPath;

	private STTF_PathManager[] smallFishPath;
	...
	public static STTF_FishPathMngr GetSingleton()
	{
	...
	private void Awake()
	{
		...
		int num = 0;
		_AllPathLength = 88;
		allFishPath = new STTF_PathManager[_AllPathLength];
		...
		for (int i = 0; i < _AllPathLength; i++)
		{
			...
			allFishPath[i] = new STTF_PathManager();
		...
		smallFishPath = new STTF_PathManager[_AllPathLength * 4];
	...
	public STTF_PathManager GetFishPath(int type)
	{
		return allFishPath[type];
	}

	public STTF_PathManager GetSmallFishPath(int type)
	{
		return smallFishPath[type];
	}

	private STTF_PathManager ChangePath(STTF_PathManager path, Vector3 vec)
	{
		int num = path.vecs.Length;
		STTF_PathManager sTTF_PathManager = new STTF_PathManager();
		sTTF_PathManager.vecs = new Vector3[num];
		for (int i = 0; i < num; i++)
		{
			sTTF_PathManager.vecs[i] = path.vecs[i] + vec;
		}
		return sTTF_PathManager;
	}

	public void GetAllPath(int type)
	{
		STTF_PathManager sTTF_PathManager = smallFishPath[type];
		int num = sTTF_PathManager.vecs.Length;
		string text = string.Empty;
		for (int i = 0; i < num; i++)
		{
			string text2 = text;
			text = text2 + sTTF_PathManager.vecs[i][0].ToString() + "," + sTTF_PathManager.vecs[i][1].ToString() + "," + sTTF_PathManager.vecs[i][2].ToString() + " ";
		}
