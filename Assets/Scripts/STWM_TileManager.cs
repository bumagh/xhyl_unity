...
public class STWM_TileManager : MonoBehaviour
{
	...
	public IEnumerator StartChasing(int targetIndex, Action endAction)
	{
		...
		STWM_XWait xwait = new STWM_XWait(this).Begin();
		int count = 0;
		float timeCount = 0f;
		...
		{
			STWM_SoundManager.Instance.PlayConstantSpeedAudio();
		}));
		...
		{
			STWM_SoundManager.Instance.PlayReduceSpeedAudio(i + 1);
			count++;
	...
	public IEnumerator WaitSecondsAction(float seconds, Action<int> action, int param1, STWM_XWait xwait = null)
	{
		if (xwait == null)
		{
			yield return new WaitForSeconds(seconds);
