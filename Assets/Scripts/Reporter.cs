...
public class Reporter : MonoBehaviour
{
	...
	private bool showCopyAllButton = true;
	...
	private GUIContent copyAllContent;
	...
	public void Initialize()
	{
		if (!created)
		{
			...
			copyAllContent = new GUIContent(string.Empty, images.copyAllImage, "Copy all logs to clipboard");
	...
	private void drawInfo_enableDisableToolBarButtons()
	{
		...
		if (GUILayout.Button(copyContent, (!showCopyButton) ? barStyle : buttonActiveStyle, GUILayout.Width(size.x * 2f), GUILayout.Height(size.y * 2f)))
		{
			showCopyButton = !showCopyButton;
		}
		if (GUILayout.Button(copyAllContent, (!showCopyAllButton) ? barStyle : buttonActiveStyle, GUILayout.Width(size.x * 2f), GUILayout.Height(size.y * 2f)))
		{
			showCopyAllButton = !showCopyAllButton;
		}
		if (GUILayout.Button(saveLogsContent, (!showSaveButton) ? barStyle : buttonActiveStyle, GUILayout.Width(size.x * 2f), GUILayout.Height(size.y * 2f)))
		{
	...
	private void drawToolBar()
	{
		...
		if (showCopyAllButton && GUILayout.Button(copyAllContent, barStyle, GUILayout.Width(size.x * 2f), GUILayout.Height(size.y * 2f)))
		{
			string allLogsToClipboard = string.Empty;
			logs.ForEach(delegate(Log l)
			{
				string text = allLogsToClipboard;
				allLogsToClipboard = text + l.condition + Environment.NewLine + Environment.NewLine + l.stacktrace;
			});
	...
	private void Update()
	{
		...
		{
			UnityEngine.Debug.LogError("打开性能监视器");
		}
