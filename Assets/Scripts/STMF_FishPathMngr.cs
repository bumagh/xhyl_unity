...
public class STMF_FishPathMngr : MonoBehaviour
{
	public static STMF_FishPathMngr G_FishPathMngr;

	private STMF_PathManager[] allFishPath;

	private STMF_PathManager[] smallFishPath;
	...
	public static STMF_FishPathMngr GetSingleton()
	{
	...
	private void Awake()
	{
		...
		int num = 0;
		_AllPathLength = 88;
		allFishPath = new STMF_PathManager[_AllPathLength];
		...
		for (int i = 0; i < _AllPathLength; i++)
		{
			...
			allFishPath[i] = new STMF_PathManager();
		...
		smallFishPath = new STMF_PathManager[_AllPathLength * 4];
	...
	public STMF_PathManager GetFishPath(int type)
	{
		return allFishPath[type];
	}

	public STMF_PathManager GetSmallFishPath(int type)
	{
		return smallFishPath[type];
	}

	private STMF_PathManager ChangePath(STMF_PathManager path, Vector3 vec)
	{
		int num = path.vecs.Length;
		STMF_PathManager sTMF_PathManager = new STMF_PathManager();
		sTMF_PathManager.vecs = new Vector3[num];
		for (int i = 0; i < num; i++)
		{
			sTMF_PathManager.vecs[i] = path.vecs[i] + vec;
		}
		return sTMF_PathManager;
	}

	public void GetAllPath(int type)
	{
		STMF_PathManager sTMF_PathManager = smallFishPath[type];
		int num = sTMF_PathManager.vecs.Length;
		string text = string.Empty;
		for (int i = 0; i < num; i++)
		{
			string text2 = text;
			text = text2 + sTMF_PathManager.vecs[i][0].ToString() + "," + sTMF_PathManager.vecs[i][1].ToString() + "," + sTMF_PathManager.vecs[i][2].ToString() + " ";
		}
