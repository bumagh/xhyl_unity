...
public class WHN_GamblePanel : MonoBehaviour
{
	...
	public static WHN_GamblePanel ins;
	...
	{
		WHN_MB_Singleton<WHN_DiceGameController2>.GetInstance().Send_MultipleInfo(WHN_BetDiceType.Half);
		SetButton(canclick: false);
	}

	private void ClickBlack()
	{
		WHN_MB_Singleton<WHN_DiceGameController2>.GetInstance().Send_MultipleInfo(WHN_BetDiceType.Risk);
		SetButton(canclick: false);
	}
	...
	private Sprite Getsprite()
	{
		try
		{
			uint num = Getnum(WHN_MB_Singleton<WHN_DiceGameController2>.GetInstance().Gamelewinid);
	...
	{
		balancescore = WHN_GVars.winScorce;
		doublescore = WHN_GVars.winScorce * 2;
	...
	{
		WHN_MB_Singleton<WHN_DiceGameController2>.GetInstance().OnBtnReady_Click();
		WHN_MB_Singleton<WHN_DiceGameController2>.GetInstance().Hide();
		int credit = (!isGamble) ? (WHN_GVars.winScorce + WHN_GVars.credit) : AllScroce;
		WHN_MB_Singleton<WHN_MajorGameController>.GetInstance().SetCredit(credit);
