...
{
	public class LL_SimpleWriter : ObjectWriter
	{
		...
		public override void write(object ob, XmlTextWriter el)
		{
			...
			if (map.ContainsKey(ob))
			{
				...
				string value = (string)Serial.mapCSharpToWOX[ob.GetType().ToString()];
			...
			{
				el.WriteStartElement("object");
				string value2 = (string)Serial.mapCSharpToWOX[ob.GetType().ToString()];
		...
		{
			string text = (string)Serial.mapCSharpToWOX[ob.GetType().GetElementType().ToString()];
			if (text == null)
			{
				text = (string)Serial.mapArrayCSharpToWOX[ob.GetType().GetElementType().ToString()];
				if (text != null)
				{
		...
		public void writePrimitiveArray(object ob, XmlTextWriter el)
		{
			el.WriteStartElement("object");
			el.WriteAttributeString("type", "array");
			string value = (string)Serial.mapCSharpToWOX[ob.GetType().GetElementType().ToString()];
		...
		public string arrayString(Array obArray, int len)
		{
			...
			for (int i = 0; i < len; i++)
			{
				...
				if (obArray is Type[])
				{
					...
					if (type != null)
					{
						string text = (string)Serial.mapCSharpToWOX[type.ToString()];
						if (text != null)
						{
							stringBuilder.Append(text);
							continue;
						}
						text = (string)Serial.mapArrayCSharpToWOX[type.ToString()];
						if (text != null)
						{
							stringBuilder.Append(text);
						}
		...
		public void writeFields(object o, XmlTextWriter parent)
		{
			...
			for (int i = 0; i < fields.Length; i++)
			{
				try
				{
					...
					if (Serial.mapCSharpToWOX.ContainsKey(fields[i].FieldType.ToString()))
					{
						if (writePrimitiveTypes)
						{
							parent.WriteAttributeString("type", (string)Serial.mapCSharpToWOX[fields[i].FieldType.ToString()]);
						}
