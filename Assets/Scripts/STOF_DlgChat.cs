...
public class STOF_DlgChat : MonoBehaviour
{
	[SerializeField]
	...
	[HideInInspector]
	public STOF_ChatCtrl sptChatCtrl;
	...
	private void ClickBtnCommonUse(int index)
	{
		...
		STOF_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, ifChart.text);
	...
	private void ClickBtnSend()
	{
		if (ifChart.text != string.Empty)
		{
			...
			STOF_NetMngr.GetSingleton().MyCreateSocket.SendChat(chatType, chatUserId, chatMessage);
			base.gameObject.SetActive(value: false);
		}
		else
		{
