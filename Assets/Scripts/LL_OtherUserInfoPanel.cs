...
public class LL_OtherUserInfoPanel : MonoBehaviour
{
	...
	public void ShowOtherInfo(string strNickname, int nGameScore, int nUserLever, bool bIsNeedPrivateChat)
	{
		...
		{
			LL_ErrorManager.GetSingleton().AddError("玩家" + strNickname + "等级错误：" + nUserLever);
			nUserLever = 1;
		}
		...
		if (LL_GameInfo.getInstance().Language == 1)
		{
			...
			mLevelText.text = "[0bbffa]Level: [ffffff]lv." + nUserLever + "(" + LL_MyUICommon.gLeverName[nUserLever - 1] + ")";
			mGameScoreText.text = "[0bbffa]Credits: [ffffff]" + nGameScore;
		}
		else
		{
			mNickNameText.text = "[0bbffa]昵    称: [ffffff]" + strNickname;
			mLevelText.text = "[0bbffa]等    级: [ffffff]lv." + nUserLever + "(" + LL_MyUICommon.gLeverName[nUserLever - 1] + ")";
